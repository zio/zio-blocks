...
object DomSelection {
  ...
  private[template] def selectorMatches(el: Dom.Element, selector: CssSelector): Boolean =
    selector match {
   ...
   * itself).
   */
  private[template] def collectDescendants(
    node: Dom,
  ...
  private[template] def extractText(el: Dom.Element): String = {
   ...
   * attribute).
   */
  private[template] def hasClass(el: Dom.Element, cls: String): Boolean = {
    var i = 0
  ...
  private[template] def hasAttribute(el: Dom.Element, name: String): Boolean = {
    var i = 0
    while (i < el.attributes.length) {
      el.attributes(i) match {
  ...
  private[template] def hasAttributeValue(el: Dom.Element, name: String, value: String): Boolean =
  ...
  private[template] def getAttributeValue(el: Dom.Element, name: String): Option[String] = {
    var i = 0
    while (i < el.attributes.length) {
      el.attributes(i) match {
