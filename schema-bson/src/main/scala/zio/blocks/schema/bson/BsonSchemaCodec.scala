...
object BsonSchemaCodec {
        ...
        case None =>
          if (reflect.isRecord) {
            ...
            throw new UnsupportedOperationException(
              s"BSON codec for ${reflect.typeId} (type: ${reflect.nodeType}) is not yet implemented."
            )
          }
      }
    }
  ...
  private def deriveMapCodec[M[_, _], K, V](map: Reflect.Map.Bound[K, V, M], config: Config): BsonCodec[M[K, V]] = {
    ...
    if (isStringKey) {
      ...
      throw new UnsupportedOperationException(s"Map with non-string keys not yet supported for ${map.typeId}")
    }
  }
