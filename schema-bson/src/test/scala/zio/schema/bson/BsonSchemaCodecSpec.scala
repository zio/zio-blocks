...
import zio.bson._
import zio.blocks.schema.{Schema, Reflect, TypeName, Namespace}
...
object BsonSchemaCodecSpec extends ZIOSpecDefault {
  ...
  object SimpleClass {
    implicit val schema: Schema[SimpleClass] = Schema.derived
  ...
  object BigDecimalClass {
    implicit val schema: Schema[BigDecimalClass] = Schema.derived
  ...
  object EnumLike {
    ...
    implicit lazy val schema: Schema[EnumLike] = Schema.derived
  ...
  object Customer {
    implicit lazy val objectIdSchema: Schema[ObjectId] = new Schema(
      new Reflect.Wrapper[Binding, ObjectId, String](
        implicitly[Schema[String]].reflect,
        TypeName(new Namespace(List("org", "bson", "types")), "ObjectId"),
        None,
        new Binding.Wrapper(
          (s: String) => scala.util.Try(new ObjectId(s)).toEither.left.map(_.getMessage),
          (id: ObjectId) => id.toHexString
        )
      )
    )

    implicit lazy val customerIdSchema: Schema[CustomerId] = new Schema(
      new Reflect.Wrapper[Binding, CustomerId, ObjectId](
        objectIdSchema.reflect,
        TypeName(new Namespace(List("zio", "schema", "bson")), "CustomerId"),
        None,
        new Binding.Wrapper(
          (id: ObjectId) => Right(CustomerId(id)),
          (c: CustomerId) => c.value
        )
      )
    )

    implicit lazy val customerSchema: Schema[Customer] = Schema.derived[Customer]
  ...
  def spec: Spec[TestEnvironment with Scope, Any] = suite("BsonSchemaCodecSpec")(
    suite("round trip")(
      roundTripTest("SimpleClass")(
        Gen.string.zip(Gen.int).map { case (s, i) => SimpleClass(s, i) },
      ...
      roundTripTest("Customer")(
        ...
        Customer.example,
        doc(
          "id"             -> new BsonString(Customer.example.id.value.toHexString),
          "name"           -> str(Customer.example.name),
          "age"            -> int(Customer.example.age),
          "invitedFriends" -> array(
            Customer.example.invitedFriends.map(id => new BsonString(id.value.toHexString)): _*
          )
        )
      ),
    ...
    suite("configuration")(
      suite("schema annotations")(
        roundTripExamples[SchemaConfig.NoDiscriminator]("no discriminator")(
          SchemaConfig.NoDiscriminator.A("str") -> doc("A" -> doc("a" -> str("str"))),
          SchemaConfig.NoDiscriminator.B("str") -> doc("B" -> doc("b" -> str("str"))),
          SchemaConfig.NoDiscriminator.C("str") -> doc("C" -> doc("c" -> str("str")))
        ),
      ...
      suite("mixed annotations (bson priority)")(
        roundTripExamples[MixedConfig.NoDiscriminator]("no discriminator")(
          MixedConfig.NoDiscriminator.A("str") -> doc("A" -> doc("a" -> str("str"))),
          MixedConfig.NoDiscriminator.B("str") -> doc("B" -> doc("b" -> str("str"))),
          MixedConfig.NoDiscriminator.C("str") -> doc("C" -> doc("c" -> str("str")))
        ),
