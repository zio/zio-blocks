...
class JdbcTransactor(
  ...
  def transact[A](f: DbTx ?=> A): A = {
    ...
    if (dialect == SqlDialect.SQLite) {
      ...
      // transaction implicitly, making a subsequent BEGIN IMMEDIATE fail with
      ...
      try {
        ...
        val result = f
        try {
          val commitStmt = conn.createStatement()
          if (commitStmt != null) {
            try commitStmt.execute("COMMIT")
            finally
              try commitStmt.close()
              catch { case _: Throwable => () }
          }
        } catch { case _: Throwable => () }
        result
        ...
        case e: Throwable =>
          try {
            val rbStmt = conn.createStatement()
            if (rbStmt != null) {
              try rbStmt.execute("ROLLBACK")
              finally
                try rbStmt.close()
                catch { case _: Throwable => () }
            }
