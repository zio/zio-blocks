  ...
  def spec = suite("ExprSpec")(
    suite("where filters")(
      test("simple comparison renders qualified column with placeholder") {
        val q    = SqlQuery.from(userTable).where(col[User](_.name) === lit("Alice"))
      ...
      test("and combinator renders parenthesized AND with two placeholders") {
        val q = SqlQuery
          .from(userTable)
          .where((col[User](_.age) > lit(21)) && col[User](_.name).like("a%"))
        val frag = q.toFrag(SqlDialect.PostgreSQL)
        val exp  =
      ...
      test("or and not combinators") {
        val qNot   = SqlQuery.from(userTable).where(!(col[User](_.age) > lit(18)))
        val expNot =
          "SELECT t0.\"id\", t0.\"name\", t0.\"age\", t0.\"salary\" FROM \"user\" AS t0 WHERE NOT (t0.\"age\" > ?)"
        val qOr   = SqlQuery.from(userTable).where((col[User](_.age) < lit(10)) || (col[User](_.name) === lit("Bob")))
        val expOr =
      ...
      test("comparison operators cover = < > <= >= <>") {
        val q = SqlQuery
          .from(userTable)
          .where(col[User](_.id) =!= lit(5))
        val exp = "SELECT t0.\"id\", t0.\"name\", t0.\"age\", t0.\"salary\" FROM \"user\" AS t0 WHERE t0.\"id\" <> ?"
        assertTrue(q.toFrag(SqlDialect.PostgreSQL).sql(SqlDialect.PostgreSQL) == exp)
      },
      test("in combinator renders IN list with placeholders") {
        val q    = SqlQuery.from(userTable).where(col[User](_.id).in(Seq(1, 2, 3)))
        val frag = q.toFrag(SqlDialect.PostgreSQL)
        val exp  =
      ...
      test("arithmetic plus renders qualified arithmetic") {
        val q   = SqlQuery.from(userTable).where((col[User](_.age) + lit(5)) > lit(30))
        val exp =
      ...
      test("qualified column respects join alias") {
        ...
        val q   = SqlQuery.from(userTable).innerJoin(rel).where(col[Repo](_.name) === lit("foo"))
        val exp =
      ...
      test("placeholder count equals params count") {
        val q            = SqlQuery.from(userTable).where((col[User](_.id) > lit(1)) && (col[User](_.name) === lit("a")))
        val frag         = q.toFrag(SqlDialect.PostgreSQL)
      ...
      test("repeated dialect rendering is deterministic and stateless") {
        val q  = SqlQuery.from(userTable).where(col[User](_.name) === lit("x"))
        val p1 = q.toFrag(SqlDialect.PostgreSQL).sql(SqlDialect.PostgreSQL)
        val p2 = q.toFrag(SqlDialect.PostgreSQL).sql(SqlDialect.PostgreSQL)
    ...
    suite("groupBy and having")(
      test("groupBy single column renders qualified GROUP BY") {
        val q   = SqlQuery.from(userTable).groupBy(col[User](_.name))
        val exp = "SELECT t0.\"id\", t0.\"name\", t0.\"age\", t0.\"salary\" FROM \"user\" AS t0 GROUP BY t0.\"name\""
        assertTrue(q.toFrag(SqlDialect.PostgreSQL).sql(SqlDialect.PostgreSQL) == exp)
      },
      test("groupBy multiple columns") {
        val q   = SqlQuery.from(userTable).groupBy(col[User](_.name), col[User](_.age))
        val exp =
      ...
      test("groupBy with join alias") {
        val rel = Rel.manyToOne(repoTable, _.ownerId, userTable, _.id)
        val q   = SqlQuery.from(userTable).innerJoin(rel).groupBy(col[Repo](_.name))
        val exp =
      ...
      test("having with count aggregate renders HAVING COUNT > ?") {
        val q   = SqlQuery.from(userTable).groupBy(col[User](_.name)).having(count(col[User](_.id)) > lit(1L))
        val exp =
      ...
      test("having with sum/avg/min/max") {
        val qSum   = SqlQuery.from(userTable).groupBy(col[User](_.name)).having(sum(col[User](_.salary)) > lit(1000.0))
        val expSum =
          "SELECT t0.\"id\", t0.\"name\", t0.\"age\", t0.\"salary\" FROM \"user\" AS t0 GROUP BY t0.\"name\" HAVING SUM(t0.\"salary\") > ?"
        val qAvg   = SqlQuery.from(userTable).groupBy(col[User](_.name)).having(avg(col[User](_.salary)) > lit(500.0))
        val expAvg =
          "SELECT t0.\"id\", t0.\"name\", t0.\"age\", t0.\"salary\" FROM \"user\" AS t0 GROUP BY t0.\"name\" HAVING AVG(t0.\"salary\") > ?"
        val qMin   = SqlQuery.from(userTable).groupBy(col[User](_.name)).having(min(col[User](_.age)) > lit(18))
        val expMin =
          "SELECT t0.\"id\", t0.\"name\", t0.\"age\", t0.\"salary\" FROM \"user\" AS t0 GROUP BY t0.\"name\" HAVING MIN(t0.\"age\") > ?"
        val qMax   = SqlQuery.from(userTable).groupBy(col[User](_.name)).having(max(col[User](_.salary)) > lit(2000.0))
        val expMax =
      ...
      test("countStar renders COUNT(*) in having") {
        val q   = SqlQuery.from(userTable).groupBy(col[User](_.name)).having(countStar > lit(2L))
        val exp =
      ...
      test("having placeholder count equals params") {
        val q            = SqlQuery.from(userTable).groupBy(col[User](_.name)).having(count(col[User](_.id)) > lit(5L))
        val frag         = q.toFrag(SqlDialect.PostgreSQL)
    ...
    suite("self-join alias binding")(
      test("self-join ambiguous col throws when rendering") {
        val rel    = Rel.manyToOne(employeeTable, _.managerId, employeeTable, _.id)
        val q      = SqlQuery.from(employeeTable).innerJoin(rel)
        val failed = try {
          q.where(col[Employee](_.name) === lit("x")).toFrag(SqlDialect.PostgreSQL).sql(SqlDialect.PostgreSQL)
          None
        } catch {
          case e: IllegalArgumentException => Some(e.getMessage)
        }
        assertTrue(failed.isDefined, failed.get.contains("ambiguous"))
      },
      test("self-join alias-qualified colAt binds to requested alias") {
        val rel = Rel.manyToOne(employeeTable, _.managerId, employeeTable, _.id)
        val q   = SqlQuery.from(employeeTable).innerJoin(rel).where(colAt[Employee]("t1", _.name) === lit("boss"))
        val exp =
      ...
      test("String arithmetic must not compile") {
        val errors = scala.compiletime.testing.typeCheckErrors(
          """{
            ...
            case class User(id: Int, name: String, age: Int)
            object User { given Schema[User] = Schema.derived }
            val bad = (col[User](_.name) + lit("x")) > lit("y")
          }"""
        )
        assertTrue(errors.nonEmpty)
      },
      test("mismatched comparison must not compile") {
        val errors = scala.compiletime.testing.typeCheckErrors(
          """{
            ...
            case class User(id: Int, name: String, age: Int)
            object User { given Schema[User] = Schema.derived }
            val bad = col[User](_.age) === lit("string")
          }"""
        )
        assertTrue(errors.nonEmpty)
      },
    ...
    suite("renamed field and custom table name mapping")(
      test("renamed field uses Modifier.rename column name") {
        ...
        val q            = SqlQuery.from(renamedTable).where(col[RenamedUser](_.name) === lit("Alice"))
      ...
      test("custom table name via Modifier.config is respected for alias") {
        ...
        val q   = SqlQuery.from(customTable).where(col[CustomUser](_.name) === lit("Bob"))
        val sql = q.toFrag(SqlDialect.PostgreSQL).sql(SqlDialect.PostgreSQL)
    ...
    suite("alias validation")(
      test("invalid alias throws at render time") {
        val q      = SqlQuery.from(userTable).where(colAt[User]("t99", _.name) === lit("x"))
        val failed = try {
          q.toFrag(SqlDialect.PostgreSQL).sql(SqlDialect.PostgreSQL)
          None
        } catch { case e: IllegalArgumentException => Some(e.getMessage) }
        assertTrue(failed.isDefined, failed.get.contains("unknown alias") || failed.get.contains("not found"))
      },
      test("foreign alias (Repo alias for User column) throws") {
        val rel = Rel.manyToOne(repoTable, _.ownerId, userTable, _.id)
        val q   = SqlQuery.from(userTable).innerJoin(rel).where(colAt[User]("t1", _.name) === lit("x"))
        // t1 is Repo, but col is User -> should throw table mismatch/ambiguous
        val failed = try {
          q.toFrag(SqlDialect.PostgreSQL).sql(SqlDialect.PostgreSQL)
          None
        } catch { case e: IllegalArgumentException => Some(e.getMessage) }
        assertTrue(
          failed.isDefined,
          failed.get.contains("t1") && failed.get.contains("repo") && failed.get.contains("user")
        )
      }
    ),
    suite("second-repair type-safety and exact rendering")(
      ...
      test("Boolean ordering < must not compile") {
        val errors = scala.compiletime.testing.typeCheckErrors(
          """{
            ...
            val bad = col[Flag](_.flag) < lit(true)
          }"""
        )
        assertTrue(errors.nonEmpty)
      },
      test("exact custom-table SQL full-string") {
        ...
        val q           = SqlQuery.from(customTable).where(col[CustomUser2](_.name) === lit("Bob"))
        val sql         = q.toFrag(SqlDialect.PostgreSQL).sql(SqlDialect.PostgreSQL)
        val exp         = "SELECT t0.\"id\", t0.\"name\" FROM \"my_users2\" AS t0 WHERE t0.\"name\" = ?"
        assertTrue(sql == exp, q.toFrag(SqlDialect.PostgreSQL).params == IndexedSeq(DbValue.DbString("Bob")))
      },
      test("placeholder parity for IN") {
        val q    = SqlQuery.from(userTable).where(col[User](_.id).in(Seq(1, 2, 3, 4, 5)))
        val frag = q.toFrag(SqlDialect.PostgreSQL)
      ...
      test("placeholder parity for aggregate having with IN") {
        val q    = SqlQuery.from(userTable).groupBy(col[User](_.name)).having(count(col[User](_.id)) > lit(2L))
        val frag = q.toFrag(SqlDialect.PostgreSQL)
        assertTrue(frag.sql(SqlDialect.PostgreSQL).count(_ == '?') == frag.params.size)
        val q2    = SqlQuery.from(userTable).where(col[User](_.id).in(Seq(10, 20)))
        val frag2 = q2.toFrag(SqlDialect.PostgreSQL)
        assertTrue(
          frag2.sql(
