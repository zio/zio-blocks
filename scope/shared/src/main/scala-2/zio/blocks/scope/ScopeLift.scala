...
object ScopeLift extends ScopeLiftMidPriority {
   ...
   * of use-after-close.
   */
  implicit def globalScope[A]: ScopeLift.Aux[A, Scope.GlobalTag, A] =
...
trait ScopeLiftMidPriority extends ScopeLiftLowPriority {
   ...
   */
  implicit def nothing[S]: ScopeLift.Aux[Nothing, S, Nothing] =
   ...
   */
  implicit def unscoped[A, S](implicit ev: Unscoped[A]): ScopeLift.Aux[A, S, A] =
    new ScopeLift[A, S] {
      type Out = A
      def apply(a: A): Out = a
    }
...
trait ScopeLiftLowestPriority {
   ...
   *
   * When the inner type `A` is `Unscoped` (pure data), it's safe to extract
   * the raw value from `A @@ T` regardless of the scope tag `T`. The value
   * is evaluated while the scope is still open, and the result is pure data
   * that doesn't hold resources.
   */
  implicit def scopedUnscoped[A, T, S](implicit ev: Unscoped[A]): ScopeLift.Aux[A @@ T, S, A] =
    new ScopeLift[A @@ T, S] {
      type Out = A
