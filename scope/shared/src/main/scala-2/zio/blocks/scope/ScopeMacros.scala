...
private[scope] object ScopeMacros {
   ...
   * Used to understand what the compiler sees for path-dependent types.
   */
  def debugTypeImpl[A: c.WeakTypeTag](c: whitebox.Context)(value: c.Expr[A]): c.Expr[A] = {
    import c.universe._
    
    val tpe = weakTypeOf[A]
    
    // Print various representations of the type (info doesn't fail compilation)
    c.info(c.enclosingPosition, s"""
      |=== DEBUG TYPE INFO ===
      |Type: $tpe
      |Type (showRaw): ${showRaw(tpe)}
      |TypeSymbol: ${tpe.typeSymbol}
      |TypeArgs: ${tpe.typeArgs}
      |Dealias: ${tpe.dealias}
      |Dealias (showRaw): ${showRaw(tpe.dealias)}
      |Widen: ${tpe.widen}
      |=======================""".stripMargin, force = true)
    
    value
  }

  /**
   * MockScope.scoped macro implementation (test prototype).
   * 
   * This macro:
   * 1. Takes a lambda (SAM converted to MockScopeUser)
   * 2. Type-checks the lambda to get the return type
   * 3. Extracts the underlying type from child.Dollar[T]
   * 4. Validates Unscoped[T]
   * 5. Returns properly typed T
   */
  def mockScopedImpl(c: whitebox.Context)(f: c.Tree): c.Tree = {
    ...
    // For lambdas, the body's type is the actual return type (e.g., child.$[Int])
    // Also capture the lambda parameter name to validate scope ownership
    val (bodyType: Type, lambdaParamName: Option[TermName]) = fTyped match {
      case Function(List(param), body) =>
        c.info(c.enclosingPosition, s"mockScopedImpl: Lambda param = ${param.name}, body type = ${body.tpe}", force = true)
        (body.tpe.widen, Some(param.name))
      case Block(_, Function(List(param), body)) =>
        c.info(c.enclosingPosition, s"mockScopedImpl: Block->Lambda param = ${param.name}, body type = ${body.tpe}", force = true)
        (body.tpe.widen, Some(param.name))
      case _ =>
    ...
    c.info(c.enclosingPosition, s"mockScopedImpl: bodyType=$bodyType", force = true)
    ...
    val underlyingType = bodyType match {
      case TypeRef(pre, sym, args) if sym.name.toString == "$" && args.nonEmpty =>
        ...
        c.info(c.enclosingPosition, s"mockScopedImpl: Scoped type $bodyType", force = true)
        ...
        c.info(c.enclosingPosition, s"  prefixName = $prefixName, lambdaParamName = $lambdaParamName", force = true)
        ...
        underlying
      case _ =>
        c.info(c.enclosingPosition, s"mockScopedImpl: Plain type $bodyType", force = true)
        bodyType
    }
    ...
    q"""
      val parent = $self
      val child = new _root_.zio.blocks.scope.MockScope.Child[parent.type](parent)
