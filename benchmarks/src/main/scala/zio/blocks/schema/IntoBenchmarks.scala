...
// Coproduct types for benchmark - defined at package level for macro access
// Note: Signatures must match for macro to work (it matches by signature, then converts field types)
// Case objects must have matching names for name-based matching to work
object BenchmarkEvents {
  ...
  object EventV2 {
    case class Created(name: String, id: Int) extends EventV2 // Same signature, field conversion happens recursively
    case object Deleted                       extends EventV2 // Same name for name-based matching
  }
}
...
class IntoBenchmarks {
  ...
  val eventInto  = Into.derived[BenchmarkEvents.EventV1, BenchmarkEvents.EventV2]
  ...
  @Benchmark
  def nestedProductConversion(): Any = {
  ...
  def coproductConversion(): Any =
    eventInto.intoOrThrow(BenchmarkEvents.EventV1.Created("test", 42))
  ...
  def repeatedConversion(): PersonV2 = {
    ...
    // Convert PersonV1 -> PersonV2 multiple times
    var result = personInto.intoOrThrow(personV1)
    for (_ <- 1 to 100) {
      // Each conversion creates a new PersonV2 instance
      result = personInto.intoOrThrow(personV1)
    }
    result
  }
