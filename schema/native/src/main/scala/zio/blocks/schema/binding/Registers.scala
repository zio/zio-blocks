...
class Registers private (userRegister: RegisterOffset) {
  ...
  def getBoolean(offset: RegisterOffset): Boolean = ByteArrayAccess.getBoolean(bytes, RegisterOffset.getBytes(offset))
  ...
  def setBoolean(offset: RegisterOffset, value: Boolean): Unit = {
    ...
    ByteArrayAccess.setBoolean(bytes, idx, value)
  }
