...
final class JsonReader private[json] (
  ...
  def readKeyAsLocalDateTime(): LocalDateTime = {
    nextTokenOrError('"', head)
    val x = parseLocalDateTime(false)
    nextTokenOrError(':', head)
    x
  }
  ...
  def readKeyAsMonthDay(): MonthDay = {
    nextTokenOrError('"', head)
    val x = parseMonthDay(head, false)
    nextTokenOrError(':', head)
    x
  }
  ...
  def readKeyAsOffsetDateTime(): OffsetDateTime = {
    nextTokenOrError('"', head)
    val x = parseOffsetDateTime(false)
    nextTokenOrError(':', head)
    x
  }
  ...
  def readKeyAsZonedDateTime(): ZonedDateTime = {
    nextTokenOrError('"', head)
    val x = parseZonedDateTime(false)
    nextTokenOrError(':', head)
    x
  }
  ...
  /**
   * Reads a JSON key into a `BigInt` instance with the default limit of allowed
   * digits.
   *
   * @return
  ...
  /**
   * Reads a JSON key into a `BigInt` instance with the given limit of allowed
   * digits.
   *
   * @param digitsLimit
  ...
  /**
   * Reads a JSON key into a `BigDecimal` instance with the default limit of
   * allowed digits for mantissa, the default limit for scale, and the default
   * instance of [[java.math.MathContext]] for precision.
   *
   * @return
  ...
  /**
   * Reads a JSON key into a Scala `BigDecimal` instance with the given
   * precision, scale limit, and digits limit.
   *
   * @param mc
  ...
  def readYearMonth(default: YearMonth): YearMonth =
    if (isNextToken('"', head)) parseYearMonth(false)
    else {
      head -= 1
      readNullOrTokenError(default, '"')
    }
  ...
  /**
   * Reads a JSON string value into the internal char buffer and returns the
  ...
  /**
   * Reads a raw JSON value into an [[java.time.Duration]] instance.
   *
   * @return
  ...
  /**
   * Reads a raw JSON value into an [[java.time.Instant]] instance.
   *
   * @return
  ...
  /**
   * Reads a raw JSON value into an [[java.time.LocalDate]] instance.
   *
   * @return
  ...
  /**
   * Reads a raw JSON value into an [[java.time.LocalDateTime]] instance.
   *
   * @return
  ...
  /**
   * Reads a raw JSON value into an [[java.time.LocalTime]] instance.
   *
   * @return
  ...
  /**
   * Reads a raw JSON value into an [[java.time.MonthDay]] instance.
   *
   * @return
  ...
  /**
   * Reads a raw JSON value into an [[java.time.OffsetDateTime]] instance.
   *
   * @return
  ...
  /**
   * Reads a raw JSON value into an [[java.time.OffsetTime]] instance.
   *
   * @return
  ...
  /**
   * Reads a raw JSON value into an [[java.time.Period]] instance.
   *
   * @return
   ...
   *   in cases of reaching the end of input or invalid type of JSON value
   */
  private[json] def readRawValAsYear(): Year = Year.of(parseYear(9, head))
   ...
   *   in cases of reaching the end of input or invalid type of JSON value
   */
  private[json] def readRawValAsYearMonth(): YearMonth = parseYearMonth(true)
  ...
  private[json] def endOfInputOrError(): Unit =
    if (skipWhitespaces()) decodeError("expected end of input")
  ...
  private[this] def nextTokenOrError(t: Byte, pos: Int): Unit =
    if (pos < tail) {
      val b = buf(pos)
      head = pos + 1
      if (b != t && ((b != ' ' && b != '\n' && (b | 0x4) != '\r') || nextToken(pos + 1) != t)) tokenError(t)
  ...
  private[this] def parseYearWithByte(t: Byte, maxDigits: Int, pos: Int): Int =
    if (pos + 4 < tail) {
      ...
      if (b1 >= '0' && b1 <= '9') {
        ...
        val b5 = buf(pos + 4)
        head = pos + 5
        if (b2 < '0' || b2 > '9' || b3 < '0' || b3 > '9' || b4 < '0' || b4 > '9') digitError()
        if (b5 != t) tokenError(t)
  ...
  private[this] def parseNon4DigitYearWithByte(t: Byte, maxDigits: Int, b1: Byte, p: Int): Int = {
    ...
    val b4  = buf(pos + 3)
    val b5  = buf(pos + 4)
    if (b1 != '-' && b1 != '+') decodeError("expected '-' or '+' or digit")
    if (b2 < '0' || b2 > '9' || b3 < '0' || b3 > '9' || b4 < '0' || b4 > '9' || b5 < '0' || b5 > '9') digitError()
    ...
    if (b != t) yearError(t, maxDigits, b1, yearDigits)
    if (b1 == '-') year = -year
  ...
  @tailrec
  private[this] def parseYear(maxDigits: Int, pos: Int): Int =
    if (pos + 3 < tail) {
      val buf = this.buf
      val b1  = buf(pos)
      if (b1 >= '0' && b1 <= '9') {
        ...
        if (b2 < '0' || b2 > '9' || b3 < '0' || b3 > '9' || b4 < '0' || b4 > '9') digitError()
        b1 * 1000 + b2 * 100 + b3 * 10 + b4 - 53328 // 53328 == '0' * 1111
      } else parseNon4DigitYear(maxDigits, b1, pos)
    } else parseYear(maxDigits, loadMoreOrError(pos))

  private[this] def parseNon4DigitYear(maxDigits: Int, b1: Byte, p: Int): Int = {
    var pos = p
    var buf = this.buf
    val b2  = buf(pos + 1)
    val b3  = buf(pos + 2)
  ...
  private[this] def parseOffsetMinute(pos: Int): Int =
    if (pos + 1 < tail) {
      ...
      if (b1 == '0' && b2 == '0') 0
        ...
        if (b1 < '0' || b1 > '9' || b2 < '0' || b2 > '9') digitError()
        if (b1 > '5') timezoneOffsetMinuteError()
        b1 * 10 + b2 - 528 // 528 == '0' * 11
      }
  ...
  private[this] def parseOffsetSecond(pos: Int): Int =
    if (pos + 1 < tail) {
      ...
      head = pos + 2
      if (b1 < '0' || b1 > '9' || b2 < '0' || b2 > '9') digitError()
      if (b1 > '5') timezoneOffsetSecondError()
      b1 * 10 + b2 - 528 // 528 == '0' * 11
  ...
  private[this] def parseBoolean(isToken: Boolean, pos: Int): Boolean =
    if (pos + 3 < tail) {
      ...
      val b1  = buf(pos)
      if (b1 == 't') {
        if (buf(pos + 1) != 'r' || buf(pos + 2) != 'u' || buf(pos + 3) != 'e') booleanError()
        head = pos + 4
        true
      } else if (b1 == 'f') {
        if (pos + 4 < tail) {
          if (buf(pos + 1) != 'a' || buf(pos + 2) != 'l' || buf(pos + 3) != 's' || buf(pos + 4) != 'e') booleanError()
          head = pos + 5
          false
  ...
  private[this] def booleanError(): Nothing = decodeError("illegal boolean")

  private[this] def readByte(isToken: Boolean): Byte = {
    var b =
  ...
  private[this] def readShort(isToken: Boolean): Short = {
    var b =
      if (isToken) nextToken(head)
      else nextByte(head)
    var s = 0
  ...
  private[this] def readInt(isToken: Boolean): Int = {
    var b =
      if (isToken) nextToken(head)
      else nextByte(head)
    var s = -1
    ...
    if (isToken && x == 0) ensureNotLeadingZero()
      ...
      x ^= s
      x -= s
      if ((s & x) == -2147483648) intOverflowError()
      if ((b | 0x20) == 'e' || b == '.') numberError()
    }
    x
  }

  private[this] def readLong(isToken: Boolean): Long = {
    var b =
      if (isToken) nextToken(head)
      else nextByte(head)
    var isPos = true
    ...
    if (isToken && x1 == 0) {
      ...
      head = pos
      if (isPos) {
        if (x == -9223372036854775808L) longOverflowError()
        x = -x
      }
      if ((b | 0x20) == 'e' || b == '.') numberError()
      x
  ...
  private[this] def readDouble(isToken: Boolean): Double = {
    var b =
      if (isToken) nextToken(head)
      else nextByte(head)
    var isNeg = false
  ...
  private[this] def toDouble(pos: Int): Double = {
    val from = marks(markNum - 1)
    java.lang.Double.parseDouble(new String(buf, from, pos - from))
  }
  ...
  private[this] def toFloat(pos: Int): Float = {
    val from = marks(markNum - 1)
    java.lang.Float.parseFloat(new String(buf, from, pos - from))
  }
  ...
  private[this] def digitsLimitError(): Nothing = decodeError("value exceeds limit for number of digits")

  private[this] def scaleLimitError(): Nothing = decodeError("value exceeds limit for scale")

  private[this] def leadingZeroError(): Nothing = decodeError("illegal number with leading zero")

  private[this] def byteOverflowError(): Nothing = decodeError("value is too large for byte")

  private[this] def shortOverflowError(): Nothing = decodeError("value is too large for short")

  private[this] def intOverflowError(): Nothing = decodeError("value is too large for int")

  private[this] def longOverflowError(): Nothing = decodeError("value is too large for long")

  private[this] def parseDuration(isRaw: Boolean): Duration = {
    var b = nextByte(head)
    var s = 0
    if (b == '-') {
      b = nextByte(head)
    ...
    while ({
      ...
      if (b == 'D' && state < 0) {
        if (x < -106751991167300L || x > 106751991167300L)
          durationError() // -106751991167300L == Long.MinValue / 86400
        seconds = x * 86400
        state = 0
      } else if (b == 'H' && state <= 0) {
        if (x < -2562047788015215L || x > 2562047788015215L)
          durationError() // -2562047788015215L == Long.MinValue / 3600
        seconds = sumSeconds((x << 12) - (x << 9) + (x << 4), seconds)
        state = 1
      } else if (b == 'M' && state <= 1) {
        if (x < -153722867280912930L || x > 153722867280912930L)
          durationError() // -153722867280912930L == Long.MinValue / 60
        seconds = sumSeconds(((x << 4) - x) << 2, seconds)
        state = 2
      } else if (b == 'S' || b == '.') {
        ...
        if (b == '.') {
          ...
          if (b != 'S') nanoError(nanoDigitWeight, 'S')
          nano = (nano ^ sx) - sx
        }
      } else durationError(state)
      if (isRaw) {
        head = pos + 1
        if (hasRemaining()) {
          b = nextByte(head)
  ...
  private[this] def parseInstant(isRaw: Boolean): Instant = {
    ...
    if (b == 'Z') {
      if (!isRaw) nextByteOrError('"', head)
    } else if (b == '-') {
      epochSecond +=
        (if (isRaw) parseOffsetTotal(head)
         else parseOffsetTotalWithDoubleQuotes(head))
    } else if (b == '+') {
      epochSecond -=
        (if (isRaw) parseOffsetTotal(head)
         else parseOffsetTotalWithDoubleQuotes(head))
  ...
  private[this] def parseLocalDate(isRaw: Boolean): LocalDate = {
    ...
    val month = parseMonthWithByte('-', head)
    val day   =
      if (isRaw) parseDay(year, month, head)
      else parseDayWithByte(year, month, '"', head)
    LocalDate.of(year, month, day)
  }

  private[this] def parseLocalDateTime(isRaw: Boolean): LocalDateTime = {
    val year   = parseYearWithByte('-', 9, head)
    val month  = parseMonthWithByte('-', head)
    val day    = parseDayWithByte(year, month, 'T', head)
    ...
    if (b == ':') {
      ...
      nano =
        if (isRaw) parseOptionalNano()
        else parseOptionalNanoWithDoubleQuotes()
    } else {
      if (isRaw) {
        if (b >= 0) tokenError(':')
      } else {
  ...
  private[this] def parseLocalTime(isRaw: Boolean): LocalTime = {
    val hour   = parseHourWithColon(head)
    val minute = parseMinute(head)
    val b      =
      if (isRaw) {
  ...
  @tailrec
  private[this] def parseMonthDay(pos: Int, isRaw: Boolean): MonthDay =
    if (isRaw) {
      if (pos + 6 < tail) {
        val buf = this.buf
        if (buf(pos) != '-' || buf(pos + 1) != '-') tokenError('-')
        val b3    = buf(pos + 2)
        val b4    = buf(pos + 3)
        val month = b3 * 10 + b4 - 528 // 528 == '0' * 11
        if (b3 < '0' || b3 > '9' || b4 < '0' || b4 > '9') digitError()
        if (month < 1 || month > 12) monthError()
        if (buf(pos + 4) != '-') tokenError('-')
        val b6  = buf(pos + 5)
        val b7  = buf(pos + 6)
        val day = b6 * 10 + b7 - 528 // 528 == '0' * 11
        head = pos + 7
        if (b6 < '0' || b6 > '9' || b7 < '0' || b7 > '9') digitError()
        if (day == 0 || (day > 28 && day > maxDayForMonth(month))) dayError()
        MonthDay.of(month, day)
      } else parseMonthDay(loadMoreOrError(pos), isRaw)
    } else {
      if (pos + 7 < tail) {
        val buf = this.buf
        if (buf(pos) != '-' || buf(pos + 1) != '-') tokenError('-')
        ...
        if (b6 < '0' || b6 > '9' || b7 < '0' || b7 > '9') digitError()
        if (day == 0 || (day > 28 && day > maxDayForMonth(month))) dayError()
        if (buf(pos + 7) != '"') tokenError('"')
        MonthDay.of(month, day)
      } else parseMonthDay(loadMoreOrError(pos), isRaw)
    }

  private[this] def parseOffsetDateTime(isRaw: Boolean): OffsetDateTime = {
    val year   = parseYearWithByte('-', 9, head)
    val month  = parseMonthWithByte('-', head)
    val day    = parseDayWithByte(year, month, 'T', head)
    val hour   = parseHourWithColon(head)
  ...
  private[this] def parseOffsetTime(isRaw: Boolean): OffsetTime = {
    val hour   = parseHourWithColon(head)
    val minute = parseMinute(head)
    var b      =
      if (isRaw) {
  ...
  private[this] def parsePeriod(isRaw: Boolean): Period = {
    var b = nextByte(head)
    var s = 0
    if (b == '-') {
      b = nextByte(head)
    ...
    while ({
      ...
      x = sx - (x ^ sx)
      if ((sx | x) == -2147483648) periodError()
      if (b == 'Y' && state <= 0) {
        years = x
        state = 1
      } else if (b == 'M' && state <= 1) {
        ...
        days += x
        state = 4
        if (((x ^ days) & (d ^ days)) < 0) periodError()
      } else periodError(state)
      if (isRaw) {
        head = pos + 1
        if (hasRemaining()) {
          b = nextByte(head)
  ...
  private[this] def parseYearMonth(isRaw: Boolean): YearMonth =
    YearMonth.of(
      parseYearWithByte('-', 9, head),
      if (isRaw) parseMonth(head)
      else parseMonthWithByte('"', head)
    )

  private[this] def parseZonedDateTime(isRaw: Boolean): ZonedDateTime = {
    val year   = parseYearWithByte('-', 9, head)
    val month  = parseMonthWithByte('-', head)
    val day    = parseDayWithByte(year, month, 'T', head)
    val hour   = parseHourWithColon(head)
    ...
    } else if (isRaw || b == '"') ZonedDateTime.ofLocal(localDateTime, zoneOffset, null)
  ...
  private[this] def digitError(): Nothing = decodeError("expected digit")

  private[this] def periodError(): Nothing = decodeError("illegal period")

  private[this] def periodError(state: Int): Nothing = decodeError(
    state match {
  ...
  private[this] def durationError(): Nothing = decodeError("illegal duration")

  private[this] def durationError(state: Int): Nothing = decodeError(
    state match {
  ...
  private[this] def yearError(t: Byte, maxDigits: Int, b: Byte, yearDigits: Int): Nothing = {
    if (b != '-' && yearDigits == 4) digitError()
    if (yearDigits == maxDigits) tokenError(t)
    tokenOrDigitError(t)
  }
  ...
  private[this] def timezoneError(): Nothing = decodeError("illegal timezone")
    ...
    if (pos + 36 < tail) {
      ...
      val mostSigBits2 =
        ns(buf(pos + 9) & 0xff) << 12 |
          ...
          ns(buf(pos + 12) & 0xff)
      if (mostSigBits2 < 0) hexDigitError()
      if (buf(pos + 13) != '-') tokenError('-')
      val mostSigBits3 =
        ns(buf(pos + 14) & 0xff) << 12 |
          ns(buf(pos + 15) & 0xff) << 8 |
          ns(buf(pos + 16) & 0xff) << 4 |
          ns(buf(pos + 17) & 0xff)
      if (mostSigBits3 < 0) hexDigitError()
      if (buf(pos + 18) != '-') tokenError('-')
      val leastSigBits1 =
        ns(buf(pos + 19) & 0xff) << 12 |
          ns(buf(pos + 20) & 0xff) << 8 |
          ns(buf(pos + 21) & 0xff) << 4 |
          ns(buf(pos + 22) & 0xff)
      if (leastSigBits1 < 0) hexDigitError()
      if (buf(pos + 23) != '-') tokenError('-')
      val leastSigBits2 =
      ...
      head = pos + 37
      new UUID(
        mostSigBits1 << 32 | mostSigBits2.toLong << 16 | mostSigBits3,
      ...
      if (remaining > 0) {
        ...
        if (b1 >= 0) {
          if (b1 == '"') {
            ...
            if (b1 < ' ') unescapedControlCharacterError()
            charBuf(i) = b1.toChar
            ...
            if (b2 != 'u') {
              ...
              if ((ch1 & 0xf800) != 0xd800) parseEncodedString(i + 1, lim, charBuf, pos + 6)
                ...
                if (buf(pos + 6) != '\\' || buf(pos + 7) != 'u') escapeSequenceError()
                val ch2 = readEscapedUnicode(pos + 8, buf)
                charBuf(i + 1) = ch2
                if (ch1 >= 0xdc00 || (ch2 & 0xfc00) != 0xdc00) decodeError("illegal surrogate character pair")
          ...
          if (remaining > 1) {
            ...
            charBuf(i) = ch
            if ((b2 & 0xc0) != 0x80 || ch < 0x80) malformedBytesError(b1, b2)
            parseEncodedString(i + 1, lim, charBuf, pos + 2)
          } else parseEncodedString(i, lim, charBuf, loadMoreOrError(pos))
      ...
      if (b1 >= 0) {
        if (b1 == '"') characterError()
          ...
          if (b2 != 'u') {
            ...
            head = pos + 6
            if ((ch & 0xf800) == 0xd800) surrogateCharacterError()
            return ch
          }
        }
      } else if ((b1 & 0xe0) == 0xc0) { // 110bbbbb 10aaaaaa (UTF-8 bytes) -> 00000bbbbbaaaaaa (UTF-16 char)
  ...
  private[this] def readEscapedUnicode(pos: Int, buf: Array[Byte]): Char = {
    ...
    val x  =
      ns(buf(pos) & 0xff) << 12 |
        ...
        ns(buf(pos + 2) & 0xff) << 4 |
        ns(buf(pos + 3) & 0xff)
    if (x < 0) hexDigitError()
    x.toChar
  }

  private[this] def hexDigitError(): Nothing = decodeError("expected hex digit")

  private[this] def characterError(): Nothing = decodeError("illegal character")

  private[this] def escapeSequenceError(): Nothing = decodeError("illegal escape sequence")

  private[this] def surrogateCharacterError(): Nothing = decodeError("illegal surrogate character")

  private[this] def unescapedControlCharacterError(): Nothing = decodeError("unescaped control character")
  ...
  private[this] def tooLongInputError(): Nothing = decodeError("too long part of input exceeded 'maxBufSize'")

  private[this] def tooLongStringError(): Nothing = decodeError("too long string exceeded 'maxCharBufSize'")

  private[this] def endOfInputError(): Nothing = decodeError("unexpected end of input")
...
private class Key {
  private[this] var hash: Int       = 0
  private[this] var bs: Array[Byte] = null
