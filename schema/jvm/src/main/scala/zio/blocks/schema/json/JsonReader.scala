...
final class JsonReader private[json] (
  ...
  def readNullOrError[A](default: A, msg: String): A =
    if (default != null) {
      val pos = head
      if (pos + 3 < tail) {
        if (ByteArrayAccess.getInt(buf, pos) == 0x6c6c756e) {
          head = pos + 4
          default
  ...
  private[this] def parseBoolean(isToken: Boolean, pos: Int): Boolean =
    if (pos + 3 < tail) {
      ...
      if (bs == 0x65757274) {
        ...
        if (nextByte(pos + 4) != 'e') booleanError()
        false
      } else if (
        isToken && {
          val b1 = bs.toByte
  ...
  private[this] def readLong(isToken: Boolean): Long = {
    ...
    if (isToken && x == 0) ensureNotLeadingZero()
      ...
      x ^= s
      x -= s
      if ((s & x) == -9223372036854775808L) longOverflowError()
      if ((b | 0x20) == 'e' || b == '.') numberError()
    }
    x
  }
  ...
  private[this] def parseDuration(isRaw: Boolean): Duration = {
    ...
    while ({
      ...
      if (b == 'D' && state < 0) {
        if (x < -106751991167300L) durationError() // -106751991167300L == Long.MinValue / 86400
        seconds = (sx - (x ^ sx)) * 86400
        state = 0
      } else if (b == 'H' && state <= 0) {
        if (x < -2562047788015215L) durationError() // -2562047788015215L == Long.MinValue / 3600
        seconds = sumSeconds((sx - (x ^ sx)) * 3600, seconds)
        state = 1
      } else if (b == 'M' && state <= 1) {
        if (x < -153722867280912930L) durationError() // -153722867280912930L == Long.MinValue / 60
        seconds = sumSeconds((sx - (x ^ sx)) * 60, seconds)
        state = 2
      } else if (b == 'S' || b == '.') {
        if ((x | sx) == -9223372036854775808L) durationError()
        seconds = sumSeconds(sx - (x ^ sx), seconds)
        state = 3
        if (b == '.') {
          pos += 1
    ...
    if (b == 'Z') {
      ...
      if (
        pos + 7 < tail && {
          ...
          offsetTotal =
            (((bs & 0xf07000f01L) * 2561 & 0x3f00001f00L) * 1979120931962880L >>> 47).toInt // Based on the fast time string to seconds conversion: https://johnnylee-sde.github.io/Fast-time-string-to-seconds/
        ...
        offsetTotal =
          if (isRaw) parseOffsetTotal(pos)
          else parseOffsetTotalWithDoubleQuotes(pos)
  ...
  private[this] def parseLocalDate(isRaw: Boolean): LocalDate = {
    ...
    if (
      ...
      if (isRaw) day = parseDay(year, month, head)
      else day = parseDayWithByte(year, month, '"', head)
    }
  ...
  private[this] def parseLocalDateTime(isRaw: Boolean): LocalDateTime = {
    ...
    if (b == '.') {
      ...
      if (!isRaw && b != '"') nanoError(nanoDigitWeight, '"')
    } else if (!isRaw && b != '"') tokensError('.', '"')
    head = pos
  ...
  private[this] def parseMonthDay(pos: Int, isRaw: Boolean): MonthDay =
    if (isRaw) {
      ...
      if (pos + 7 < tail) {
        ...
        head = pos + 8
        // Based on the fast parsing of numbers by 8-byte words: https://github.com/wrandelshofer/FastDoubleParser/blob/0903817a765b25e654f02a5a9d4f1476c98a80c9/src/main/java/ch.randelshofer.fastdoubleparser/ch/randelshofer/fastdoubleparser/FastDoubleSimd.java#L114-L130
        if (((bs + 0x767c00767e0000L | bs) & 0xff8080ff8080ffffL) != 0) monthDayError(pos)
        if (month < 1 || month > 12) monthError()
        if (day == 0 || (day > 28 && day > maxDayForMonth(month))) dayError()
        MonthDay.of(month, day)
      } else parseMonthDay(loadMoreOrError(pos), isRaw)
