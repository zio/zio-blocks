 ...
 *
 * Note: Bidirectional conversion between nominal and structural requires both
 * directions to work. nominal->structural works, but structural->nominal
 * requires the structural value to be created with proper method backing (which
 * the Into macro handles internally on JVM).
 */
