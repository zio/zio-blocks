  ...
  def spec: Spec[TestEnvironment, Any] = suite("NeotypeSupportSpec")(
    test("derive schemas for cases classes with subtype and newtype fields") {
      ...
      assert(Planet.name.focus.typeId)(
        equalTo(
          TypeId.nominal("Name", Owner.parse("zio.blocks.schema.NeotypeSupportSpec"), Nil)
        )
      ) &&
      assert(Planet.mass.focus.typeId)(
        equalTo(
          TypeId.nominal("Kilogram", Owner.parse("zio.blocks.schema.NeotypeSupportSpec"), Nil)
        )
      ) &&
      assert(Planet.radius.focus.typeId)(
        equalTo(
          TypeId.nominal("Meter", Owner.parse("zio.blocks.schema.NeotypeSupportSpec"), Nil)
        )
      ) &&
