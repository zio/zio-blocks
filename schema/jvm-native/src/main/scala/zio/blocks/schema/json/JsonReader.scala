...
final class JsonReader private[json] (
  ...
  /**
   * Throws a [[JsonError]] indicating that a required field with the
  ...
  /**
   * Throws a [[JsonError]] indicating that a field with the given
   * name is duplicated.
   *
   * @param len
  ...
  /**
   * Throws a [[JsonError]] indicating that an unexpected field with
  ...
  /**
   * Throws a [[JsonError]] indicating that an illegal discriminator
   * field name was encountered.
   */
  def discriminatorError(): Nothing = decodeError("illegal discriminator")

  /**
   * Throws a [[JsonError]] indicating that an illegal value was
  ...
  /**
   * Throws a [[JsonError]] indicating that an illegal enum value
   ...
   * default value or throws a [[JsonError]] if the provided default
   * value is `null`.
   *
   * @param default
   ...
   * [[JsonError]] if the provided default value is `null`.
   *
   * @param default
  ...
  /**
   * Finishes reading the `null` JSON value and returns the provided default
   * value or throws [[JsonError]] with a message of expecting `null`
  ...
  /**
   * Throws a [[JsonError]] with the message `expected ','`.
   *
   * @throws JsonError
   *   always
   */
  ...
  /**
   * Throws a [[JsonError]] with the message `expected ']' or ','`.
   *
   * @throws JsonError
   *   always
   */
  ...
  /**
   * Throws a [[JsonError]] with the given message.
   *
   * @param msg
   *   the exception message
   * @throws JsonError
  ...
  /**
   * Throws a [[JsonError]] wrapping the given error and adding a
   * span.
   *
   * @param span
  ...
  def decodeError(span: DynamicOptic.Node, error: Throwable): Nothing = error match {
    case e: JsonError =>
      throw e.copy(path = new DynamicOptic(Vector(span) ++ e.path.nodes))
    case _ => throw JsonError(error.getMessage, new DynamicOptic(Vector(span)))
  }

  /**
   * Throws a [[JsonError]] wrapping the given error and adding two
   * spans.
   *
   * @param span1
  ...
  def decodeError(span1: DynamicOptic.Node, span2: DynamicOptic.Node, error: Throwable): Nothing = error match {
    case e: JsonError =>
      throw e.copy(path = new DynamicOptic(Vector(span1, span2) ++ e.path.nodes))
    case _ => throw JsonError(error.getMessage, new DynamicOptic(Vector(span1, span2)))
  }
  ...
  def decodeError(spans: List[DynamicOptic.Node], error: Throwable): Nothing = error match {
    case e: JsonError =>
      throw e.copy(path = new DynamicOptic(spans.toVector ++ e.path.nodes))
    case _ => throw JsonError(error.getMessage, new DynamicOptic(spans.toVector))
  }
  ...
  private[this] def decodeError(from: Int): Nothing =
    throw JsonError(new String(charBuf, 0, from))
  ...
  private[this] def parseMonthDay(pos: Int): MonthDay =
    if (pos + 7 < tail) {
      ...
      val day      = monthDay >> 24
      head = pos + 8
      if (((bs + 0x767c00767e0000L | bs) & 0xff8080ff8080ffffL) != 0)
        monthDayError(
          pos
        ) // Based on the fast parsing of numbers by 8-byte words: https://github.com/wrandelshofer/FastDoubleParser/blob/0903817a765b25e654f02a5a9d4f1476c98a80c9/src/main/java/ch.randelshofer.fastdoubleparser/ch/randelshofer/fastdoubleparser/FastDoubleSimd.java#L114-L130
      if (month < 1 || month > 12) monthError()
