...
object DynamicMigrationSpec extends ZIOSpecDefault {
  ...
  def spec = suite("DynamicMigrationSpec")(
    ...
    test("Optionalize wraps in Some") {
       ...
       val result = migration(start)
       assert(result)(isRight(equalTo(
         DynamicValue.Record(Vector("name" -> DynamicValue.Variant("Some", DynamicValue.Primitive(PrimitiveValue.String("Alice")))))
       )))
    },
    test("Mandate unwraps Some and fails None") {
       val startSome = DynamicValue.Record(Vector("name" -> DynamicValue.Variant("Some", DynamicValue.Primitive(PrimitiveValue.String("Alice")))))
       val path = DynamicOptic(Vector(DynamicOptic.Node.Field("name")))
       val action = MigrationAction.Mandate(path, SchemaExpr.Literal((), Schema.unit))
       val migration = DynamicMigration(Vector(action))
       
       val res1 = migration(startSome)
       assert(res1)(isRight(equalTo(
         DynamicValue.Record(Vector("name" -> DynamicValue.Primitive(PrimitiveValue.String("Alice"))))
       )))
       
       val startNone = DynamicValue.Record(Vector("name" -> DynamicValue.Variant("None", DynamicValue.Primitive(PrimitiveValue.Unit))))
       val res2 = migration(startNone)
       assert(res2)(isLeft)
    },
    test("TransformElements applies migration to elements") {
       val start = DynamicValue.Sequence(Vector(
    ...
    test("Split distributes fields (using Literal)") {
       ...
       val splitResult = DynamicValue.Record(Vector(
    ...
    test("Advanced selectors in Builder") {
       ...
       val m = Migration.newBuilder[Box, Box]
       ...
       assert(resIdx)(isRight(equalTo(
          DynamicValue.Record(Vector("items" -> 
             DynamicValue.Sequence(Vector(DynamicValue.Variant("Some", DynamicValue.Primitive(PrimitiveValue.String("a")))))
          ))
       )))
    }
  )
