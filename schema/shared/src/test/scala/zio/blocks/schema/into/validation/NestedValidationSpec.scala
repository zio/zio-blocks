...
object NestedValidationSpec extends ZIOSpecDefault {
  ...
  implicit val innerInto: Into[Inner, InnerInt] = Into.derived
  ...
  def spec: Spec[TestEnvironment, Any] = suite("NestedValidationSpec")(
    ...
    test("fails when list element overflows") {
      case class Source(items: List[Long])
      case class Target(items: List[Int])

      val result = Into.derived[Source, Target].into(Source(List(1L, Long.MaxValue)))
      assert(result)(isLeft)
    },
    test("fails when Option value overflows") {
      case class Source(value: Option[Long])
      case class Target(value: Option[Int])

      val result = Into.derived[Source, Target].into(Source(Some(Long.MaxValue)))
      assert(result)(isLeft)
    }
  )
}
