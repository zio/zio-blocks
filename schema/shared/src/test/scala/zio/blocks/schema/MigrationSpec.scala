...
object MigrationSpec extends ZIOSpecDefault {
  def spec = suite("MigrationSpec")(
    ...
    test("MigrationAction.AddField should add a field with a literal default") {
      ...
      val default = SchemaExpr.Literal[DynamicValue, DynamicValue](DynamicValue.Primitive(PrimitiveValue.Int(0)), Schema.dynamic)
      ...
      assert(result.fields.find(_._1 == "newField").map(_._2))(isSome(equalTo(DynamicValue.Primitive(PrimitiveValue.Int(0)))))
    },
    test("MigrationAction.DropField should remove a field") {
      val optic = DynamicOptic.root.field("toDrop")
      val action = MigrationAction.DropField(optic, SchemaExpr.Literal[DynamicValue, DynamicValue](DynamicValue.Null, Schema.dynamic))
      val input = DynamicValue.Record(Chunk("toDrop" -> DynamicValue.Primitive(PrimitiveValue.Int(1)), "keep" -> DynamicValue.Primitive(PrimitiveValue.Int(2))))
