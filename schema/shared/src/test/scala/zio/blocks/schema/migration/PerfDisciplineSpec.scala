...
/**
 * Zero-allocation discipline for the migration success path — JS-safe runtime
 * portion only.
 *
 * The shared runtime no-op tests live here. The source-grep against
 * `Interpreter.scala` was moved to
 * `schema/jvm/src/test/scala/.../PerfDisciplineJvmSpec.scala` because it relies
 * on `scala.io.Source.fromFile`, which cannot link on Scala.js.
 *
...
object PerfDisciplineSpec extends SchemaBaseSpec {
  ...
  def spec: Spec[Any, Any] = suite("PerfDisciplineSpec")(
    test(
      "runtime no-op: DynamicMigration.empty.apply(dv) returns Right(dv) — success path does not touch MigrationError"
    ) {
    ...
    test("runtime no-op across 5 identity-composed empty migrations — still Right, still no error construction") {
      ...
      val dv   = intVal(42)
      val five =
        DynamicMigration.empty ++ DynamicMigration.empty ++ DynamicMigration.empty ++ DynamicMigration.empty ++ DynamicMigration.empty
