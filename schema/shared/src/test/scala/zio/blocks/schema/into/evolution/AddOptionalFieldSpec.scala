...
object AddOptionalFieldSpec extends ZIOSpecDefault {
  ...
  def spec: Spec[TestEnvironment, Any] = suite("AddOptionalFieldSpec")(
    ...
    test("existing source value used when Option field has matching source") {
      case class SourceWithEmail(name: String, age: Int, email: String)
      val result = Into.derived[SourceWithEmail, PersonV2].into(SourceWithEmail("Bob", 25, "bob@test.com"))
