  ...
  def spec: Spec[TestEnvironment, Any] = suite("JsonComprehensiveSpec")(
    ...
    suite("ADT Construction")(
      test("constructs all JSON types") {
        ...
        assertTrue(obj.isObject && arr.isArray && str.isString && 
    ...
    suite("Navigation")(
      ...
      test("chained navigation") {
        val json = Json.Object("users" -> Json.Array(
    ...
    suite("Modification")(
      test("modify values") {
        ...
        val modified = json.modify(p"count", {
    ...
    suite("Transformation")(
      test("transformUp - bottom-up") {
        ...
        val transformed = json.transformUp { (path, value) =>
          if (path.nodes.lastOption.exists {
      ...
      test("transformKeys") {
        ...
        val transformed = json.transformKeys((_, key) => if (key.startsWith("old_")) key.replace("old_", "new_") else key)
    ...
    suite("DynamicValue Interop")(
      ...
      test("convert from DynamicValue") {
        val dynamic = DynamicValue.Record(Vector(
    ...
    suite("Edge Cases")(
      test("deeply nested structures") {
        ...
        val path = (1 to 10).foldLeft(DynamicOptic.root) { (acc, _) =>
          acc.field("nested")
        }.field("value")
