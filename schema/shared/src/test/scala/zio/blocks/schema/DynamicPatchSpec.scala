  ...
  def spec: Spec[TestEnvironment, Any] = suite("DynamicPatchSpec")(
    ...
    suite("Roundtrip Law")(
      ...
      test("diff(a, b).apply(a) == b for sequences") {
        val oldSeq = DynamicValue.Sequence(Vector(
          DynamicValue.Primitive(PrimitiveValue.Int(1)),
          DynamicValue.Primitive(PrimitiveValue.Int(2)),
          DynamicValue.Primitive(PrimitiveValue.Int(3))
        ))
        val newSeq = DynamicValue.Sequence(Vector(
          DynamicValue.Primitive(PrimitiveValue.Int(1)),
      ...
      test("diff(a, b).apply(a) == b for records with same fields") {
        val oldRec = DynamicValue.Record(Vector(
          "name" -> DynamicValue.Primitive(PrimitiveValue.String("Alice")),
          "age" -> DynamicValue.Primitive(PrimitiveValue.Int(30))
        ))
        val newRec = DynamicValue.Record(Vector(
    ...
    suite("Operation Application")(
      ...
      test("BigDecimalDelta adds to BigDecimal primitive") {
        check(Gen.bigDecimal(BigDecimal(0), BigDecimal(1000000)), Gen.bigDecimal(BigDecimal(-1000), BigDecimal(1000))) { (base, delta) =>
    ...
    suite("PatchMode Behavior")(
      test("Strict mode fails on missing field") {
        ...
        val patch = DynamicPatch(Vector(DynamicPatchOp(optic, Operation.Set(DynamicValue.Primitive(PrimitiveValue.String("new"))))))
    ...
    suite("JSON Serialization")(
      test("DynamicPatch roundtrip") {
        ...
        implicit val codec: zio.blocks.schema.json.JsonBinaryCodec[DynamicPatch] = DynamicPatch.schema.derive(JsonBinaryCodecDeriver)
