  ...
  def spec: Spec[TestEnvironment, Any] = suite("DynamicPatchSpec")(
      ...
      test("patches compose via ++") {
        ...
        val patch2 = DynamicPatch.single(
          DynamicPatch.Op(
            ...
            DynamicPatch.Operation.Set(DynamicValue.Primitive(PrimitiveValue.Int(2)))
          )
        )
    ...
    suite("Primitive operations")(
      ...
      test("DoubleDelta adds to double") {
        ...
        val patch = DynamicPatch.single(
          DynamicPatch.Op(
            DynamicOptic.root,
            DynamicPatch.Operation.PrimitiveDelta(DynamicPatch.PrimitiveOp.DoubleDelta(0.86))
          )
        )
      ...
      test("StringEdit applies insert/delete operations") {
        ...
        val patch = DynamicPatch.single(
          DynamicPatch.Op(
            DynamicOptic.root,
            DynamicPatch.Operation.PrimitiveDelta(DynamicPatch.PrimitiveOp.StringEdit(ops))
          )
        )
        assert(patch.apply(value, DynamicPatch.PatchMode.Strict))(
        ...
        val patch = DynamicPatch.single(
          DynamicPatch.Op(
            ...
            DynamicPatch.Operation.SequenceEdit(
              Vector(DynamicPatch.SeqOp.Insert(1, Vector(DynamicValue.Primitive(PrimitiveValue.Int(2)))))
            )
          )
        )
        assert(patch.apply(value, DynamicPatch.PatchMode.Strict))(
    ...
    suite("Monoid laws")(
      ...
      test("(p1 ++ p2) ++ p3 == p1 ++ (p2 ++ p3)") {
        val p1 = DynamicPatch.single(
          DynamicPatch.Op(
            DynamicOptic.root.field("a"),
            DynamicPatch.Operation.Set(DynamicValue.Primitive(PrimitiveValue.Int(1)))
          )
        )
        val p2 = DynamicPatch.single(
          DynamicPatch.Op(
            DynamicOptic.root.field("b"),
            DynamicPatch.Operation.Set(DynamicValue.Primitive(PrimitiveValue.Int(2)))
          )
        )
        val p3 = DynamicPatch.single(
          DynamicPatch.Op(
            DynamicOptic.root.field("c"),
            DynamicPatch.Operation.Set(DynamicValue.Primitive(PrimitiveValue.Int(3)))
          )
        )
