 ...
 * - DynamicOptic.scala $anon (28.11%) - Constructor/Deconstructor/Matcher/Discriminator
...
object ExtendedCoverageSpec extends SchemaBaseSpec {
  ...
  val dynamicOpticRenderPrimitivesTests = suite("DynamicOptic render other primitives")(
    ...
    test("toString with Map DynamicValue") {
      val map = DynamicValue.Map(Chunk((DynamicValue.Primitive(PrimitiveValue.String("k")), DynamicValue.Primitive(PrimitiveValue.Int(1)))))
