...
object TransformKeysSpec extends SchemaBaseSpec {
  ...
  def spec: Spec[Any, Any] = suite("TransformKeysSpec")(
    test(
      "TransformKeys applies transform to each key, preserving values (happy path — single-entry map, no collision possible)"
    ) {
    ...
    test(
      "collisionAtIndex0: TransformKeys returns KeyCollision(path, srcKey) on immediate collision (entry 0 vs entry 1)/ "
    ) {
      val k0                         = stringVal("a")
      val k1                         = stringVal("b")
      ...
      assertTrue(result.swap.toOption.exists {
        case kc: MigrationError.KeyCollision =>
          kc.key == k1 // first colliding source key (entry 1 collides with entry 0)
        case _ => false
      })
    },
    /**
     * Covers collision detection framed from the opposite end of the map: a
     * 2-entry input where the LAST entry's transformed key collides with the
     * first entry's transformed key already in the accumulator. The offending
     * source key reported in `KeyCollision` is the LAST key.
     *
     ...
     */
    test(
      "collisionAtLastIndex: 2-entry map — last entry collides with index 0 — fail-fast reports last key as offending"
    ) {
      val k0                         = stringVal("a")
      ...
      assertTrue(result.swap.toOption.exists {
        case kc: MigrationError.KeyCollision =>
          kc.key == k1 // LAST source key (offending; collides with k0's transformed image already in accumulator)
        case _ => false
      })
    },
    ...
    test("TransformKeys on a non-Map yields SchemaMismatch") {
      val action =
        MigrationAction.TransformKeys(DynamicOptic.root, SchemaExpr.Literal[DynamicValue, String]("X", Schema[String]))
      val m      = new DynamicMigration(Chunk.single(action))
