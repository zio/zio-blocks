 ...
 * - Operation (Set, PrimitiveDelta, SequenceEdit, MapEdit, Patch) - 5 variants
 */
object PatchCoverageSpec extends SchemaBaseSpec {
    ...
    suite("PrimitiveOp.BigDecimalDelta")(
      (1 to 10).map { i =>
        test(s"BigDecimalDelta roundtrip $i") {
          val op: PrimitiveOp = PrimitiveOp.BigDecimalDelta(BigDecimal(i) * BigDecimal("123.456789012345678901234567890"))
          val dv = Schema[PrimitiveOp].toDynamicValue(op)
          val restored = Schema[PrimitiveOp].fromDynamicValue(dv)
          assertTrue(restored == Right(op))
        }
  ...
  val seqOpSchemaTests = suite("SeqOp schema coverage")(
    suite("SeqOp.Insert")(
      (1 to 10).map { i =>
        test(s"Insert roundtrip $i") {
          val values = Chunk.fromArray((1 to i).map(j => DynamicValue.Primitive(PrimitiveValue.Int(j)): DynamicValue).toArray)
    ...
    suite("SeqOp.Append")(
      (1 to 10).map { i =>
        test(s"Append roundtrip $i") {
          val values = Chunk.fromArray((1 to i).map(j => DynamicValue.Primitive(PrimitiveValue.String(s"item$j")): DynamicValue).toArray)
  ...
  val mapOpSchemaTests = suite("MapOp schema coverage")(
    ...
    suite("MapOp.Modify")(
      (1 to 10).map { i =>
        test(s"Modify roundtrip $i") {
          val key = DynamicValue.Primitive(PrimitiveValue.String(s"key$i"))
          val innerPatch = DynamicPatch(Vector(
    ...
    suite("Operation.MapEdit")(
      (1 to 10).map { i =>
        test(s"MapEdit roundtrip $i") {
          val mapOps = Vector(
            MapOp.Add(DynamicValue.Primitive(PrimitiveValue.String(s"key$i")), DynamicValue.Primitive(PrimitiveValue.Int(i))),
    ...
    suite("Operation.Patch")(
      (1 to 10).map { i =>
        test(s"Patch roundtrip $i") {
          val innerPatch = DynamicPatch(Vector(
            DynamicPatch.DynamicPatchOp(DynamicOptic.root.field("field"), Operation.Set(DynamicValue.Primitive(PrimitiveValue.Int(i))))
          ))
  ...
  val renderOpTests = suite("Render operation strings")(
    test("Render Set operation") {
      val patch = DynamicPatch(Vector(
        DynamicPatch.DynamicPatchOp(DynamicOptic.root.field("x"), Operation.Set(DynamicValue.Primitive(PrimitiveValue.Int(42))))
      ))
    ...
    test("Render BigDecimalDelta") {
      val patch = DynamicPatch(Vector(
        DynamicPatch.DynamicPatchOp(DynamicOptic.root, Operation.PrimitiveDelta(PrimitiveOp.BigDecimalDelta(BigDecimal("123.45"))))
      ))
      assertTrue(patch.toString.contains("123.45"))
    },
    test("Render InstantDelta") {
      val patch = DynamicPatch(Vector(
        DynamicPatch.DynamicPatchOp(DynamicOptic.root, Operation.PrimitiveDelta(PrimitiveOp.InstantDelta(Duration.ofSeconds(3600))))
      ))
      assertTrue(patch.toString.contains("PT"))
    },
    test("Render DurationDelta") {
      val patch = DynamicPatch(Vector(
        DynamicPatch.DynamicPatchOp(DynamicOptic.root, Operation.PrimitiveDelta(PrimitiveOp.DurationDelta(Duration.ofHours(2))))
      ))
      assertTrue(patch.toString.contains("PT2H"))
    },
    test("Render LocalDateDelta") {
      val patch = DynamicPatch(Vector(
        DynamicPatch.DynamicPatchOp(DynamicOptic.root, Operation.PrimitiveDelta(PrimitiveOp.LocalDateDelta(Period.ofDays(10))))
      ))
      assertTrue(patch.toString.contains("P10D"))
    },
    test("Render LocalDateTimeDelta") {
      val patch = DynamicPatch(Vector(
        DynamicPatch.DynamicPatchOp(DynamicOptic.root, Operation.PrimitiveDelta(PrimitiveOp.LocalDateTimeDelta(Period.ofMonths(1), Duration.ofHours(2))))
      ))
      assertTrue(patch.toString.contains("P1M"))
    },
    test("Render PeriodDelta") {
      val patch = DynamicPatch(Vector(
        DynamicPatch.DynamicPatchOp(DynamicOptic.root, Operation.PrimitiveDelta(PrimitiveOp.PeriodDelta(Period.of(1, 2, 3))))
      ))
      assertTrue(patch.toString.contains("P1Y2M3D"))
    },
    test("Render StringEdit") {
      val patch = DynamicPatch(Vector(
        DynamicPatch.DynamicPatchOp(DynamicOptic.root, Operation.PrimitiveDelta(PrimitiveOp.StringEdit(Vector(
    ...
    test("Render SequenceEdit insert") {
      val patch = DynamicPatch(Vector(
        DynamicPatch.DynamicPatchOp(DynamicOptic.root, Operation.SequenceEdit(Vector(
    ...
    test("Render MapEdit add") {
      val patch = DynamicPatch(Vector(
        DynamicPatch.DynamicPatchOp(DynamicOptic.root, Operation.MapEdit(Vector(
          MapOp.Add(DynamicValue.Primitive(PrimitiveValue.String("key")), DynamicValue.Primitive(PrimitiveValue.Int(1)))
        )))
      ))
      assertTrue(patch.toString.contains("+"))
    },
    ...
    test("Render nested Patch operation") {
      val innerPatch = DynamicPatch(Vector(
        DynamicPatch.DynamicPatchOp(DynamicOptic.root.field("inner"), Operation.Set(DynamicValue.Primitive(PrimitiveValue.Int(99))))
      ))
      val patch = DynamicPatch(Vector(
