 ...
 * 
 * These tests verify that the macro can handle products with 20+ fields
 * and that conversions work correctly with large data structures.
 */
object LargeProductStressSpec extends ZIOSpecDefault {

  // 10-field product
  case class Product10A(
    field1: Int, field2: String, field3: Long, field4: Boolean, field5: Double,
    field6: Int, field7: String, field8: Long, field9: Boolean, field10: Double
  )
  ...
  case class Product20A(
    f1: Int, f2: String, f3: Long, f4: Boolean, f5: Double,
    f6: Int, f7: String, f8: Long, f9: Boolean, f10: Double,
    f11: Int, f12: String, f13: Long, f14: Boolean, f15: Double,
    f16: Int, f17: String, f18: Long, f19: Boolean, f20: Double
  )
  ...
  case class AllPrimitivesA(
    byteVal: Byte, shortVal: Short, intVal: Int, longVal: Long,
    floatVal: Float, doubleVal: Double, boolVal: Boolean, charVal: Char,
    stringVal: String
  )
  ...
  def spec = suite("LargeProductStressSpec")(
    suite("Into - Large Products")(
      ...
      test("converts 20-field product correctly") {
        ...
        val result = into.into(source)
        assertTrue(result.isRight, result.map(_.f1) == Right(1), result.map(_.f10) == Right(6.0), result.map(_.f20) == Right(12.0))
      },
      test("converts product with all primitive types") {
        val source = AllPrimitivesA(
          1.toByte, 2.toShort, 3, 4L, 5.0f, 6.0, true, 'x', "test"
        )
        val into = Into.derived[AllPrimitivesA, AllPrimitivesB]
        
        val result = into.into(source)
        assertTrue(
          result == Right(AllPrimitivesB(
            1.toByte, 2.toShort, 3, 4L, 5.0f, 6.0, true, 'x', "test"
          ))
        )
      }
    ...
    suite("Into - Collection Types")(
      test("converts product with various collections") {
        ...
        val result = into.into(source)
        assertTrue(result.isRight, result.map(_.listVal) == Right(List(1, 2, 3)), result.map(_.optionVal) == Right(Some(3.14)))
      },
      ...
      test("converts product with large collection (1000 elements)") {
        ...
        val result = into.into(source)
        assertTrue(result.isRight, result.map(_.items.size) == Right(1000), result.map(_.items.head) == Right(1), result.map(_.items.last) == Right(1000))
      },
