...
import zio.blocks.schema.json.JsonInterpolator._
      ...
      test("supports interpolated Float keys and values") {
        check(Gen.float)(x =>
          assertTrue(
            ...
            json"""{"$x": "v"}""".get(JsonBinaryCodec.floatCodec.encodeToString(x)).string == Right("v")
          )
        )
      },
      test("supports interpolated Double keys and values") {
        check(Gen.double)(x =>
          assertTrue(
            ...
            json"""{"$x": "v"}""".get(JsonBinaryCodec.doubleCodec.encodeToString(x)).string == Right("v")
          )
        )
      },
