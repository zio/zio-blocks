          ...
          equalTo(
            new Schema[Record1](
              reflect = Reflect.Record[Binding, Record1](
                ...
                typeId = TypeId.nominal(
                  "Record-1",
                  Owner.parse("zio.blocks.schema.SchemaSpec.spec"),
                  Nil
                ),
                recordBinding = null,
                modifiers = Seq(
        ...
        implicit def chunkSchema[V](implicit ev: Schema[V]): Schema[Chunk[V]] =
          new Schema(
            new Reflect.Wrapper[Binding, Chunk[V], List[V]](
              ...
              TypeId.nominal("Chunk", Owner.parse("zio"), Nil),
              None,
    ...
    suite("Reflect.Variant")(
      ...
      test("derives schema for a variant with cases on different levels using a macro call") {
        ...
        assert(variant1.map(_.typeId))(
          isSome(equalTo(TypeId.nominal("A", Owner.parse("zio.blocks.schema.SchemaSpec"), Nil)))
        ) &&
        ...
        assert(variant2.map(_.typeId))(
          isSome(
            equalTo(
              TypeId.nominal(
                "MultiLevel",
                Owner.parse("zio.blocks.schema.SchemaSpec.Level1"),
                Nil
              )
            )
          )
