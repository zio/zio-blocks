...
object MasterAlgebraSpec extends ZIOSpecDefault {
  ...
  lazy val m1: Migration[V1, V2] = MigrationBuilder.make[V1, V2]
    .addField((v: V2) => v.age, 0)
    .build

  lazy val m2: Migration[V2, V3] = MigrationBuilder.make[V2, V3]
  ...
  def spec = suite("Pillar 1: Master Algebra & Laws Verification")(
    ...
    test("Example: PersonSource to PersonTarget (Add Field Check)") {
      val builder = MigrationBuilder.make[PersonSource, PersonTarget]
    ...
    test("Error Handling: Errors must capture path information") {
      ...
      val errorPath = DynamicOptic(Vector(DynamicOptic.Node.Field("missing_address"), DynamicOptic.Node.Field("street")))
