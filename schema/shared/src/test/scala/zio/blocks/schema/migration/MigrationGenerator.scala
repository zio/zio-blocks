...
object MigrationGenerator {
  ...
  val anyPrimitive: Gen[Any, DynamicValue] = Gen.oneOf(
    Gen.int(0, 20).flatMap(n => Gen.stringN(n)(Gen.alphaNumericChar)).map(s => DynamicValue.Primitive(PrimitiveValue.String(s))),
  ...
  val anyRecord: Gen[Any, DynamicValue] = 
    Gen.int(1, 10).flatMap(n => Gen.vectorOfN(n)(genName.zip(anyPrimitive)))
  ...
  val anyDynamicMigration: Gen[Any, DynamicMigration] = 
    Gen.int(1, 5).flatMap(n => Gen.vectorOfN(n)(anyAction))
