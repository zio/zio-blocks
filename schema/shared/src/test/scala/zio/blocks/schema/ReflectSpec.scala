...
object ReflectSpec extends ZIOSpecDefault {
  def spec: Spec[TestEnvironment, Any] = suite("ReflectSpec")(
    ...
    suite("Reflect.Primitive")(
      test("has consistent equals and hashCode") {
        val long1 = Primitive[Binding, Long](
          ...
          typeId = TypeId.derive[Long]
        )
        val long2 = long1.copy(primitiveType = PrimitiveType.Long(Validation.Numeric.Positive))
        val long3 = long1.copy(typeId = TypeId.nominal("Long1", Owner.parse("zio.blocks.schema"), Nil))
      ...
      test("gets and updates primitive type name") {
        val int1 = Reflect.int[Binding]
        assert(int1.typeId)(equalTo(TypeId.derive[Int])) &&
        assert(
          int1
            .typeId(TypeId.nominal("IntWrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil))
            .typeId
        )(equalTo(TypeId.nominal("IntWrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil)))
      },
    ...
    suite("Reflect.Record")(
      test("has consistent equals and hashCode") {
        ...
        val record2 = record1.copy(typeId = TypeId.nominal("Tuple4", Owner.parse("zio.blocks.schema"), Nil))
      ...
      test("gets and updates record type name") {
        assert(tuple4Reflect.typeId)(
          equalTo(
            TypeId.derive[(Byte, Short, Int, Long)]
          )
        ) &&
        assert(
          tuple4Reflect
            .typeId(TypeId.nominal("Tuple4Wrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil))
            .typeId
        )(
          equalTo(
            TypeId.nominal("Tuple4Wrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil)
          )
        )
      },
    ...
    suite("Reflect.Variant")(
      ...
      test("gets and updates variant type name") {
        assert(eitherReflect.typeId)(
          equalTo(
            TypeId.derive[Either[Int, Long]]
          )
        ) &&
        assert(
          eitherReflect
            .typeId(TypeId.nominal("EitherWrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil))
            .typeId
        )(
          equalTo(
            TypeId.nominal("EitherWrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil)
          )
        )
      },
    ...
    suite("Reflect.Sequence")(
      test("has consistent equals and hashCode") {
        val sequence1 = Reflect.Sequence[Binding, Double, List](
          ...
          typeId = TypeId.derive[List[Double]],
          seqBinding = null // should be ignored in equals and hashCode
        )
        val sequence2 = sequence1.copy(element =
          Primitive(PrimitiveType.Double(Validation.None), TypeId.derive[Double], Binding.Primitive.double, Doc("text"))
        )
        val sequence3 = sequence1.copy(typeId = TypeId.nominal("List2", Owner.parse("scala"), Nil))
      ...
      test("gets and updates sequence type name") {
        ...
        assert(sequence1.typeId)(
          equalTo(TypeId.derive[Vector[Int]])
        ) &&
        assert(
          sequence1
            .typeId(TypeId.nominal("VectorWrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil))
            .typeId
        )(
          equalTo(
            TypeId.nominal("VectorWrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil)
          )
        )
      },
    ...
    suite("Reflect.Map")(
      test("has consistent equals and hashCode") {
        val map1 = Reflect.Map[Binding, Short, Float, Map](
          key = Reflect.short,
          value = Reflect.float,
          typeId = TypeId.derive[Map[Short, Float]],
        ...
        val map3 = map1.copy(value =
          Primitive(PrimitiveType.Float(Validation.None), TypeId.derive[Float], Binding.Primitive.float, Doc("text"))
        )
        val map4 = map1.copy(typeId = TypeId.nominal("Map2", Owner.parse("scala"), Nil))
      ...
      test("gets and updates map type name") {
        val map1 = Reflect.map(Reflect.int[Binding], Reflect.long[Binding])
        assert(map1.typeId)(
          equalTo(TypeId.derive[Map[Int, Long]])
        ) &&
        assert(
          map1
            .typeId(TypeId.nominal("MapWrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil))
            .typeId
        )(
          equalTo(
            TypeId.nominal("MapWrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil)
          )
        )
      },
      ...
      test("gets and updates map documentation") {
        val map1 = Reflect.Map[Binding, Int, Long, Map](
          key = Reflect.int,
          value = Reflect.long,
          typeId = TypeId.derive[Map[Int, Long]],
          mapBinding = null, // should be ignored in equals and hashCode
    ...
    suite("Reflect.Dynamic")(
      ...
      test("gets and updates dynamic type name") {
        val dynamic1 = Reflect.dynamic[Binding]
        assert(dynamic1.typeId)(equalTo(TypeId.derive[DynamicValue])) &&
        assert(
          dynamic1
            .typeId(TypeId.nominal("DynamicWrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil))
            .typeId
        )(
          equalTo(
            TypeId.nominal("DynamicWrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil)
          )
        )
      },
    ...
    suite("Reflect.Wrapper")(
      test("has consistent equals and hashCode") {
        ...
        val wrapper2 = wrapper1.copy(typeId = TypeId.nominal("Tuple4", Owner.parse("zio.blocks.schema"), Nil))
      ...
      test("gets and updates wrapper type name") {
        assert(wrapperReflect.typeId)(
          equalTo(TypeId.derive[Wrapper])
        ) &&
        assert(
          wrapperReflect
            .typeId(TypeId.nominal("Wrapper2", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil))
            .typeId
        )(equalTo(TypeId.nominal("Wrapper2", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil)))
      },
    ...
    suite("Reflect.Deferred")(
      test("has consistent equals and hashCode") {
        ...
        val deferred4 =
          Primitive(PrimitiveType.Int(Validation.Numeric.Positive), TypeId.derive[Int], Binding.Primitive.int)
      ...
      test("gets and updates deferred type name") {
        val deferred1 = Reflect.Deferred[Binding, Year](() => Reflect.year)
        assert(deferred1.typeId)(equalTo(TypeId.derive[Year])) &&
        assert(
          deferred1
            .typeId(TypeId.nominal("YearWrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil))
            .typeId
        )(equalTo(TypeId.nominal("YearWrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil)))
      },
      ...
      test("avoids stack overflow for circulary dependent structures") {
        ...
        assert(deferred1.typeId)(equalTo(TypeId.derive[Any]))
      }
    )
  )
  ...
  val wrapperReflect: Reflect.Wrapper[Binding, Wrapper, Long] = new Reflect.Wrapper(
    ...
    typeId = TypeId.nominal("Wrapper", Owner.parse("zio.blocks.schema.ReflectSpec"), Nil),
