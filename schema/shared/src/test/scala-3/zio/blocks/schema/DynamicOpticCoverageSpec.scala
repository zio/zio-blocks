...
/**
 * Coverage tests for DynamicOptic.Node prism matchers.
 * Each Node type has a downcastOrNull matcher that needs to be exercised
...
object DynamicOpticCoverageSpec extends SchemaBaseSpec {
  ...
  val atMapKeysNode: DynamicOptic.Node = DynamicOptic.Node.AtMapKeys(Seq(
  ...
  val allNodes: Vector[DynamicOptic.Node] = Vector(
    fieldNode, caseNode, atIndexNode, atMapKeyNode, atIndicesNode,
    atMapKeysNode, elementsNode, mapKeysNode, mapValuesNode, wrappedNode
  )
  ...
  val selectionOperationTests = suite("DynamicValueSelection coverage")(
    ...
    test("sortFields works") {
      val dv = DynamicValue.Record("b" -> DynamicValue.Primitive(PrimitiveValue.Int(1)), "a" -> DynamicValue.Primitive(PrimitiveValue.Int(2)))
      val selection = DynamicValueSelection.succeed(dv)
