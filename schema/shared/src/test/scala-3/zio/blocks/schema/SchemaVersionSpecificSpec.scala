...
object SchemaVersionSpecificSpec extends SchemaBaseSpec {
  def spec: Spec[TestEnvironment, Any] = suite("SchemaVersionSpecificSpec")(
    ...
    suite("deriveFormat with Format")(
      test("compiles when explicit type annotation is used with schema.deriveFormat(Format)") {
        typeCheck {
          """
          ...
          val jsonCodec: JsonBinaryCodec[Person] = schema.deriveFormat(JsonFormat)
          """
