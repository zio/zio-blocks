  ...
  override def spec: Spec[TestEnvironment, Any] = suite("Error Message Quality")(
    ...
    suite("Field path in nested errors")(
      test("Nested product error includes field path") {
        ...
        assertTrue(
          result.isLeft,
          errorMsg.exists(msg => 
            msg.contains("address") || msg.contains("number") || msg.contains("field")
          )
        )
      }
    ),
    ...
    suite("Collection element errors")(
      test("Invalid element in collection provides context") {
        ...
        assertTrue(
          result.isLeft,
          result.left.map(_.getMessage).exists(msg => 
            msg.contains("element") || msg.contains("index") || msg.contains("collection")
          )
        )
      }
    ),
    ...
    suite("Coproduct errors")(
      test("Unmapped case provides clear error") {
        ...
        assertTrue(
          result.isLeft,
          errorMsg.exists(msg => 
            msg.contains("B") || msg.contains("case") || msg.contains("unmapped")
          )
        )
      }
    ),
