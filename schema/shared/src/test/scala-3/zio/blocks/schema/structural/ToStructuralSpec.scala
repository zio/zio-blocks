...
object ToStructuralSpec extends ZIOSpecDefault {
  ...
  def spec = suite("ToStructuralSpec")(
    ...
    suite("Collections")(
      ...
      test("List[CaseClass]") {
        ...
        val first = s.addresses.head.asInstanceOf[StructuralRecord]
        assertTrue(first.selectDynamic("street") == "A")
      },
      test("Map[String, Int]") {
      ...
      test("Either[String, Int] Left") {
        ...
        val s      = ts.toStructural(WithEither(Left("error")))
        val either = s.value.asInstanceOf[StructuralRecord]
        assertTrue(
      ...
      test("Either[String, Int] Right") {
        val ts     = ToStructural.derived[WithEither]
        val s      = ts.toStructural(WithEither(Right(42)))
        val either = s.value.asInstanceOf[StructuralRecord]
        assertTrue(
    ...
    suite("Complex Combinations")(
      ...
      test("Map with case class values") {
        ...
        val home = map("home")
        assertTrue(home.selectDynamic("street") == "A")
      },
      test("Either with case class") {
        case class WithEitherAddress(result: Either[String, Address])
        val ts = ToStructural.derived[WithEitherAddress]

        val s1          = ts.toStructural(WithEitherAddress(Right(Address("Main", "NYC", 10001))))
        val rightRecord = s1.result.asInstanceOf[StructuralRecord]
        assertTrue(rightRecord.selectDynamic("Tag") == "Right")
        val addr = rightRecord.selectDynamic("value").asInstanceOf[StructuralRecord]
        assertTrue(addr.selectDynamic("city") == "NYC")

        val s2         = ts.toStructural(WithEitherAddress(Left("not found")))
        val leftRecord = s2.result.asInstanceOf[StructuralRecord]
        assertTrue(
          leftRecord.selectDynamic("Tag") == "Left",
          leftRecord.selectDynamic("value") == "not found"
        )
      },
