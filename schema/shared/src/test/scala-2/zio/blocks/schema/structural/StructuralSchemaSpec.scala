  ...
  def spec = suite("StructuralSchemaSpec (Scala 2)")(
    ...
    suite("Nested Case Classes + Collections")(
      test("nested case class round-trip") {
        ...
        // toStructural works - verify field access (need cast for nested in Scala 2)
        assertTrue(
          structural.name == "Alice",
          structural.address.asInstanceOf[StructuralRecord].city == "NYC",
          structural.address.asInstanceOf[StructuralRecord].zip == 10001
        ) &&
        // Now test structuralSchema round-trip
        {
          ...
          assertTrue(
            ...
            roundTrip.map(_.address.asInstanceOf[StructuralRecord].city) == Right("NYC"),
            roundTrip.map(_.address.asInstanceOf[StructuralRecord].zip) == Right(10001)
          )
        }
      },
    ...
    suite("Edge Cases")(
      ...
      test("single field case class round-trip") {
        ...
        implicit val schema: Schema[SingleField] = Schema.derived[SingleField]
        val structSchema                         = ts.structuralSchema
      ...
      test("large product (10 fields) round-trip") {
        ...
        implicit val schema: Schema[LargeProduct] = Schema.derived[LargeProduct]
        val structSchema                          = ts.structuralSchema
    ...
    suite("TypeName Normalization")(
      ...
      test("same structure produces same TypeName") {
        ...
        implicit val schemaA: Schema[PersonA] = Schema.derived[PersonA]
        implicit val schemaB: Schema[PersonB] = Schema.derived[PersonB]
    ...
    suite("Schema.structural convenience method")(
      ...
      test("schema.structural round-trip works") {
        ...
        // Use asInstanceOf[StructuralRecord] since StructuralType = StructuralRecord in Scala 2
        assertTrue(
          roundTrip.isRight,
          roundTrip.map(_.asInstanceOf[StructuralRecord].name) == Right("Bob"),
          roundTrip.map(_.asInstanceOf[StructuralRecord].age) == Right(25)
        )
      }
    )
  )
