...
object SeqConstructor {
  ...
  val iArrayConstructor: SeqConstructor[IArray] = new IArrayConstructor

  class IArrayConstructor extends SeqConstructor[IArray] {
    case class Builder[A](var buffer: Array[A], var size: Int)
    ...
    def resultByte(builder: ByteBuilder): IArray[Byte] = IArray.unsafeFromArray {
      val buf  = builder.buffer
      val size = builder.size
      if (buf.length == size) buf
      else java.util.Arrays.copyOf(buf, size)
    ...
    def resultShort(builder: ShortBuilder): IArray[Short] = IArray.unsafeFromArray {
      val buf  = builder.buffer
      val size = builder.size
      if (buf.length == size) buf
      else java.util.Arrays.copyOf(buf, size)
    ...
    def resultInt(builder: IntBuilder): IArray[Int] = IArray.unsafeFromArray {
      val buf  = builder.buffer
      val size = builder.size
      if (buf.length == size) buf
      else java.util.Arrays.copyOf(buf, size)
    ...
    def resultLong(builder: LongBuilder): IArray[Long] = IArray.unsafeFromArray {
      val buf  = builder.buffer
      val size = builder.size
      if (buf.length == size) buf
      else java.util.Arrays.copyOf(buf, size)
    ...
    def resultFloat(builder: FloatBuilder): IArray[Float] = IArray.unsafeFromArray {
      val buf  = builder.buffer
      val size = builder.size
      if (buf.length == size) buf
      else java.util.Arrays.copyOf(buf, size)
    ...
    def resultDouble(builder: DoubleBuilder): IArray[Double] = IArray.unsafeFromArray {
      val buf  = builder.buffer
      val size = builder.size
      if (buf.length == size) buf
      else java.util.Arrays.copyOf(buf, size)
    ...
    def resultChar(builder: CharBuilder): IArray[Char] = IArray.unsafeFromArray {
      val buf  = builder.buffer
      val size = builder.size
      if (buf.length == size) buf
      else java.util.Arrays.copyOf(buf, size)
