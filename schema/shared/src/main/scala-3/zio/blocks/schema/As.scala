 ...
 * - For any value `a: A`, `from(into(a))` should succeed and produce an equivalent value
 * - For any value `b: B`, `into(from(b))` should succeed and produce an equivalent value
 * 
 * @tparam A First type
...
object As {
   ...
   * 
   * Note: As[A, B] can only be derived when both Into[A, B] and Into[B, A] exist.
   */
...
private object AsMacro {
  ...
  private def checkCompatibility[A: Type, B: Type](using Quotes)(
  ...
  private def checkNoDefaultsUsed[A: Type, B: Type](using Quotes)(
  ...
  private def isNarrowingConversion(using Quotes)(
    source: quotes.reflect.TypeRepr,
    target: quotes.reflect.TypeRepr
  ): Boolean = {
    import quotes.reflect.*
