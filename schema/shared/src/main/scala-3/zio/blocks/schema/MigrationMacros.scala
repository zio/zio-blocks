...
private[schema] object MigrationMacros {
  ...
  def derivedImpl[A: Type, B: Type](schemaA: Expr[Schema[A]], schemaB: Expr[Schema[B]])(using
    q: Quotes
  ): Expr[Migration[A, B]] =
    ...
    '{
      ...
      val recordA =
        reflectA.asRecord.getOrElse(throw new Exception("Migration.derived only supports records (case classes)"))
      val recordB =
        reflectB.asRecord.getOrElse(throw new Exception("Migration.derived only supports records (case classes)"))
      ...
      recordB.fields.foreach { fieldB =>
        if (!fieldsA.contains(fieldB.name)) {
          ...
          val defaultValue = reflectB.getDefaultValue
            .map(v => reflectB.toDynamicValue(v))
