  ...
  def dropFieldImpl[A: Type, B: Type](
    ...
    '{
      new MigrationBuilder(
        ${ builder }.sourceSchema,
        ${ builder }.targetSchema,
        ${ builder }.actions :+ MigrationAction.DropField(
          DynamicOptic.root.field(${ Expr(fieldName) }),
          SchemaExpr.Literal(DynamicValue.Null, Schema.dynamic)
        )
      )
    }
  }
  ...
  def renameFieldImpl[A: Type, B: Type](
    ...
    '{
      new MigrationBuilder(
        ${ builder }.sourceSchema,
        ${ builder }.targetSchema,
        ${ builder }.actions :+ MigrationAction.RenameField(
          DynamicOptic.root.field(${ Expr(fromName) }),
          ${ Expr(toName) }
        )
      )
    }
