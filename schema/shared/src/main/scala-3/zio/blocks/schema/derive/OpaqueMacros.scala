      ...
      underlyingOpt.flatMap { underlying =>
        if (source =:= underlying || source.dealias =:= underlying.dealias) {
          ...
          if (companionSymbol != Symbol.noSymbol) {
            ...
            report.info(
              s"[OpaqueMacros] Companion: ${companionSymbol.name}, Found ${allApplyMethods.size} apply methods"
            )
                ...
                method.termRef.widenTermRefByName match {
                  case mt: MethodType if mt.paramTypes.size == 1 =>
                    ...
                    if (paramMatches) {
                      ...
                      report.info(
                        s"[OpaqueMacros] Method ${method.name}: paramMatches=$paramMatches, returnsEitherStringTarget=$returnsEitherStringTarget, returnType=${returnType.show}, target=${target.show}"
                      )
                    ...
                    method.termRef.widenTermRefByName match {
                      case mt: MethodType if mt.paramTypes.size == 1 =>
                        ...
                        if (paramMatches) {
                          ...
                          report.info(
                            s"[OpaqueMacros] Simple method ${method.name}: paramMatches=$paramMatches, returnsNonEither=$returnsNonEither, returnType=${returnType.show}"
                          )
