 ...
 *
 * Field sets are represented as tuples of string literal types,
 * e.g., `("name", "age", "email")` represents a set of three fields.
 *
 * The type-level operations (Remove, Contains, IsEmpty) are implemented
 * using Scala 3 match types, which provide compile-time type computation.
 *
 * These operations are used by [[MigrationBuilder]] to track which fields
 * have been handled during migration construction, ensuring compile-time
...
object FieldSet {
  ...
  /**
   * Get the size of a tuple (number of fields).
   * Note: This returns a type-level representation, use Tuple.Size for runtime.
   */
