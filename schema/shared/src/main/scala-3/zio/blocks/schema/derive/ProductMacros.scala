...
/**
 * Macros for handling product type conversions (case classes and tuples) in Into derivation.
 */
object ProductMacros {
   ...
   */
  def productTypeConversion[A: Type, B: Type](using Quotes)(
  ...
  private def generateProductConversion[A: Type, B: Type](using Quotes)(
    target: quotes.reflect.TypeRepr,
    ...
    target.classSymbol match {
      ...
      case None =>
        ...
        if (injectNoneActions.nonEmpty || useDefaultActions.nonEmpty) {
          report.errorAndAbort("Schema evolution (Option injection, default values) is not supported for tuple conversions")
        }
              ...
              def into(input: A): Either[zio.blocks.schema.SchemaError, B] = {
                ...
                val result = ${
                  size match {
                    ...
                    case 2 => '{ (prod.productElement(${ Expr(indices(0)) }), prod.productElement(${ Expr(indices(1)) })) }
                    case _ =>
  ...
  /**
   * Generate conversion code for N fields with error accumulation and schema evolution support.
   ...
   * 
   * Note: This uses inline code generation to avoid Quotes context and type issues.
   */
  private def generateNFieldConversionWithEvolution[A: Type, B: Type](using Quotes)(
    target: quotes.reflect.TypeRepr,
      ...
      action match {
        case FieldMappingAction.MapField(sourceIdx, targetIdx) =>
          ...
          val conversionInfo = conversions.find(c => c._1 == sourceIdx && c._2 == targetIdx).getOrElse {
            val sourceField = sourceFields.find(_.position == sourceIdx).getOrElse(
        ...
        case FieldMappingAction.UseDefault(targetIdx, defaultValueTerm) =>
          ...
          targetFieldType.asType match {
            case '[t] =>
              FieldConversionInfo(targetIdx, "default", None, None, Type.of[t], Some(defaultValueTerm.asInstanceOf[Term]))
          }
      }
    }
    ...
    def generateFieldConversion(inputExpr: Expr[A], info: FieldConversionInfo): Expr[Either[zio.blocks.schema.SchemaError, Any]] = {
        ...
        case "into" =>
          ...
          (info.sourceTypeTag.get, info.targetTypeTag) match {
            case ('[s], '[t]) =>
              ...
              '{ 
                val prod = $inputExpr.asInstanceOf[Product]
                $intoExpr.into(prod.productElement(${ Expr(sourceIdx) }).asInstanceOf[s]).asInstanceOf[Either[zio.blocks.schema.SchemaError, Any]]
              }
          }
  ...
  private def generateSingleFieldConversion[A: Type, B: Type](using Quotes)(
    target: quotes.reflect.TypeRepr,
    constructor: quotes.reflect.Symbol,
    info0: { val targetIdx: Int; val conversionType: String; val sourceIdx: Option[Int]; val sourceTypeTag: Option[Type[?]]; val targetTypeTag: Type[?]; val defaultTerm: Option[quotes.reflect.Term] }
  ): Expr[zio.blocks.schema.Into[A, B]] = {
    import quotes.reflect.*
  ...
  private def generateTwoFieldConversion[A: Type, B: Type](using Quotes)(
    target: quotes.reflect.TypeRepr,
    constructor: quotes.reflect.Symbol,
    info0: { val targetIdx: Int; val conversionType: String; val sourceIdx: Option[Int]; val sourceTypeTag: Option[Type[?]]; val targetTypeTag: Type[?]; val defaultTerm: Option[quotes.reflect.Term] },
    info1: { val targetIdx: Int; val conversionType: String; val sourceIdx: Option[Int]; val sourceTypeTag: Option[Type[?]]; val targetTypeTag: Type[?]; val defaultTerm: Option[quotes.reflect.Term] }
  ): Expr[zio.blocks.schema.Into[A, B]] = {
    import quotes.reflect.*
    
    // Generate field extractors for each field
    def extractField(info: { val targetIdx: Int; val conversionType: String; val sourceIdx: Option[Int]; val sourceTypeTag: Option[Type[?]]; val targetTypeTag: Type[?]; val defaultTerm: Option[quotes.reflect.Term] }): (Expr[Product => Either[zio.blocks.schema.SchemaError, Any]], Type[?]) = {
      info.conversionType match {
        case "direct" =>
          val sourceIdx = info.sourceIdx.get
          info.targetTypeTag match {
        ...
        case "into" =>
          ...
          (info.sourceTypeTag.get, info.targetTypeTag) match {
            case ('[s], '[t]) =>
              ...
              ('{ (prod: Product) => $intoExpr.into(prod.productElement(${ Expr(sourceIdx) }).asInstanceOf[s]) }, Type.of[t])
          }
  ...
  private def generateThreeFieldConversion[A: Type, B: Type](using Quotes)(
    target: quotes.reflect.TypeRepr,
    constructor: quotes.reflect.Symbol,
    info0: { val targetIdx: Int; val conversionType: String; val sourceIdx: Option[Int]; val sourceTypeTag: Option[Type[?]]; val targetTypeTag: Type[?]; val defaultTerm: Option[quotes.reflect.Term] },
    info1: { val targetIdx: Int; val conversionType: String; val sourceIdx: Option[Int]; val sourceTypeTag: Option[Type[?]]; val targetTypeTag: Type[?]; val defaultTerm: Option[quotes.reflect.Term] },
    info2: { val targetIdx: Int; val conversionType: String; val sourceIdx: Option[Int]; val sourceTypeTag: Option[Type[?]]; val targetTypeTag: Type[?]; val defaultTerm: Option[quotes.reflect.Term] }
  ): Expr[zio.blocks.schema.Into[A, B]] = {
    import quotes.reflect.*
    
    def extractField(info: { val targetIdx: Int; val conversionType: String; val sourceIdx: Option[Int]; val sourceTypeTag: Option[Type[?]]; val targetTypeTag: Type[?]; val defaultTerm: Option[quotes.reflect.Term] }): Expr[Product => Either[zio.blocks.schema.SchemaError, Any]] = {
      info.conversionType match {
        case "direct" =>
          val sourceIdx = info.sourceIdx.get
  ...
  private def generateMultiFieldConversion[A: Type, B: Type](using Quotes)(
    target: quotes.reflect.TypeRepr,
    constructor: quotes.reflect.Symbol,
    fieldInfos: Seq[{ val targetIdx: Int; val conversionType: String; val sourceIdx: Option[Int]; val sourceTypeTag: Option[Type[?]]; val targetTypeTag: Type[?]; val defaultTerm: Option[quotes.reflect.Term] }]
  ): Expr[zio.blocks.schema.Into[A, B]] = {
    import quotes.reflect.*
    ...
    val extractorsExpr = Expr.ofList(extractors.toSeq)(using Type.of[Product => Either[zio.blocks.schema.SchemaError, Any]])
    '{
      new zio.blocks.schema.Into[A, B] {
        ...
        def into(input: A): Either[zio.blocks.schema.SchemaError, B] = {
          ...
          if (errors.nonEmpty) {
            ...
            Right(${ 
              val newExpr = Select(New(Inferred(target)), constructor)
              val args = (0 until numFields).zip(targetTypes).map { case (i, tpe) =>
                tpe match {
                  case '[t] =>
