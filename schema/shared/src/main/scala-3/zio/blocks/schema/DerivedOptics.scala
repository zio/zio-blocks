...
private object DerivedOpticsMacros {
  ...
  def opticsImpl[S: Type](schema: Expr[Schema[S]])(using q: Quotes): Expr[Any] = {
    ...
    val caseClassType = TypeRepr.of[S].dealias
  ...
  private def buildCaseClassOptics[S: Type](
    ...
    // Get class name at compile time for the cache key (use as string)
    val cacheKey: Expr[String] = Expr(tpeCast.typeSymbol.fullName)
