 ...
 * @param tpe Field type representation (stored as string to avoid quote context issues)
 * @param position Field position in parameter list
 * @param hasDefault Whether this field has a default value in the constructor
 * @param defaultValue Optional default value term (for use in code generation)
 */
...
private[schema] object FieldInfo {
  ...
  def apply(using Quotes)(name: String, tpe: quotes.reflect.TypeRepr, position: Int, hasDefault: Boolean, defaultValue: Option[Any]): FieldInfo =
    new FieldInfo(name, tpe.show, tpe, position, hasDefault, defaultValue)
}

/**
 * Actions for field mapping in schema evolution.
 * Represents different strategies for mapping source to target fields.
 */
...
/**
 * Algorithm for mapping fields between source and target types.
 * Implements a 5-level disambiguation strategy:
 ...
 * 5. Schema Evolution: Option injection or default values for unmapped target fields
 */
private[schema] object FieldMapper {
   ...
   * 
   * @param sourceFields Fields from source type
   * @param targetFields Fields from target type
   * @return Right with mapping actions or Left with error message
   */
  def mapFields(using Quotes)(
    ...
    for (targetIdx <- targetFields.indices if !isTargetMapped(targetIdx)) {
      ...
      if (targetTypeCount == 1) {
        ...
        if (candidateSources.size == 1 && sourceTypeCount == 1) {
          ...
          // Ambiguous: multiple source fields match
          val candidateNames = candidateSources.map { case (f, _) => s"${f.name}: ${f.tpe}" }.mkString(", ")
          return Left(
            s"Cannot derive Into: Ambiguous field mapping\n\n" +
            s"Target field '${targetField.name}' (type: ${targetField.tpe}) matches multiple source fields:\n" +
            s"  $candidateNames\n\n" +
            s"Disambiguation strategies attempted:\n" +
            s"  ✓ Level 1: Exact name + type match - failed\n" +
            s"  ✓ Level 2: Name match with coercion - failed\n" +
            s"  ✗ Level 3: Unique type match - AMBIGUOUS\n\n" +
            s"Possible solutions:\n" +
            s"  1. Rename fields to match exactly (e.g., '${targetField.name}' in both types)\n" +
            s"  2. Make field types unique in both source and target\n" +
            s"  3. Provide explicit Into instance"
          )
        }
      }
    }
   ...
   * Converts new FieldMappingAction results to old (Int, Int) format.
   */
  def mapFieldsLegacy(using Quotes)(
    sourceFields: Seq[FieldInfo],
    targetFields: Seq[FieldInfo]
  ): Either[String, Seq[(Int, Int)]] = {
    FieldMapper.mapFields(using quotes)(sourceFields, targetFields).map { actions =>
      actions.collect {
        case FieldMappingAction.MapField(sourceIdx, targetIdx) => (sourceIdx, targetIdx)
      }
    }
  }
  ...
  private def isZioNewtypePattern(using Quotes)(tpe: quotes.reflect.TypeRepr): Boolean = {
    ...
    val dealiased = try { tpe.dealias } catch { case _: Throwable => tpe }
