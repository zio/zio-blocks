 ...
 *
 * These instances are needed for implicit search by library users (issue
 * #1030). They are defined in a Scala 3-specific source file because Scala 2's
 * macro implementation cannot derive schemas for types from external modules in
