...
/**
 * Macros for handling structural type conversions (Selectable) in Into derivation.
 * 
 * Structural types allow converting case classes to types with structural refinements,
 * e.g., `case class Point(x: Int, y: Int)` → `{ def x: Int; def y: Int }`
 */
object StructuralMacros {
   ...
   */
  def structuralTypeConversion[A: Type, B: Type](using Quotes)(
   ...
   */
  private def generateProductToStructural[A: Type, B: Type](using Quotes)(
    source: quotes.reflect.TypeRepr,
    target: quotes.reflect.TypeRepr
          ...
          try {
            ...
            case e: Exception =>
              Left(zio.blocks.schema.SchemaError.expectationMismatch(
                Nil,
                "Failed to convert " + ${ Expr(source.show) } + " to structural type " + ${ Expr(target.show) } + ": " + e.getMessage
              ))
          }
        }
      }
   ...
   */
  private def generateStructuralToProduct[A: Type, B: Type](using Quotes)(
    source: quotes.reflect.TypeRepr,
    target: quotes.reflect.TypeRepr
  ): Expr[zio.blocks.schema.Into[A, B]] = {
    import quotes.reflect.*
   ...
   */
  private def generateStructuralToStructural[A: Type, B: Type](using Quotes)(
    source: quotes.reflect.TypeRepr,
    target: quotes.reflect.TypeRepr
  ): Expr[zio.blocks.schema.Into[A, B]] = {
    import quotes.reflect.*
        ...
        def into(input: A): Either[zio.blocks.schema.SchemaError, B] = {
          try {
            ...
            case e: Exception =>
              Left(zio.blocks.schema.SchemaError.expectationMismatch(
                Nil,
                "Failed to convert structural type " + ${ Expr(source.show) } + " to " + ${ Expr(target.show) } + ": " + e.getMessage
              ))
          }
        }
      }
   ...
   * Returns a list of tuples (name, paramCount, returnType) for inline comparison.
   */
  private def extractStructuralMethodsWithTypes(using Quotes)(tpe: quotes.reflect.TypeRepr): List[(String, Int, quotes.reflect.TypeRepr)] = {
    import quotes.reflect.*
  ...
  /**
   * Extract fields from a product type (case class) with types.
   * Returns a list of tuples (name, returnType) for inline comparison.
   */
  private def extractProductFieldsWithTypes(using Quotes)(tpe: quotes.reflect.TypeRepr): List[(String, quotes.reflect.TypeRepr)] = {
    import quotes.reflect.*
  ...
  /**
   * Check if two types are compatible for conversion (allowing widening/narrowing with validation).
   */
  private def isTypeCompatible(using Quotes)(source: quotes.reflect.TypeRepr, target: quotes.reflect.TypeRepr): Boolean = {
    import quotes.reflect.*
