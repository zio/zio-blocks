...
object StructuralRuntime {
  final class StructuralSelectable(private val map: Map[String, Any]) extends Selectable {
    def selectDynamic(name: String): Any = map.getOrElse(name, throw new NoSuchElementException(name))
    override def toString: String        = s"StructuralSelectable($map)"
  }

  def fromDynamicValue(value: DynamicValue): StructuralSelectable = {
    def toAny(dv: DynamicValue): Any = dv match {
      case DynamicValue.Primitive(p)     => primitiveToAny(p)
      case DynamicValue.Record(fields)   => new StructuralSelectable(fields.map { case (k, v) => k -> toAny(v) }.toMap)
