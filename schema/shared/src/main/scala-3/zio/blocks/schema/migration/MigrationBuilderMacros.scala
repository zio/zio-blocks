      ...
      case '[t] =>
        val fieldSchema: Expr[Schema[t]] =
          '{
            ...
            _root_.zio.blocks.schema.Schema[t](
              __bound.asInstanceOf[_root_.zio.blocks.schema.Reflect.Bound[t]]
            )
          }
      ...
      case '[t] =>
        ...
        append(
          ...
          '{
            DropField(
              at = $atField,
              defaultForReverse = $capturedReverse.asInstanceOf[SchemaExpr[Any, Any]]
            )
          }
        )
  ...
  def mandateFieldImpl[A: Type, B: Type](
    ...
    fieldTpe.asType match {
      case '[t] =>
        val fieldSchema: Expr[Schema[t]] =
          '{
            val __atNew = $sourceParen.field($targetNameE)
            ...
            val __bound =
              $builder.targetSchema
                ...
                .getOrElse(
                  throw new IllegalArgumentException(
                    "mandateField(target): field not found in target schema: " + __atNew
                  )
                )
