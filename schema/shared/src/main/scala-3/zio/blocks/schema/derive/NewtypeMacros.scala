...
/**
 * Macros for handling ZIO Prelude Newtype/Subtype conversions in Into derivation.
 * 
 * ZIO Prelude newtypes are NOT Scala 3 opaque types - they use a different encoding:
...
object NewtypeMacros {
  ...
  /**
   * Attempts to derive an Into[A, B] instance for ZIO Prelude Newtype conversions.
   * 
   * @return Some(expr) if conversion is possible, None otherwise
   */
  def newtypeConversion[A: Type, B: Type](using Quotes)(
    ...
    if (!isSourceNewtype && isTargetNewtype) {
      ...
      (underlyingOpt, companionOpt) match {
        case (Some(underlying), Some(companion)) 
          ...
          val methodNamesToTry = List("make", "apply", "validate", "fromString", "fromInt", "fromLong", "fromDouble", "fromFloat")
          ...
          validatingMethod match {
            case Some(method) =>
              ...
              if (isValidation) {
                ...
                val lambda = Lambda(Symbol.spliceOwner, mt, { (_, args) =>
                      ...
                      try {
                        ...
                        either.fold(
                           errs => {
                              ...
                              Left(zio.blocks.schema.SchemaError.expectationMismatch(Nil, errorMsg + " (input: " + input + ")"))
                           },
                           success => Right(success)
                        )
                      } catch {
                        case e: Exception => 
                           Left(zio.blocks.schema.SchemaError.expectationMismatch(Nil, "Validation error: " + e.toString))
                      }
                    }
                  }
                })
