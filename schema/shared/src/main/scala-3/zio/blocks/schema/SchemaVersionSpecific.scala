...
private object SchemaVersionSpecific {
  ...
  def derived[A: Type](using Quotes): Expr[Schema[A]] = {
    ...
    val schemaRefs = new mutable.HashMap[TypeRepr, Expr[Schema[?]]]
    val schemaDefs = new mutable.ListBuffer[ValDef]

    def findImplicitOrDeriveSchema[T: Type](tpe: TypeRepr)(using Quotes): Expr[Schema[T]] = schemaRefs
      .getOrElseUpdate(
        tpe, {
          ...
          tpeParams.map(_.map { symbol =>
            ...
            val defaultValue =
              if (symbol.flags.is(Flags.HasDefault)) {
                ...
                if (methodSymbols eq Nil) fail(s"Cannot find default value for '$symbol' in class '${tpe.show}'.")
