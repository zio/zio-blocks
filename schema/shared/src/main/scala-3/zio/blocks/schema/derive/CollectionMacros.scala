...
object CollectionMacros {
   ...
   */
  def collectionConversion[A: Type, B: Type](using
    Quotes
  )(
    ...
    if (sourceIsArray && targetIsArray) {
      ...
      return Some(
        generateArrayToCollectionConversion[A, B](
          sourceElemType,
          targetElemType,
    ...
    Some(
      generateCollectionConversion[A, B](
        source,
        target,
        sourceElemType,
        targetElemType,
  ...
  private def generateOptionConversion[A: Type, B: Type](using
    Quotes
  )(
                      ...
                      case Some(value) =>
                        ${
                          ...
                          Expr.summon[zio.blocks.schema.Into[s, t]] match {
                            ...
                            case None =>
                              // Pattern Variable + Cast to resolve E172
                              // Use IntoMacro.summonOrDerive to avoid inline expansion loops
                              val innerInto = IntoMacro.summonOrDerive[s, t]
                              '{
                                lazy val fallback: zio.blocks.schema.Into[s, t] = $innerInto
                                fallback
                                  .asInstanceOf[zio.blocks.schema.Into[s, t]]
                                  .into(value)
                                  .map(v => Some(v).asInstanceOf[B])
                              }
                          }
                        }
                    }
  ...
  private def generateCollectionConversion[A: Type, B: Type](using
    Quotes
  )(
    source: quotes.reflect.TypeRepr,
    target: quotes.reflect.TypeRepr,
  ...
  private def generateEitherConversion[A: Type, B: Type](using
    Quotes
  )(
                                  ...
                                  if (leftSame) {
                                    ...
                                    val leftInto = Expr.summon[zio.blocks.schema.Into[sl, tl]] match {
                                      ...
                                      case None       =>
                                        // Pattern Variable + Cast to resolve E172
                                        // Use IntoMacro.summonOrDerive to avoid inline expansion loops
                                        val innerInto = IntoMacro.summonOrDerive[sl, tl]
                                        '{
                                          lazy val fallback: zio.blocks.schema.Into[sl, tl] = $innerInto
                                          fallback.asInstanceOf[zio.blocks.schema.Into[sl, tl]]
                                        }
                                    }
                                  ...
                                  if (rightSame) {
                                    ...
                                    val rightInto = Expr.summon[zio.blocks.schema.Into[sr, tr]] match {
                                      ...
                                      case None       =>
                                        // Pattern Variable + Cast to resolve E172
                                        // Use IntoMacro.summonOrDerive to avoid inline expansion loops
                                        val innerInto = IntoMacro.summonOrDerive[sr, tr]
                                        '{
                                          lazy val fallback: zio.blocks.schema.Into[sr, tr] = $innerInto
                                          fallback.asInstanceOf[zio.blocks.schema.Into[sr, tr]]
                                        }
                                    }
  ...
  private def generateMapConversion[A: Type, B: Type](using
    Quotes
  )(
                            ...
                            ${
                              val keyIntoExpr = if (keySame) {
                                ...
                                Some(Expr.summon[zio.blocks.schema.Into[sk, tk]] match {
                                  ...
                                  case None       =>
                                    // Pattern Variable + Cast to resolve E172
                                    // Use IntoMacro.summonOrDerive to avoid inline expansion loops
                                    val innerInto = IntoMacro.summonOrDerive[sk, tk]
                                    '{
                                      lazy val fallback: zio.blocks.schema.Into[sk, tk] = $innerInto
                                      fallback.asInstanceOf[zio.blocks.schema.Into[sk, tk]]
                                    }
                                })
                              }
                              ...
                              val valueIntoExpr = if (valueSame) {
                                ...
                                Some(Expr.summon[zio.blocks.schema.Into[sv, tv]] match {
                                  ...
                                  case None       =>
                                    // Pattern Variable + Cast to resolve E172
                                    // Use IntoMacro.summonOrDerive to avoid inline expansion loops
                                    val innerInto = IntoMacro.summonOrDerive[sv, tv]
                                    '{
                                      lazy val fallback: zio.blocks.schema.Into[sv, tv] = $innerInto
                                      fallback.asInstanceOf[zio.blocks.schema.Into[sv, tv]]
                                    }
                                })
                              }
                              ...
                              (keyIntoExpr, valueIntoExpr) match {
                                ...
                                case (Some(keyInto), None) =>
                                  ...
                                  '{
                                    map
                                      .foldLeft[Either[zio.blocks.schema.SchemaError, Map[tk, sv]]](Right(Map.empty)) {
                                        case (Right(acc), (k, v)) =>
                                ...
                                case (None, Some(valueInto)) =>
                                  ...
                                  '{
                                    map
                                      .foldLeft[Either[zio.blocks.schema.SchemaError, Map[sk, tv]]](Right(Map.empty)) {
                                        case (Right(acc), (k, v)) =>
                                ...
                                case (Some(keyInto), Some(valueInto)) =>
                                  ...
                                  '{
                                    map
                                      .foldLeft[Either[zio.blocks.schema.SchemaError, Map[tk, tv]]](Right(Map.empty)) {
                                        case (Right(acc), (k, v)) =>
                                          for {
  ...
  private def generateArrayToArrayConversion[A: Type, B: Type](using
    Quotes
  )(
    sourceElemType: quotes.reflect.TypeRepr,
    targetElemType: quotes.reflect.TypeRepr
    ...
    if (sourceElemType =:= targetElemType) {
      ...
      sourceElemType.asType match {
        case '[s] =>
          targetElemType.asType match {
            case '[t] =>
              // Summon ClassTag OUTSIDE quotes (crucial)
              Expr.summon[scala.reflect.ClassTag[t]] match {
                case Some(classTagExpr) =>
                  val innerInto = IntoMacro.summonOrDerive[s, t]
                  '{
                    // Inject the summoned ClassTag into the generated scope
                    given scala.reflect.ClassTag[t] = $classTagExpr

                    new zio.blocks.schema.Into[A, B] {
                      def into(input: A): Either[zio.blocks.schema.SchemaError, B] = {
                        val arr      = input.asInstanceOf[Array[s]]
                        val elemInto = ${ innerInto }
                        ...
                        error match {
                          case Some(err) => Left(err)
                          case None      => Right(buf.toArray.asInstanceOf[B]) // Works because ClassTag[t] is present!
                        }
                      }
                    }
                  }
  ...
  private def generateArrayToCollectionConversion[A: Type, B: Type](using
    Quotes
  )(
    sourceElemType: quotes.reflect.TypeRepr,
    targetElemType: quotes.reflect.TypeRepr,
              ...
              '{
                new zio.blocks.schema.Into[A, B] {
                  def into(input: A): Either[zio.blocks.schema.SchemaError, B] = {
                    ...
                    // Pattern Variable + Cast to resolve E172
                    lazy val fallback: zio.blocks.schema.Into[s, t] = $innerInto
                    val elemInto                                    = fallback.asInstanceOf[zio.blocks.schema.Into[s, t]]

                    val converted =
                      arr.foldLeft[Either[zio.blocks.schema.SchemaError, List[t]]](Right(Nil)) {
                        case (Right(acc), elem) =>
  ...
  private def generateCollectionToArrayConversion[A: Type, B: Type](using
    Quotes
  )(
    sourceElemType: quotes.reflect.TypeRepr,
    targetElemType: quotes.reflect.TypeRepr
    ...
    if (sourceElemType =:= targetElemType) {
      // Even when types match, we need ClassTag to create the correct array type
      // Vector -> Array requires explicit ClassTag to avoid Object[] creation
      sourceElemType.asType match {
        case '[s] =>
          Expr.summon[scala.reflect.ClassTag[s]] match {
            case Some(classTagExpr) =>
              '{
                implicit val classTag: scala.reflect.ClassTag[s] = $classTagExpr
                new zio.blocks.schema.Into[A, B] {
                  def into(input: A): Either[zio.blocks.schema.SchemaError, B] = {
                    val coll = input.asInstanceOf[Iterable[s]]
                    // CRITICAL: Use toArray with explicit ClassTag to avoid Object[] creation
                    // Without explicit ClassTag, toArray creates Array[Object] due to type erasure
                    val arr: Array[s] = coll.toArray
                    Right(arr.asInstanceOf[B])
                  }
                }
              }
            case None =>
              report.errorAndAbort(s"Cannot find ClassTag for ${sourceElemType.show}")
          }
      }
    } else {
      sourceElemType.asType match {
            ...
            case '[t] =>
              ...
              Expr.summon[scala.reflect.ClassTag[t]] match {
                case Some(classTagExpr) =>
                  ...
                  '{
                    // Inject the summoned ClassTag into the generated scope
                    implicit val classTag: scala.reflect.ClassTag[t] = $classTagExpr
                    ...
                    new zio.blocks.schema.Into[A, B] {
                      def into(input: A): Either[zio.blocks.schema.SchemaError, B] = {
                        ...
                        converted.map { list =>
                          val reversed = list.reverse
                          // CRITICAL: Use toArray with implicit ClassTag to avoid Object[] creation
                          reversed.toArray.asInstanceOf[B]
                        }
                      }
                    }
                  }
