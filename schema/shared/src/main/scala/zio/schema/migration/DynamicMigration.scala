...
object DynamicMigration {
         ...
         sourceValues.find(_.isLeft) match {
            ...
            case _ =>
               ...
               if (values.contains(None)) Left(EvaluationError(at, "Join failed: One or more source paths missing"))
                  ...
                  val combinedInput = DynamicValue.Record(values.map(_.get).zipWithIndex.map { case (v, i) => s"_${i + 1}" -> v })
                        ...
                        case (acc, (path, idx)) =>
                           acc.flatMap { curr =>
                              fields.find(_._1 == s"_${idx + 1}") match {
                                 ...
                                 case None => Left(EvaluationError(at, s"Splitter result missing field _${idx + 1} for target $path"))
                              }
                           }
                     }
                  case other => Left(EvaluationError(at, s"Splitter must return a Record with positional fields (_1, _2...), got $other"))
               }
         }
  ...
  private def insertAt(root: DynamicValue, path: DynamicOptic, valueToInsert: DynamicValue): Either[MigrationError, DynamicValue] =
  ...
  private def transformAt(root: DynamicValue, path: DynamicOptic, expr: SchemaExpr[_, _]): Either[MigrationError, DynamicValue] =
    updateAt(root, path.nodes, v => evalExpr(expr, v), insert = false)

  private def transformAtSimple(root: DynamicValue, path: DynamicOptic, f: DynamicValue => Either[MigrationError, DynamicValue]): Either[MigrationError, DynamicValue] =
    ...
    expr match {
      ...
      case DefaultValue(schema) =>
         schema.getDefaultValue match {
           ...
           case None => Left(EvaluationError(zio.blocks.schema.DynamicOptic(Vector.empty), "DefaultValue expression failed: No default value in schema"))
         }
      ...
      case l: SchemaExpr.Logical[_] =>
         ...
         for {
           ...
           rVal <- evalExpr(l.right, input)
            ...
            (lVal, rVal) match {
              case (DynamicValue.Primitive(PrimitiveValue.Boolean(a)), DynamicValue.Primitive(PrimitiveValue.Boolean(b))) =>
