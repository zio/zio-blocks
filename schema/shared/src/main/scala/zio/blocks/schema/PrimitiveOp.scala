...
/**
 * PrimitiveOp represents delta operations on primitive values. These allow
 * representing changes as increments rather than full replacements, enabling
 * more efficient patches and better merge semantics.
 */
...
object PrimitiveOp {
  ...
  def apply(value: PrimitiveValue, op: PrimitiveOp): Either[SchemaError, PrimitiveValue] = op match {
    ...
    case LocalDateTimeDelta(period, duration) =>
      value match {
