...
/** Top-level ADTs for [[MigrationExpr]] serialization (Schema.derived). */

sealed trait MigrationSchemaSlot {
  def flip: MigrationSchemaSlot
}
object MigrationSchemaSlot {
  case object Source extends MigrationSchemaSlot {
    def flip: MigrationSchemaSlot = Target
  }
  case object Target extends MigrationSchemaSlot {
    def flip: MigrationSchemaSlot = Source
  }
}
