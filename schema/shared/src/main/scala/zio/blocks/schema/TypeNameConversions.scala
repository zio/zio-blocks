...
/**
 * Utilities for converting between TypeName and zio.schema.TypeId.
 * These conversions enable the migration from TypeName to TypeId while maintaining backward compatibility.
 */
object TypeNameConversions {
   ...
   * 
   * Type parameters are encoded in the typeName string (e.g., "List[Int]") since
   * TypeId.Nominal doesn't support type parameters directly.
   * 
   * @param typeName The TypeName to convert
  ...
  /**
   * Converts a zio.schema.TypeId to a TypeName.
   * This is provided for backward compatibility during the migration period.
   * 
   * Note: Type parameters encoded in the typeName string cannot be reliably parsed back,
   * so they will be lost in this conversion. This is a limitation of the conversion.
   * 
  ...
  /**
   * Extension methods for TypeId to enable easy conversion to TypeName (for backward compatibility).
   */
