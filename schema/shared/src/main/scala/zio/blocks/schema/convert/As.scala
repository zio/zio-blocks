 ...
 * 
 * Compatibility Rules:
 *   - Field mappings must be consistent: The same field correspondence in both directions
 *   - Coercions must be invertible with runtime validation (Int ↔ Long, Float ↔ Double, etc.)
 *   - Optional fields: Can add optional fields in one direction (becomes None in reverse)
 *   - Default values: Cannot use default arguments (breaks round-trip guarantee)
 *   - Collection types: Can convert between different collection types (may be lossy for Set/List conversions)
 */
trait As[A, B] {
