  ...
  def applyAt(
    ...
    case head :: tail =>
      ...
      (head, current) match {
        ...
        case (DynamicOptic.Node.AtIndex(idx), DynamicValue.Sequence(elems)) =>
          if (idx >= 0 && idx < elems.length) {
            ...
            if (mode == PatchMode.Strict)
              Left(SchemaError.expectationMismatch(currentTrace, s"Index $idx out of bounds"))
            else Right(current)
          }
        ...
        case _ =>
          if (mode == PatchMode.Strict)
            Left(SchemaError.expectationMismatch(currentTrace, s"Path not found or type mismatch at $head"))
          else Right(current)
      }
  }
}
  ...
  case class SequenceEdit(ops: Vector[SeqOp]) extends Operation {
    def apply(target: DynamicValue, mode: PatchMode): Either[SchemaError, DynamicValue] = target match {
      case DynamicValue.Sequence(elements) =>
        ops
          .foldLeft[Either[SchemaError, Vector[DynamicValue]]](Right(elements)) {
  ...
  case class MapEdit(ops: Vector[MapOp]) extends Operation {
    def apply(target: DynamicValue, mode: PatchMode): Either[SchemaError, DynamicValue] = target match {
      case DynamicValue.Map(entries) =>
        ops
          .foldLeft[Either[SchemaError, Vector[(DynamicValue, DynamicValue)]]](Right(entries)) {
            case (Right(seq), op) => op.apply(seq, mode)
            case (Left(e), _)     => Left(e)
          }
...
object PrimitiveOp {
  ...
  case class BigIntDelta(delta: BigInt) extends PrimitiveOp {
    def apply(value: DynamicValue): Either[SchemaError, DynamicValue] = value match {
      case DynamicValue.Primitive(PrimitiveValue.BigInt(v)) =>
        Right(DynamicValue.Primitive(PrimitiveValue.BigInt(v + delta)))
  ...
  case class BigDecimalDelta(delta: BigDecimal) extends PrimitiveOp {
    def apply(value: DynamicValue): Either[SchemaError, DynamicValue] = value match {
      case DynamicValue.Primitive(PrimitiveValue.BigDecimal(v)) =>
        Right(DynamicValue.Primitive(PrimitiveValue.BigDecimal(v + delta)))
  ...
  case class DurationDelta(delta: java.time.Duration) extends PrimitiveOp {
    def apply(value: DynamicValue): Either[SchemaError, DynamicValue] = value match {
      case DynamicValue.Primitive(PrimitiveValue.Duration(v)) =>
        Right(DynamicValue.Primitive(PrimitiveValue.Duration(v.plus(delta))))
  ...
  case class InstantDelta(delta: java.time.Duration) extends PrimitiveOp {
    def apply(value: DynamicValue): Either[SchemaError, DynamicValue] = value match {
      case DynamicValue.Primitive(PrimitiveValue.Instant(v)) =>
        Right(DynamicValue.Primitive(PrimitiveValue.Instant(v.plus(delta))))
  ...
  case class StringEdit(ops: Vector[StringOp]) extends PrimitiveOp {
    def apply(value: DynamicValue): Either[SchemaError, DynamicValue] = value match {
      case DynamicValue.Primitive(PrimitiveValue.String(s)) =>
        ops
          .foldLeft[Either[SchemaError, String]](Right(s)) {
  ...
  case class Insert(index: Int, text: String) extends StringOp {
    def apply(s: String): Either[SchemaError, String] =
      if (index < 0 || index > s.length)
        Left(SchemaError.expectationMismatch(Nil, s"String index $index out of bounds"))
  ...
  case class Delete(index: Int, length: Int) extends StringOp {
    def apply(s: String): Either[SchemaError, String] =
      if (index < 0 || index + length > s.length)
        Left(SchemaError.expectationMismatch(Nil, s"String deletion out of bounds"))
...
object SeqOp {
  ...
  case class Delete(index: Int, count: Int) extends SeqOp {
    def apply(seq: Vector[DynamicValue], mode: PatchMode): Either[SchemaError, Vector[DynamicValue]] =
      if (index < 0 || index + count > seq.length) {
        if (mode == PatchMode.Strict)
          Left(SchemaError.expectationMismatch(Nil, s"Sequence index $index out of bounds for delete"))
        else Right(seq)
  ...
  case class Modify(index: Int, op: Operation) extends SeqOp {
    def apply(seq: Vector[DynamicValue], mode: PatchMode): Either[SchemaError, Vector[DynamicValue]] =
      if (index < 0 || index >= seq.length) {
        if (mode == PatchMode.Strict)
          Left(SchemaError.expectationMismatch(Nil, s"Sequence index $index out of bounds for modify"))
        else Right(seq)
      } else {
        ...
        if (oldV == newV) empty
          ...
          (oldV, newV) match {
            case (PrimitiveValue.Int(o), PrimitiveValue.Int(n)) =>
              DynamicPatch(
                Vector(DynamicPatchOp(toIdentityOptic, Operation.PrimitiveDelta(PrimitiveOp.IntDelta(n - o))))
              )
            case (PrimitiveValue.Long(o), PrimitiveValue.Long(n)) =>
              DynamicPatch(
                Vector(DynamicPatchOp(toIdentityOptic, Operation.PrimitiveDelta(PrimitiveOp.LongDelta(n - o))))
              )
            case (PrimitiveValue.Float(o), PrimitiveValue.Float(n)) =>
              DynamicPatch(
                Vector(DynamicPatchOp(toIdentityOptic, Operation.PrimitiveDelta(PrimitiveOp.FloatDelta(n - o))))
              )
            case (PrimitiveValue.Double(o), PrimitiveValue.Double(n)) =>
              DynamicPatch(
                Vector(DynamicPatchOp(toIdentityOptic, Operation.PrimitiveDelta(PrimitiveOp.DoubleDelta(n - o))))
              )
            case (PrimitiveValue.BigInt(o), PrimitiveValue.BigInt(n)) =>
              DynamicPatch(
                Vector(DynamicPatchOp(toIdentityOptic, Operation.PrimitiveDelta(PrimitiveOp.BigIntDelta(n - o))))
              )
            case (PrimitiveValue.BigDecimal(o), PrimitiveValue.BigDecimal(n)) =>
              DynamicPatch(
                Vector(DynamicPatchOp(toIdentityOptic, Operation.PrimitiveDelta(PrimitiveOp.BigDecimalDelta(n - o))))
              )
            case (PrimitiveValue.Duration(o), PrimitiveValue.Duration(n)) =>
              DynamicPatch(
                Vector(DynamicPatchOp(toIdentityOptic, Operation.PrimitiveDelta(PrimitiveOp.DurationDelta(n.minus(o)))))
              )
            case (PrimitiveValue.Instant(o), PrimitiveValue.Instant(n)) =>
              DynamicPatch(
                Vector(
                  DynamicPatchOp(
                    toIdentityOptic,
                    Operation.PrimitiveDelta(PrimitiveOp.InstantDelta(java.time.Duration.between(o, n)))
                  )
                )
              )
            case (PrimitiveValue.String(o), PrimitiveValue.String(n)) =>
              ...
              DynamicPatch(
                Vector(DynamicPatchOp(toIdentityOptic, Operation.PrimitiveDelta(PrimitiveOp.StringEdit(ops))))
              )
            case _ =>
