...
sealed trait Reflect[F[_, _], A] extends Reflectable[A] { self =>
  ...
  def isOption: Boolean = isVariant && {
    ...
    tid.name == "Option" && tid.owner.isScala &&
...
object Reflect {
  ...
  case class Dynamic[F[_, _]](
    ...
    typeId: TypeId[DynamicValue] = TypeId.nominal(
      "DynamicValue",
      zio.blocks.typeid.Owner.parse("zio.blocks.schema"),
      Nil,
  ...
  private[this] def some[F[_, _], A](element: Reflect[F, A])(implicit F: FromBinding[F]): Record[F, Some[A]] =
    new Record(
      Vector(new Term("value", element)),
      TypeId.Option.Some(element.typeId),
      F.fromBinding(Binding.Record.some[AnyRef].asInstanceOf[Binding[BindingType.Record, Some[A]]])
    )
  ...
  private[this] def none[F[_, _]](implicit F: FromBinding[F]): Record[F, None.type] =
    new Record(Vector(), TypeId.Option.None, F.fromBinding(Binding.Record.none))

  def option[F[_, _], A](element: Reflect[F, A])(implicit F: FromBinding[F]): Variant[F, Option[A]] =
    new Variant(
      Vector(
        new Term("None", none),
        new Term("Some", some(element.asInstanceOf[Reflect[F, AnyRef]]).asInstanceOf[Reflect[F, ? <: Option[A]]])
      ),
  ...
  private def tparam(t: TypeId[?]): TypeParam =
  ...
  def list[F[_, _], A](element: Reflect[F, A])(implicit F: FromBinding[F]): Sequence[F, A, List] =
    new Sequence(
      ...
      TypeId.nominal(
        ...
        Owner.parse("scala.collection.immutable"),
        List(tparam(element.typeId)),
        zio.blocks.typeid.TypeDefKind.Trait(true, Nil)
      ),
  ...
  def vector[F[_, _], A](element: Reflect[F, A])(implicit F: FromBinding[F]): Sequence[F, A, Vector] =
    new Sequence(
      ...
      TypeId.nominal(
        ...
        Owner.parse("scala.collection.immutable"),
        List(tparam(element.typeId)),
        zio.blocks.typeid.TypeDefKind.Class(true, false, false, false)
      ),
  ...
  def map[F[_, _], K, V](key: Reflect[F, K], value: Reflect[F, V])(implicit
    ...
    val kparam = TypeParam("K", 0, Variance.Invariant, TypeBounds.exact(TypeRepr.Ref(key.typeId)))
    val vparam = TypeParam("V", 1, Variance.Covariant, TypeBounds.exact(TypeRepr.Ref(value.typeId)))
    new Map(
      key,
      value,
      TypeId.nominal(
    ...
    object List {
      def unapply[F[_, _], A](reflect: Reflect[F, List[A]]): Option[Reflect[F, A]] =
        reflect.asSequenceUnknown.collect {
          case x
              if x.sequence.typeId == TypeId.nominal(
                "List",
                Owner.parse("scala.collection.immutable"),
                scala.List(tparam(x.sequence.element.typeId)),
                zio.blocks.typeid.TypeDefKind.Trait(true, Nil)
              ) =>
