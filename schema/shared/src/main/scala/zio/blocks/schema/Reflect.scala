...
sealed trait Reflect[F[_, _], A] extends Reflectable[A] { self =>
  ...
  def isOption: Boolean = isVariant && {
    ...
    // Check namespace is exactly "scala" to prevent matching scala.collection.immutable.Map etc.
    val isScalaOption = tn.owner.segments match {
      case List(zio.blocks.typeid.Owner.Package("scala")) => true
      case _                                              => false
    }
    isScalaOption && tn.name == "Option" &&
