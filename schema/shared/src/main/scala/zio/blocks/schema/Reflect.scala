...
sealed trait Reflect[F[_, _], A] extends Reflectable[A] { self =>
  ...
  def typeName: TypeName[A] = TypeName(typeId)

  def typeName(value: TypeName[A]): Reflect[F, A] = typeId(value.id)
...
object Reflect {
  ...
  case class Record[F[_, _], A](
    ...
    override def typeId(value: TypeId[A]): Record[F, A] = copy(typeId = value)

    override def typeName(value: TypeName[A]): Record[F, A] = typeId(value.id)
  ...
  case class Variant[F[_, _], A](
    ...
    override def typeId(value: TypeId[A]): Variant[F, A] = copy(typeId = value)

    override def typeName(value: TypeName[A]): Variant[F, A] = typeId(value.id)
  ...
  case class Sequence[F[_, _], A, C[_]](
    ...
    override def typeId(value: TypeId[C[A]]): Sequence[F, A, C] = copy(typeId = value)

    override def typeName(value: TypeName[C[A]]): Sequence[F, A, C] = typeId(value.id)
  ...
  case class Map[F[_, _], K, V, M[_, _]](
    ...
    override def typeId(value: TypeId[M[K, V]]): Map[F, K, V, M] = copy(typeId = value)

    override def typeName(value: TypeName[M[K, V]]): Map[F, K, V, M] = typeId(value.id)
  ...
  case class Dynamic[F[_, _]](
    ...
    typeId: TypeId[DynamicValue] = TypeName.dynamicValue,
    doc: Doc = Doc.Empty,
    modifiers: Seq[Modifier.Reflect] = Nil
    ...
    override def typeId(value: TypeId[DynamicValue]): Dynamic[F] = copy(typeId = value)

    override def typeName(value: TypeName[DynamicValue]): Dynamic[F] = typeId(value.id)
  ...
  case class Primitive[F[_, _], A](
    ...
    override def typeId(value: TypeId[A]): Primitive[F, A] = copy(typeId = value)

    override def typeName(value: TypeName[A]): Primitive[F, A] = typeId(value.id)
  ...
  case class Wrapper[F[_, _], A, B](
    ...
    override def typeId(value: TypeId[A]): Wrapper[F, A, B] = copy(typeId = value)

    override def typeName(value: TypeName[A]): Wrapper[F, A, B] = typeId(value.id)
  ...
  private[this] def some[F[_, _], A <: AnyRef](element: Reflect[F, A])(implicit F: FromBinding[F]): Record[F, Some[A]] =
    new Record(Vector(new Term("value", element)), TypeName.some(element.typeId), F.fromBinding(Binding.Record.some))
  ...
  def option[F[_, _], A <: AnyRef](element: Reflect[F, A])(implicit F: FromBinding[F]): Variant[F, Option[A]] =
    new Variant(
      ...
      TypeName.option(element.typeId),
  ...
  def set[F[_, _], A](element: Reflect[F, A])(implicit F: FromBinding[F]): Sequence[F, A, Set] =
    new Sequence(element, TypeName.set(element.typeId), F.fromBinding(Binding.Seq.set))

  def list[F[_, _], A](element: Reflect[F, A])(implicit F: FromBinding[F]): Sequence[F, A, List] =
    new Sequence(element, TypeName.list(element.typeId), F.fromBinding(Binding.Seq.list))

  def vector[F[_, _], A](element: Reflect[F, A])(implicit F: FromBinding[F]): Sequence[F, A, Vector] =
    new Sequence(element, TypeName.vector(element.typeId), F.fromBinding(Binding.Seq.vector))

  def indexedSeq[F[_, _], A](element: Reflect[F, A])(implicit F: FromBinding[F]): Sequence[F, A, IndexedSeq] =
    new Sequence(element, TypeName.indexedSeq(element.typeId), F.fromBinding(Binding.Seq.indexedSeq))

  def seq[F[_, _], A](element: Reflect[F, A])(implicit F: FromBinding[F]): Sequence[F, A, Seq] =
    new Sequence(element, TypeName.seq(element.typeId), F.fromBinding(Binding.Seq.seq))

  def map[F[_, _], K, V](key: Reflect[F, K], value: Reflect[F, V])(implicit
    F: FromBinding[F]
  ): Map[F, K, V, collection.immutable.Map] = {
    val typeId = TypeName.map(key.typeId, value.typeId)
  ...
  object Extractors {
    object List {
      def unapply[F[_, _], A](reflect: Reflect[F, List[A]]): Option[Reflect[F, A]] =
        reflect.asSequenceUnknown.collect {
          case x if x.sequence.typeId == TypeName.list(x.sequence.element.typeId) =>
    ...
    object Vector {
      def unapply[F[_, _], A](reflect: Reflect[F, Vector[A]]): Option[Reflect[F, A]] =
        reflect.asSequenceUnknown.collect {
          case x if x.sequence.typeId == TypeName.vector(x.sequence.element.typeId) =>
            x.sequence.element.asInstanceOf[Reflect[F, A]]
        }
    }
    ...
    object Set {
      def unapply[F[_, _], A](reflect: Reflect[F, Set[A]]): Option[Reflect[F, A]] =
        reflect.asSequenceUnknown.collect {
          case x if x.sequence.typeId == TypeName.set(x.sequence.element.typeId) =>
            x.sequence.element.asInstanceOf[Reflect[F, A]]
        }
    }
  }
