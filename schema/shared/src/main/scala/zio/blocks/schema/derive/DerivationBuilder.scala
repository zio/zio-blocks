...
final case class DerivationBuilder[TC[_], A](
  ...
  def modifier[B](optic: Optic[A, B], modifier: Modifier): DerivationBuilder[TC, A] = modifier match {
    ...
    case mt: Modifier.Term =>
      ...
      if (nodes.isEmpty) this
        ...
        nodes.last match {
          case Node.Field(n) =>
            copy(modifierOverrides = modifierOverrides :+ new ModifierTermOverrideByOptic(path, n, mt))
          case Node.Case(n) =>
            copy(modifierOverrides = modifierOverrides :+ new ModifierTermOverrideByOptic(path, n, mt))
          case _ => this
        }
  ...
  lazy val derive: TC[A] = {
    ...
    val instanceByOpticMap: Map[DynamicOptic, Lazy[Any]] =
      allInstanceOverrides.collect { case io: InstanceOverrideByOptic[?, ?] => (io.optic, io.instance) }.toMap
    val instanceByTypeMap: Map[TypeId[Any], Lazy[Any]] =
      allInstanceOverrides.collect { case io: InstanceOverrideByType[?, ?] =>
        (io.typeId.asInstanceOf[TypeId[Any]], io.instance)
      }.toMap
