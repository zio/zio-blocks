...
abstract class JsonBinaryCodec[A](val valueType: Int) extends BinaryCodec[A] {
...
object JsonBinaryCodec {
  ...
  private class PrimitiveCodec[A](tpe: Int, val nullValue: A, decoder: (JsonReader, A) => A, encoder: (A, JsonWriter) => Unit) extends JsonBinaryCodec[A](tpe) {
  ...
  implicit val intCodec: JsonBinaryCodec[Int] = new PrimitiveCodec(intType, 0, (in, _) => in.readInt(), (x, out) => out.writeVal(x))
  implicit val longCodec: JsonBinaryCodec[Long] = new PrimitiveCodec(longType, 0L, (in, _) => in.readLong(), (x, out) => out.writeVal(x))
  implicit val floatCodec: JsonBinaryCodec[Float] = new PrimitiveCodec(floatType, 0.0f, (in, _) => in.readFloat(), (x, out) => out.writeVal(x))
  implicit val doubleCodec: JsonBinaryCodec[Double] = new PrimitiveCodec(doubleType, 0.0, (in, _) => in.readDouble(), (x, out) => out.writeVal(x))
  implicit val booleanCodec: JsonBinaryCodec[Boolean] = new PrimitiveCodec(booleanType, false, (in, _) => in.readBoolean(), (x, out) => out.writeVal(x))
  implicit val byteCodec: JsonBinaryCodec[Byte] = new PrimitiveCodec(byteType, 0.toByte, (in, _) => in.readByte(), (x, out) => out.writeVal(x))
  implicit val charCodec: JsonBinaryCodec[Char] = new PrimitiveCodec(charType, '\u0000', (in, _) => in.readChar(), (x, out) => out.writeVal(x))
  implicit val shortCodec: JsonBinaryCodec[Short] = new PrimitiveCodec(shortType, 0.toShort, (in, _) => in.readShort(), (x, out) => out.writeVal(x))
  ...
  implicit val stringCodec: JsonBinaryCodec[java.lang.String] = new PrimitiveCodec(objectType, "", _.readString(_), (x, out) => if (x == null) out.writeNull() else out.writeVal(x))
  ...
  implicit val dynamicValueCodec: JsonBinaryCodec[DynamicValue] = new JsonBinaryCodec[DynamicValue](objectType) {
    ...
    override def decodeKey(in: JsonReader): DynamicValue = {
      val s = in.readKeyAsString()
      Json.parse(s) match {
        case Right(json) => json.toDynamicValue
  ...
  implicit val currencyCodec: JsonBinaryCodec[java.util.Currency] = new PrimitiveCodec(objectType, java.util.Currency.getInstance("USD"), (in, _) => try {
  ...
  implicit val uuidCodec: JsonBinaryCodec[java.util.UUID] = new PrimitiveCodec(objectType, new java.util.UUID(0L, 0L), (in, _) => in.readUUID(null), (x, out) => out.writeVal(x))
  
  implicit val bigIntCodec: JsonBinaryCodec[BigInt] = new PrimitiveCodec(objectType, BigInt(0), (in, _) => in.readBigInt(null), (x, out) => out.writeVal(x))
  implicit val bigDecimalCodec: JsonBinaryCodec[BigDecimal] = new PrimitiveCodec(objectType, BigDecimal(0), (in, _) => in.readBigDecimal(null), (x, out) => out.writeVal(x))
  
  implicit val dayOfWeekCodec: JsonBinaryCodec[java.time.DayOfWeek] = new PrimitiveCodec(objectType, java.time.DayOfWeek.MONDAY, (in, _) => try {
  ...
  implicit val durationCodec: JsonBinaryCodec[java.time.Duration] = new PrimitiveCodec(objectType, java.time.Duration.ZERO, (in, _) => in.readDuration(null), (x, out) => out.writeVal(x))
  implicit val instantCodec: JsonBinaryCodec[java.time.Instant] = new PrimitiveCodec(objectType, java.time.Instant.EPOCH, (in, _) => in.readInstant(null), (x, out) => out.writeVal(x))
  implicit val localDateCodec: JsonBinaryCodec[java.time.LocalDate] = new PrimitiveCodec(objectType, java.time.LocalDate.now, (in, _) => in.readLocalDate(null), (x, out) => out.writeVal(x))
  implicit val localDateTimeCodec: JsonBinaryCodec[java.time.LocalDateTime] = new PrimitiveCodec(objectType, java.time.LocalDateTime.now, (in, _) => in.readLocalDateTime(null), (x, out) => out.writeVal(x))
  implicit val localTimeCodec: JsonBinaryCodec[java.time.LocalTime] = new PrimitiveCodec(objectType, java.time.LocalTime.now, (in, _) => in.readLocalTime(null), (x, out) => out.writeVal(x))
  implicit val monthCodec: JsonBinaryCodec[java.time.Month] = new PrimitiveCodec(objectType, java.time.Month.JANUARY, (in, _) => try {
    java.time.Month.valueOf(in.readString(null))
  } catch {
    case _: IllegalArgumentException => in.decodeError("illegal month value")
  }, (x, out) => out.writeVal(x.name))
  implicit val monthDayCodec: JsonBinaryCodec[java.time.MonthDay] = new PrimitiveCodec(objectType, java.time.MonthDay.now, (in, _) => in.readMonthDay(null), (x, out) => out.writeVal(x))
  implicit val offsetDateTimeCodec: JsonBinaryCodec[java.time.OffsetDateTime] = new PrimitiveCodec(objectType, java.time.OffsetDateTime.now, (in, _) => in.readOffsetDateTime(null), (x, out) => out.writeVal(x))
  implicit val offsetTimeCodec: JsonBinaryCodec[java.time.OffsetTime] = new PrimitiveCodec(objectType, java.time.OffsetTime.now, (in, _) => in.readOffsetTime(null), (x, out) => out.writeVal(x))
  implicit val periodCodec: JsonBinaryCodec[java.time.Period] = new PrimitiveCodec(objectType, java.time.Period.ZERO, (in, _) => in.readPeriod(null), (x, out) => out.writeVal(x))
  implicit val yearCodec: JsonBinaryCodec[java.time.Year] = new PrimitiveCodec(objectType, java.time.Year.now, (in, _) => in.readYear(null), (x, out) => out.writeVal(x))
  implicit val yearMonthCodec: JsonBinaryCodec[java.time.YearMonth] = new PrimitiveCodec(objectType, java.time.YearMonth.now, (in, _) => in.readYearMonth(null), (x, out) => out.writeVal(x))
  implicit val zoneIdCodec: JsonBinaryCodec[java.time.ZoneId] = new PrimitiveCodec(objectType, java.time.ZoneId.systemDefault(), (in, _) => in.readZoneId(null), (x, out) => out.writeVal(x))
  implicit val zoneOffsetCodec: JsonBinaryCodec[java.time.ZoneOffset] = new PrimitiveCodec(objectType, java.time.ZoneOffset.UTC, (in, _) => in.readZoneOffset(null), (x, out) => out.writeVal(x))
  implicit val zonedDateTimeCodec: JsonBinaryCodec[java.time.ZonedDateTime] = new PrimitiveCodec(objectType, java.time.ZonedDateTime.now, (in, _) => in.readZonedDateTime(null), (x, out) => out.writeVal(x))
}
