 ...
 *
 * Migration wraps an untyped [[DynamicMigration]] along with the source and
 * target schemas, providing type-safe application of migrations. It supports
 * composition and reversal for building complex migration pipelines.
 *
...
final case class Migration[A, B](
   ...
   *   the input value
   * @return
   *   either a migration error or the transformed value
   */
   ...
   * @tparam C
   *   the target type of the second migration
   * @return
   *   the composed migration from A to C
   */
...
object Migration {
   ...
   *
   * This is an advanced method for creating migrations programmatically. Most
   * users should use [[newBuilder]] instead.
   */
  def fromDynamic[A, B](
    dynamicMigration: DynamicMigration,
    sourceSchema: Schema[A],
