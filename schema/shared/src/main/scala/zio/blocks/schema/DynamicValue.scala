  ...
  private[this] def modifyAtPath(
    ...
    nodes(index) match {
      ...
      case DynamicOptic.Node.SchemaSearch(schemaRepr) =>
        ...
        schemaSearchModify(dv, schemaRepr, nodes, index, f)
    }
  }
  ...
  private[this] def deleteAtPath(
    ...
    nodes(index) match {
      ...
      case DynamicOptic.Node.SchemaSearch(schemaRepr) =>
        ...
        schemaSearchDelete(dv, schemaRepr, nodes, index)
    }
  }
  ...
  private def schemaSearchModify(
    ...
    def modifyMatching(value: DynamicValue): DynamicValue = {
      ...
      val afterSelf = if (SchemaMatch.matches(pattern, value)) {
        ...
        modifyAtPath(value, nodes, idx + 1, f) match {
          case Some(modified) =>
            found = true
            modified
          case None => value
  ...
  private def schemaSearchDelete(
      ...
      value match {
        case r: Record =>
          val newFields = r.fields.map { case (name, v) =>
            val selfResult = if (SchemaMatch.matches(pattern, v)) {
              deleteAtPath(v, nodes, idx + 1) match {
                case Some(modified) =>
                  found = true
                  Some(modified)
                case None => None
        ...
        case v: Variant =>
          val selfResult = if (SchemaMatch.matches(pattern, v.value)) {
            deleteAtPath(v.value, nodes, idx + 1) match {
              case Some(modified) =>
                found = true
                Some(modified)
              case None => None
        ...
        case s: Sequence =>
          val newElems = s.elements.map { e =>
            val selfResult = if (SchemaMatch.matches(pattern, e)) {
              deleteAtPath(e, nodes, idx + 1) match {
                case Some(modified) =>
                  found = true
                  Some(modified)
                case None => None
    ...
    if (isLast) {
      ...
      if (SchemaMatch.matches(pattern, dv)) {
        ...
        deleteAtPath(dv, nodes, idx + 1) match {
