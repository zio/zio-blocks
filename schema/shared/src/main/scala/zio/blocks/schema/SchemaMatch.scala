...
/**
 * Provides matching logic for SchemaRepr patterns against DynamicValue instances.
 *
 * This object implements structural pattern matching between schema representations
 * and runtime dynamic values, enabling the search optic to find values that match
 * a given schema pattern.
 *
 * == Matching Rules ==
 *
 *   - '''Wildcard''': Matches any DynamicValue
 *   - '''Primitive(name)''': Matches DynamicValue.Primitive where the underlying
 *     PrimitiveValue type matches the name (case-insensitive)
 *   - '''Record(fields)''': Matches DynamicValue.Record using subset matching - all
 *     specified fields must exist with matching types, but extra fields are allowed
 *   - '''Variant(cases)''': Matches DynamicValue.Variant if the case name matches
 *     one of the pattern cases and the payload matches that case's pattern
 *   - '''Sequence(elem)''': Matches DynamicValue.Sequence if all elements match
 *     the element pattern (empty sequence always matches)
 *   - '''Map(k, v)''': Matches DynamicValue.Map if all entries have matching key
 *     and value types (empty map always matches)
...
object SchemaMatch {
   ...
   *
   * @param pattern the schema pattern to match against
   * @param value the dynamic value to check
  ...
  /**
   * Tests whether a PrimitiveValue's type matches a primitive type name.
  ...
  /**
   * Tests whether a DynamicValue.Record matches a Record pattern using subset matching.
   * All fields specified in the pattern must exist in the actual record with matching types.
   * The actual record may have additional fields that are not in the pattern.
   */
  ...
  /**
   * Tests whether a DynamicValue.Variant matches a Variant pattern.
   * The variant's case name must match one of the pattern's cases,
   * and the payload must match that case's pattern.
   */
  ...
  /**
   * Tests whether a DynamicValue.Sequence matches a Sequence pattern.
   * Empty sequences always match. For non-empty sequences, all elements
   * must match the element pattern.
   */
  ...
  /**
   * Tests whether a DynamicValue.Map matches a Map pattern.
   * Empty maps always match. For non-empty maps, all entries must have
   * keys matching the key pattern and values matching the value pattern.
   */
