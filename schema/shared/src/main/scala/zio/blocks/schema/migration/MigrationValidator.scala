...
object MigrationValidator {
  ...
  private def checkFieldPath(ctx: String, at: DynamicOptic): Either[MigrationError, Unit] =
    if (!endsWithField(at))
      Left(MigrationError.InvalidOp(ctx, "at must end in .field(...), got: " + at))
    else Right(())

  private def checkNonEmpty(ctx: String, n: Int, what: String): Either[MigrationError, Unit] =
    if (n <= 0) Left(MigrationError.InvalidOp(ctx, what + " cannot be empty"))
    else Right(())
  ...
  /**
   * Uses DynamicOptic.apply(reflect) (which internally consults schema
   * reflection) to check whether a path is valid for a given schema.
   */
  private def pathExists(schema: Schema[_], at: DynamicOptic): Boolean = {
    // We only need existence; erase the optic functor kind with a type lambda.
    type AnyF[_, _] = Any
  ...
  private def requirePathExists(
    ctx: String,
    schema: Schema[_],
    at: DynamicOptic
  ): Either[MigrationError, Unit] =
    if (pathExists(schema, at)) Right(())
    else Left(MigrationError.InvalidOp(ctx, "Invalid path for schema: " + at))
  ...
  private def validateAction(
    ...
    source: Schema[_],
    target: Schema[_]
  ): Either[MigrationError, Unit] = a match {
    ...
    case AddField(at, _) =>
      for {
        ...
        // _ <- if (!pathExists(source, at)) Right(()) else Left(MigrationError.InvalidOp("AddField", "Field already exists in source: " + at))
      } yield ()
    ...
    case TransformValue(at, _) =>
      // After the DSL, transform happens at the (possibly renamed) target location.
    ...
    case TransformCase(at, actions) =>
      for {
        _ <- if (containsCase(at)) Right(())
               ...
               Left(
                 MigrationError.InvalidOp("TransformCase", "at must include .when[Case] / Node.Case(...), got: " + at)
               )
