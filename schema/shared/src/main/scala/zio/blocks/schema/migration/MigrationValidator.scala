...
object MigrationValidator {
  ...
  /**
    * Uses DynamicOptic.apply(reflect) (which internally consults schema reflection)
    ...
    case TransformCase(at, actions) =>
      for {
        _ <- if (containsCase(at)) Right(())
             else Left(MigrationError.InvalidOp("TransformCase", "at must include .when[Case] / Node.Case(...), got: " + at))
