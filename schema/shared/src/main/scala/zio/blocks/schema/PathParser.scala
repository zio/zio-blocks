 ...
 * 
 * This parser converts a path string (e.g., ".users[*].email") into a 
 * Vector of DynamicOptic.Node elements. All parsing is done at compile time.
 */
private[schema] object PathParser {
  ...
  /**
   * Parse a path string into a Vector of Nodes.
   * Returns Left with error if parsing fails, Right with nodes on success.
   */
