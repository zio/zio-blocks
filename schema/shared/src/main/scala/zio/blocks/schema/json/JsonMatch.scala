 ...
 * and JSON values, enabling the search optic to find JSON values that match
 ...
 *     - "int", "long", "short", "byte", "float", "double", "number", "bigint", "bigdecimal"
 *       all match Json.Number
 ...
 *   - '''Sequence(elem)''': Matches Json.Array if all elements match the element
 *     pattern (empty array always matches)
 *   - '''Map(k, v)''': Matches Json.Object if key pattern is "string" and all
 *     values match the value pattern (empty object always matches)
 *   - '''Optional(inner)''': Matches Json.Null always, otherwise checks inner pattern
 *   - '''Nominal(name)''': Returns false (requires schema context not available in JSON)
 */
object JsonMatch {
   ...
   * @return true if the JSON matches the pattern, false otherwise
   */
  ...
  /**
   * Tests whether a Json value matches a primitive type name.
   ...
   * All fields specified in the pattern must exist in the object with matching types.
   * The object may have additional fields that are not in the pattern.
   */
  ...
  /**
   * Tests whether a Json value matches any of the Variant cases.
   * Since JSON doesn't have tagged variants, we try to match the value
   * against any of the case patterns.
   */
  ...
  /**
   * Tests whether a Json.Array matches a Sequence pattern.
   * Empty arrays always match. For non-empty arrays, all elements
  ...
  /**
   * Tests whether a Json.Object matches a Map pattern.
   * JSON objects have string keys, so we check if the key pattern matches "string"
   * and all values match the value pattern.
   * Empty objects always match.
   */
