 ...
 * Basic schema validation support provided for completeness.
 */
...
sealed trait Json { self =>
  ...
  private def deleteRec(json: Json, nodes: List[DynamicOptic.Node]): Json = {
    import DynamicOptic.Node
    nodes match {
      case Nil => Json.Null // Deleting self replaces with Null? Or "removes"? In recursive context, removal is handled by parent.
  ...
  def normalize: Json = self match {
    case Json.Object(fields) =>
      Json.Object(
        fields
          .filterNot { case (_, v) => v == Json.Null }
          .sortBy(_._1)
  ...
  /**
   * Removes all null values from this JSON, recursively.
   * Empty objects and arrays after null removal are also removed.
   */
  def dropNulls: Json = self match {
    case Json.Null => Json.Null
    case Json.Object(fields) =>
      val filtered = fields.filterNot(_._2 == Json.Null).map { case (k, v) => (k, v.dropNulls) }.filter(_._2 != Json.Null)
  ...
  private def modifyRecOrFail(json: Json, nodes: List[DynamicOptic.Node], f: PartialFunction[Json, Json], path: DynamicOptic): Either[JsonError, Json] = {
    import DynamicOptic.Node
    nodes match {
      case Nil =>
      ...
      case head :: tail =>
        head match {
          ...
          case Node.Elements =>
             json match {
               case Json.Array(elements) =>
                 elements.foldLeft[Either[JsonError, Vector[Json]]](Right(Vector.empty)) { 
  ...
  private def deleteRecOrFail(json: Json, nodes: List[DynamicOptic.Node], path: DynamicOptic): Either[JsonError, Json] = {
    import DynamicOptic.Node
    nodes match {
      ...
      // Recursive step
       head match {
          case Node.Field(name) =>
            json match {
              case Json.Object(fields) =>
                val existing = fields.find(_._1 == name).map(_._2).getOrElse(Json.Object.empty) // Assume object for path creation?
                val updated = insertRec(existing, tail.head, tail.tail, value)
                json.asInstanceOf[Json.Object].copy(fields = fields.map {
  ...
  def project(paths: DynamicOptic*): Json = {
    ...
    paths.foldLeft[Json](roots) { (acc, path) =>
       get(path).toEither.fold(_ => acc, jsons => {
  ...
  private[json] def decodeWith[A](codec: JsonBinaryCodec[A]): Either[JsonError, A] = {
    ...
    try {
      ...
      codec.decode(buffer).left.map(schemaError =>
        JsonError(schemaError.message, DynamicOptic.root, None, None, None)
      )
    } catch {
...
object Json {
   ...
   */
  def decode(bytes: scala.Array[Byte]): Either[JsonError, Json] = JsonParser.parseString(new java.lang.String(bytes, "UTF-8"))
   ...
   */
  def decode(chunk: Chunk[Byte]): Either[JsonError, Json] = JsonParser.parseString(new java.lang.String(chunk.toArray, "UTF-8"))
  ...
  def fromJsonPatch(patch: JsonPatch): Json = {
    val opObj: Json = patch match {
      case JsonPatch.Add(path, value) => Json.Object(Vector("op" -> Json.String("add"), "path" -> Json.String(path), "value" -> value))
      case JsonPatch.Remove(path) => Json.Object(Vector("op" -> Json.String("remove"), "path" -> Json.String(path)))
      case JsonPatch.Replace(path, value) => Json.Object(Vector("op" -> Json.String("replace"), "path" -> Json.String(path), "value" -> value))
      case JsonPatch.Move(from, path) => Json.Object(Vector("op" -> Json.String("move"), "from" -> Json.String(from), "path" -> Json.String(path)))
      case JsonPatch.Copy(from, path) => Json.Object(Vector("op" -> Json.String("copy"), "from" -> Json.String(from), "path" -> Json.String(path)))
      case JsonPatch.Test(path, value) => Json.Object(Vector("op" -> Json.String("test"), "path" -> Json.String(path), "value" -> value))
  ...
  def toJsonPatch(json: Json): Either[JsonError, JsonPatch] = json match {
    case Json.Object(fields) =>
      ...
      (op, path) match {
        case (Some("add"), Some(p)) => fieldMap.get("value").toRight(JsonError("missing value")).map(v => JsonPatch.Add(p, v))
        case (Some("remove"), Some(p)) => Right(JsonPatch.Remove(p))
        case (Some("replace"), Some(p)) => fieldMap.get("value").toRight(JsonError("missing value")).map(v => JsonPatch.Replace(p, v))
        case (Some("move"), Some(p)) => fieldMap.get("from").collect { case Json.String(f) => f }.toRight(JsonError("missing from")).map(f => JsonPatch.Move(f, p))
        case (Some("copy"), Some(p)) => fieldMap.get("from").collect { case Json.String(f) => f }.toRight(JsonError("missing from")).map(f => JsonPatch.Copy(f, p))
        case (Some("test"), Some(p)) => fieldMap.get("value").toRight(JsonError("missing value")).map(v => JsonPatch.Test(p, v))
  ...
  def parsePath(path: java.lang.String): DynamicOptic = {
        val nodesVector = path.split("(?=[.\\[\\]])").filter(s => s.nonEmpty && s != "." && s != "]").flatMap { token =>
           if (token.startsWith("[")) {
