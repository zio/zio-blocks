...
object Optional {
  ...
  private case class OptionalImpl[F[_, _], S, A](leafs: ArraySeq[Leaf[F, _, _]]) extends Optional[F, S, A] {
    ...
    def set(s: S, a: A)(implicit F: HasBinding[F]): S = {
      var i = leafs.length
      var g: Any => Any =
        if (leafs(i - 1).isInstanceOf[Prism[F, _, _]]) { // relaying on properly merged sequencial prims and lens
          val lensM2 = leafs(i - 2).asInstanceOf[Lens[F, Any, Any]]
          (x: Any) => lensM2.set(x, a)
        } else {
          val lensM1 = leafs(i - 1).asInstanceOf[Lens[F, Any, Any]]
          val leafM2 = leafs(i - 2).asInstanceOf[Leaf[F, Any, Any]]
          (x: Any) => leafM2.modify(x, y => lensM1.set(y, a))
        }
      i -= 2
      while (i > 0) {
        i -= 1
    ...
    def setOption(s: S, a: A)(implicit F: HasBinding[F]): Option[S] = {
      val s1 = set(s, a)
      if (s1 != s) new Some(s1) // FIXME: implement properly without equality check
      else None
    }
