  ...
  def deriveImpl[A: Type, B: Type](dynamic: Expr[DynamicMigration])(using Quotes): Expr[Migration[A, B]] = {
    ...
    def extractOps(expr: Expr[DynamicMigration]): List[MacroOp] = {
      expr match {
        ...
        // Note: We need to handle the case where the expression is a reference to a val, checking for literal support.
        // For simplicity in this macro version, we match literal constructions.
    ...
    if (!isCaseClass) {
      ...
      '{ Migration.manual(${dynamic}) }
        ...
        // For this MVP, we assume names match/rename correctly and types are compatible.
        ...
        if (!isValid) {
          ...
          val lambda = Lambda(Symbol.spliceOwner, methodType, (_, params) => {
