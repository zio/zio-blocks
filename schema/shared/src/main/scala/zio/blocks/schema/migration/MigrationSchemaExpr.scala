 ...
 * Keeps migration code in the migration package and avoids changing core SchemaExpr too much.
 */
object MigrationSchemaExpr {
   ...
   *
   * We represent it as a SchemaExpr that *cannot be evaluated without a schema*.
   * The migration interpreter will special-case it.
   */
  final case class DefaultValue[S, A](schema: Schema[A]) extends SchemaExpr[S, A] {
    override def eval(input: S) =
      Left(new zio.blocks.schema.OpticCheck("DefaultValue: requires schema default, not runtime input"))
