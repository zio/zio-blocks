    ...
    extends BinaryFormat(
      ...
      new Deriver[JsonBinaryCodec] {
            ...
            if (variant.variantBinding.isInstanceOf[Binding[?, ?]]) {
              option(variant) match {
                    ...
                    override def decodeValue(in: JsonReader, default: Option[Any]): Option[Any] =
                      if (in.nextToken() == 'n') {
                        try in.readNullOrError(default, "expected null")
                          ...
                          case error if NonFatal(error) => decodeError(new DynamicOptic.Node.Case("None"), error)
                        }
                      } else {
                        in.rollbackToken()
                        try new Some(codec.decodeValue(in, codec.nullValue))
                          ...
                          case error if NonFatal(error) =>
                            decodeError(new DynamicOptic.Node.Case("Some"), new DynamicOptic.Node.Field("value"), error)
                        }
                      }
                ...
                case _ =>
                  ...
                  var complexity = 0
                  while (idx < len) {
                    val name = cases(idx).name
                    names(idx) = name
                    complexity += name.length
                    idx += 1
                  }
                  if (len <= 8 && complexity <= 64) {
                    enumeration(variant) match {
                      case Some(constructors) =>
                        new JsonBinaryCodec[A]() {
                          private[this] val discriminator    = binding.discriminator
                          private[this] val variantNames     = names
                          private[this] val caseConstructors = constructors

                          def decodeValue(in: JsonReader, default: A): A = {
                            val valueLen = in.readStringAsCharBuf()
                            val len      = variantNames.length
                            var idx      = 0
                            while (idx < len) {
                              val name = variantNames(idx)
                              if (in.isCharBufEqualsTo(valueLen, name)) {
                                return caseConstructors(idx).construct(null, 0).asInstanceOf[A]
                              }
                              idx += 1
                            }
                          ...
                          def encodeValue(x: A, out: JsonWriter): Unit =
                            out.writeVal(variantNames(discriminator.discriminate(x)))
                        }
                      case _ =>
                      ...
                      case Some(constructors) =>
                        new JsonBinaryCodec[A]() {
                          ...
                          def decodeValue(in: JsonReader, default: A): A = {
                            ...
                            if (idx >= 0) caseConstructors(idx).construct(null, 0).asInstanceOf[A]
                                ...
                                if (idx >= 0) {
                                  ...
                                  val x     =
                                    try codec.decodeValue(in, codec.nullValue)
                                      ...
                                      case error if NonFatal(error) =>
                                        decodeError(new DynamicOptic.Node.Case(variantNames(idx)), error)
                                    }
                                  if (!in.isNextToken('}')) in.objectEndOrCommaError()
                                  return x
                                }
        ...
        private[this] def option[F[_, _], A](variant: Reflect.Variant[F, A]): Option[Reflect[F, ?]] = {
          ...
          val optionTypeName = Schema.optionUnit.reflect.typeName
          if (
            typeName.namespace == optionTypeName.namespace &&
            typeName.name == optionTypeName.name &&
            variant.cases.length == 2 &&
            variant.cases(1).name == "Some"
          ) variant.cases(1).value.asRecord.map(_.fields(0).value)
          else None
        }
        ...
        private[this] val dynamicValueCodec = new JsonBinaryCodec[DynamicValue]() {
          def decodeValue(in: JsonReader, default: DynamicValue): DynamicValue =
            in.decodeError("decoding as a JSON value is not supported")

          def encodeValue(x: DynamicValue, out: JsonWriter): Unit =
            out.encodeError("encoding as a JSON value is not supported")
        }
      }
    )
