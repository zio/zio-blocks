...
/**
 * Represents pure, serializable expressions for value transformations in migrations.
 * 
 ...
 * 
 * For this ticket, we only support primitive-to-primitive transformations.
 * No record/enum construction is allowed.
 */
sealed trait SchemaExpr {
  /**
   * Apply this expression to a DynamicValue context.
...
object SchemaExpr {
  ...
  /**
   * Special marker that uses the schema's default value.
   * The macro captures the field schema and calls schema.defaultValue.
   */
   ...
   * For example: Int -> String, String -> Int, etc.
   */
