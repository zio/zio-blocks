    ...
    action match {
      case AddField(at, default) =>
        DynamicSchemaExpr.eval(default, value).flatMap { defaultValue =>
          DynamicValue.insertAtPathOrFail(value, at, defaultValue).left.map(e => EvaluationError(at, e.message))
        }

      case DropField(at, _) =>
        DynamicValue.deleteAtPathOrFail(value, at).left.map(e => EvaluationError(at, e.message))
