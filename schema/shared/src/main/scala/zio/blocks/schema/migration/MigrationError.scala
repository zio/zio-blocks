...
object MigrationError {
  ...
  final case class UnknownEnumCase(path: DynamicOptic, got: String) extends MigrationError {
    val message: String = s"Unknown enum case at $path: $got"
  }

  final case class OpticCheckFailed(check: zio.blocks.schema.OpticCheck) extends MigrationError {
    override def message: String = s"SchemaExpr optic check failed: $check"
  }

  final case class ExpectedRecord(path: DynamicOptic, got: Any) extends MigrationError {
    val message: String =
