 ...
 * in some cases they should be altered for performance reasons: <li>increase
 * preferred size of an internal byte buffer for parsing from
 * [[java.io.InputStream]] or [[java.nio.DirectByteBuffer]] to reduce allocation
 * rate of grown and then reduced buffers during parsing of large (>16Kb)
 ...
 * rate of grown and then reduced buffers when large (>4Kb) string instances
 * need to be parsed including those one which use Base16 or Base64
 * encodings</li> </ul>
 * @param maxBufSize
