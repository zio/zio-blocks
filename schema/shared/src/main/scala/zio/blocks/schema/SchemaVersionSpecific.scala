        ...
        tpe, {
          ...
          Implicits.search(schemaTpeApplied) match {
            ...
            case _                        =>
              ...
              // Adding schema reference before derivation to prevent endless recursion on recursive data structures
          ...
          tpeParams.map(_.map { symbol =>
            ...
            val defaultValue = if (symbol.flags.is(Flags.HasDefault)) {
              if (companionRefAndClass eq null) {
                val tpeTypeSymbol = tpeTypeSymbol
              ...
              val dvSelect        = dvMethod.paramSymss match {
