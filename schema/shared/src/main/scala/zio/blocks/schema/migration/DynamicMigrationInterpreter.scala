...
object DynamicMigrationInterpreter {
  ...
  def apply(
    ...
    m.actions.foldLeft[Either[MigrationError, DynamicValue]](Right(value)) { (acc, action) =>
  ...
  private def applyAction(
    ...
    action match {
      ...
      case AddField(at, defaultExpr) =>
        evalExprToOneDynamic(expr = defaultExpr, input = root)
      ...
      case Join(at, sourcePaths, combiner) =>
        for {
          ...
          inputRecord =
            DynamicValue.Record(
              Chunk.from(
                inputs.zipWithIndex.map { case (v, i) => (s"_$i", v) }
              )
            )
        ...
        modifyAt(at, root) {
          case DynamicValue.Sequence(values) =>
            values
              ...
              .map(xs => DynamicValue.Sequence(Chunk.from(xs)))
        ...
        modifyAt(at, root) {
          case DynamicValue.Map(entries) =>
            ...
            entries
              ...
              .map(kvs => DynamicValue.Map(Chunk.from(kvs)))
  ...
  private def evalExprToOneDynamic(
    ...
    expr match {
      ...
      case SchemaExpr.DefaultValueFromSchema(schema0) =>
        schema0.getDefaultValue match {
          case Some(value) =>
            Right(schema0.toDynamicValue(value))
          case None =>
            Left(
              MigrationError.InvalidOp(
                "DefaultValueFromSchema",
                "Schema has no default value"
              )
            )
        }
            ...
            cur match {
              case DynamicValue.Record(fields) =>
                ...
                if (idx < 0) Left(MigrationError.MissingPath(at))
                  ...
                  val (k, v) = fields(idx)
                  loop(i + 1, v).map(v2 => DynamicValue.Record(fields.updated(idx, (k, v2))))
                }
            ...
            cur match {
              case DynamicValue.Map(entries) =>
                entries
                  ...
                  .map(kvs => DynamicValue.Map(zio.blocks.chunk.Chunk.from(kvs)))
