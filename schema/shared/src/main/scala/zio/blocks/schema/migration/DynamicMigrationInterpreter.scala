...
import zio.blocks.schema.{DynamicOptic, DynamicValue, OpticCheck, Schema, SchemaExpr}
...
object DynamicMigrationInterpreter {
    ...
    action match {
      ...
      case TransformValue(at, expr) =>
        modifyAt(at, root) { cur =>
          evalExprToOneDynamic(expr = expr, input = cur, atForDefault = Some(at), targetSchema = targetSchema)
        }

      case ChangeType(at, converter) =>
        modifyAt(at, root) { cur =>
          if (!isPrimitive(cur))
            Left(MigrationError.InvalidOp("ChangeType", s"Only primitive->primitive supported, but got: $cur at $at"))
          else
            evalExprToOneDynamic(expr = converter, input = cur, atForDefault = Some(at), targetSchema = targetSchema).flatMap { out =>
              if (!isPrimitive(out))
                Left(MigrationError.InvalidOp("ChangeType", s"Converter must produce a primitive, but got: $out at $at"))
              else Right(out)
            }
        }
      ...
      case Join(at, sourcePaths, combiner) =>
        for {
          inputs <- sourcePaths.foldLeft[Either[MigrationError, Vector[DynamicValue]]](Right(Vector.empty)) {
            case (acc, p) =>
              for {
                xs <- acc
                v  <- getAt(p, root)
                _  <- if (isPrimitive(v)) Right(()) else Left(MigrationError.InvalidOp("Join", s"Join inputs must be primitive; got $v at $p"))
              } yield xs :+ v
          }

          // convention: we pass inputs to combiner as a record: _0, _1, ...
          inputRecord = DynamicValue.Record(inputs.zipWithIndex.map { case (v, i) => (s"_$i", v) })

          out <- evalExprToOneDynamic(expr = combiner, input = inputRecord, atForDefault = Some(at), targetSchema = targetSchema)
          _   <- if (isPrimitive(out)) Right(()) else Left(MigrationError.InvalidOp("Join", s"Join output must be primitive; got $out at $at"))
      ...
      case Split(at, targetPaths, splitter) =>
        for {
          ...
          _ <- if (isPrimitive(original)) Right(())
               else Left(MigrationError.InvalidOp("Split", s"Split input must be primitive; got $original at $at"))

          produced <- evalExprToOneDynamic(expr = splitter, input = original, atForDefault = Some(at), targetSchema = targetSchema)
          ...
          producedRecord <- produced match {
            ...
            case other =>
              Left(MigrationError.InvalidOp("Split", s"Splitter must return a record, but got: $other"))
          }

          // remove original field (split replaces it)
          withoutOriginal <- removeField(at, root)

          withTargets <- targetPaths.foldLeft[Either[MigrationError, DynamicValue]](Right(withoutOriginal)) {
            case (acc, targetPath) =>
              for {
                curRoot <- acc
                fieldName <- targetPath.nodes.lastOption match {
                  ...
                  case other =>
                    Left(MigrationError.InvalidOp("Split", s"targetPaths must end in .field(...), got: $other"))
                }
                piece <- producedRecord.get(fieldName) match {
                  case Some(v) => Right(v)
                  case None =>
                    Left(MigrationError.InvalidOp("Split", s"Splitter did not produce field '$fieldName' required by target path: $targetPath"))
                }
                _ <- if (isPrimitive(piece)) Right(()) else Left(MigrationError.InvalidOp("Split", s"Split outputs must be primitive; got $piece for $fieldName"))
      ...
      case RenameCase(at, from, to) =>
        modifyAt(at, root) {
          ...
          case DynamicValue.Variant(other, payload) => Right(DynamicValue.Variant(other, payload)) // no-op for other cases
          case other =>
          ...
          case DynamicValue.Sequence(values) =>
            values.foldLeft[Either[MigrationError, Vector[DynamicValue]]](Right(Vector.empty)) {
              case (acc, elem) =>
                for {
                  xs  <- acc
                  out <- evalExprToOneDynamic(expr = expr, input = elem, atForDefault = Some(at), targetSchema = targetSchema)
                } yield xs :+ out
  ...
  private def evalExprToOneDynamic(
      ...
      input: DynamicValue,
      atForDefault: Option[DynamicOptic],
      targetSchema: Schema[_]
  ): Either[MigrationError, DynamicValue] =
    expr match {
      case _: MigrationSchemaExpr.DefaultValue[?, ?] =>
        atForDefault match {
          case Some(at) => resolveDefaultAt(at, targetSchema)
          case None     => Left(MigrationError.InvalidOp("DefaultValue", "Missing atForDefault path"))
        }
  ...
  private def resolveDefaultAt(
      at: DynamicOptic,
      targetSchema: Schema[_]
  ): Either[MigrationError, DynamicValue] = {
    val anySchema = targetSchema.asInstanceOf[Schema[Any]]
    anySchema.defaultValue match {
      case None =>
        Left(MigrationError.InvalidOp("DefaultValue", s"Target schema has no defaultValue; cannot fill missing value at $at"))

      case Some(targetDefault) =>
        val dynDefault = anySchema.toDynamicValue(targetDefault)
        getAt(at, dynDefault).left.map {
          case e => MigrationError.InvalidOp("DefaultValue", s"Could not focus default at $at: ${e.message}")
        }
    }
  }
  ...
  private def getAt(at: DynamicOptic, root: DynamicValue): Either[MigrationError, DynamicValue] = {
    ...
    def loop(i: Int, cur: DynamicValue): Either[MigrationError, DynamicValue] =
      if (i >= nodes.length) Right(cur)
        ...
        case DynamicOptic.Node.Elements =>
          ...
          Left(MigrationError.InvalidOp("DynamicOptic", s"Traversal (.each) not supported for DefaultValue focus: $at"))

        case DynamicOptic.Node.MapKeys | DynamicOptic.Node.MapValues =>
          Left(MigrationError.InvalidOp("DynamicOptic", s"Map traversal not supported for DefaultValue focus: $at"))
        ...
        case other =>
          Left(MigrationError.InvalidOp("DynamicOptic", s"Unsupported optic node in getAt: $other"))
      }
    ...
    def loop(i: Int, cur: DynamicValue): Either[MigrationError, DynamicValue] =
      if (i >= nodes.length) f(cur)
        ...
        nodes(i) match {
          ...
          case other =>
            Left(MigrationError.InvalidOp("DynamicOptic", s"Unsupported optic node in modifyAt: $other"))
        }
