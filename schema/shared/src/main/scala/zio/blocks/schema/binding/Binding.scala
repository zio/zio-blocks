...
object Binding {
  ...
  object Record {
    ...
    def left[A <: AnyRef, B]: Record[Left[A, B]] = new Record(
    ...
    def right[A, B <: AnyRef]: Record[Right[A, B]] = new Record(
      constructor = new Constructor[Right[A, B]] {
        def usedRegisters: RegisterOffset = RegisterOffset(objects = 1)

        def construct(in: Registers, offset: RegisterOffset): Right[A, B] = new Right(
          in.getObject(offset).asInstanceOf[B]
        )
      },
  ...
  object Variant {
    ...
    def either[A, B]: Variant[Either[A, B]] = new Variant(
      discriminator = new Discriminator[Either[A, B]] {
        def discriminate(a: Either[A, B]): Int =
          if (a.isLeft) 0 else 1
      },
      matchers = Matchers(
