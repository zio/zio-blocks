...
/**
 * Comprehensive primitive type conversion utilities for schema migrations.
 *
 * This module provides predefined conversions between primitive types, making
 * common type migrations simple while maintaining full serializability. All
 * conversions are statically defined lookup tables with no user-provided
 * functions.
 *
 * Supported conversion categories:
 *   - Numeric widening: Byte -> Short -> Int -> Long -> Float -> Double ->
 *     BigDecimal
 *   - Numeric narrowing: With bounds checking (Long -> Int may fail)
 *   - String parsing: All primitive types from/to String
 *   - Temporal types: String -> Instant, LocalDate, etc.
 *   - Boolean/Char: To/from numeric representations
 *
 * Example usage in migrations:
 * {{{
 * import PrimitiveConversions._
 *
 * // Widen Int to Long (always succeeds)
 * val result = convert(dynamicInt(42), "Int", "Long")
 * // => Right(DynamicValue.Primitive(PrimitiveValue.Long(42L)))
 *
 * // Parse String to Int (may fail)
 * val parsed = convert(dynamicString("123"), "String", "Int")
 * // => Right(DynamicValue.Primitive(PrimitiveValue.Int(123)))
 *
 * // Use with ChangeType action
 * val action = MigrationAction.ChangeType(
 *   at = DynamicOptic.root,
 *   fieldName = "count",
 *   converter = Resolved.Convert("Int", "Long"),
 *   reverseConverter = Resolved.Convert("Long", "Int")
 * )
 * }}}
 */
object PrimitiveConversions {
   ...
   *
   * @param value
   *   The DynamicValue to convert (must be Primitive)
   * @param fromType
  ...
  def convert(
    ...
    (fromType, toType, value) match {
      ...
      case ("Short", "Byte", DynamicValue.Primitive(PrimitiveValue.Short(s))) =>
        ...
        else Left(s"Value $s out of Byte range [-128, 127]")
      ...
      case ("Int", "Byte", DynamicValue.Primitive(PrimitiveValue.Int(i))) =>
        ...
        else Left(s"Value $i out of Byte range [-128, 127]")
      case ("Int", "Short", DynamicValue.Primitive(PrimitiveValue.Int(i))) =>
        ...
        else Left(s"Value $i out of Short range [-32768, 32767]")
      ...
      case ("Long", "Byte", DynamicValue.Primitive(PrimitiveValue.Long(l))) =>
        ...
        else Left(s"Value $l out of Byte range [-128, 127]")
      case ("Long", "Short", DynamicValue.Primitive(PrimitiveValue.Long(l))) =>
        ...
        else Left(s"Value $l out of Short range [-32768, 32767]")
      ...
      case ("String", "BigInt", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.BigInt(BigInt(s.trim))))
      ...
      case ("String", "BigDecimal", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.BigDecimal(BigDecimal(s.trim))))
      ...
      case ("String", "Char", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        ...
        else Left(s"Cannot convert string of length ${s.length} to Char (must be exactly 1)")

      // ─────────────────────────────────────────────────────────────────────
      // String -> UUID
      // ─────────────────────────────────────────────────────────────────────
      case ("String", "UUID", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.UUID(java.util.UUID.fromString(s.trim))))
      ...
      case ("String", "Instant", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.Instant(java.time.Instant.parse(s.trim))))
      ...
      case ("String", "LocalDate", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.LocalDate(java.time.LocalDate.parse(s.trim))))
      ...
      case ("String", "LocalTime", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.LocalTime(java.time.LocalTime.parse(s.trim))))
      ...
      case ("String", "LocalDateTime", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.LocalDateTime(java.time.LocalDateTime.parse(s.trim))))
      ...
      case ("String", "OffsetDateTime", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.OffsetDateTime(java.time.OffsetDateTime.parse(s.trim))))
      ...
      case ("String", "OffsetTime", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.OffsetTime(java.time.OffsetTime.parse(s.trim))))
      ...
      case ("String", "ZonedDateTime", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.ZonedDateTime(java.time.ZonedDateTime.parse(s.trim))))
      ...
      case ("String", "Duration", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.Duration(java.time.Duration.parse(s.trim))))
      ...
      case ("String", "Period", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.Period(java.time.Period.parse(s.trim))))
      ...
      case ("String", "Year", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.Year(java.time.Year.parse(s.trim))))
      ...
      case ("String", "YearMonth", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.YearMonth(java.time.YearMonth.parse(s.trim))))
      ...
      case ("String", "MonthDay", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.MonthDay(java.time.MonthDay.parse(s.trim))))
      ...
      case ("String", "DayOfWeek", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.DayOfWeek(java.time.DayOfWeek.valueOf(s.trim.toUpperCase))))
      ...
      case ("String", "Month", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.Month(java.time.Month.valueOf(s.trim.toUpperCase))))
      ...
      case ("String", "ZoneId", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.ZoneId(java.time.ZoneId.of(s.trim))))
      ...
      case ("String", "ZoneOffset", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.ZoneOffset(java.time.ZoneOffset.of(s.trim))))
      ...
      case ("String", "Currency", DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        try Right(DynamicValue.Primitive(PrimitiveValue.Currency(java.util.Currency.getInstance(s.trim))))
        catch {
      ...
      case ("Int", "Char", DynamicValue.Primitive(PrimitiveValue.Int(i))) =>
        ...
        else Left(s"Value $i out of Char range [0, 65535]")
  ...
  /**
   * Check if a widening conversion is available (always succeeds without data
   * loss).
   */
  def isWideningConversion(fromType: String, toType: String): Boolean = {
    val wideningPaths = Map(
      "Byte"   -> Set("Short", "Int", "Long", "Float", "Double", "BigInt", "BigDecimal"),
      "Short"  -> Set("Int", "Long", "Float", "Double", "BigInt", "BigDecimal"),
      "Int"    -> Set("Long", "Float", "Double", "BigInt", "BigDecimal"),
      "Long"   -> Set("Float", "Double", "BigInt", "BigDecimal"),
      "Float"  -> Set("Double", "BigDecimal"),
      "Double" -> Set("BigDecimal"),
      "BigInt" -> Set("BigDecimal")
    )
    wideningPaths.get(fromType).exists(_.contains(toType))
  }

  /**
   * Check if a narrowing conversion is available (may fail for out-of-range
   * values).
   */
  def isNarrowingConversion(fromType: String, toType: String): Boolean =
    isWideningConversion(toType, fromType)

  /**
   * Get all supported target types for a given source type.
   */
  def supportedConversionsFrom(fromType: String): Set[String] = fromType match {
    case "Byte"       => Set("Short", "Int", "Long", "Float", "Double", "BigInt", "BigDecimal", "String")
    case "Short"      => Set("Byte", "Int", "Long", "Float", "Double", "BigInt", "BigDecimal", "String")
    case "Int"        => Set("Byte", "Short", "Long", "Float", "Double", "BigInt", "BigDecimal", "String", "Char", "Boolean")
    case "Long"       => Set("Byte", "Short", "Int", "Float", "Double", "BigInt", "BigDecimal", "String")
    case "Float"      => Set("Int", "Long", "Double", "BigDecimal", "String")
    case "Double"     => Set("Float", "Int", "Long", "BigDecimal", "String")
    case "BigInt"     => Set("Int", "Long", "BigDecimal", "String")
    case "BigDecimal" => Set("Int", "Long", "Double", "BigInt", "String")
    case "Char"       => Set("Int", "String")
    case "Boolean"    => Set("Int", "String")
    case "String"     =>
      Set(
        "Byte",
        "Short",
    ...
    case _ => Set("String") // Everything can convert to String
  }
