...
/** A {{SchemaExpr}} is an expression on the value of a type fully described by
  * a {{Schema}}.
  *
...
object SchemaExpr {
  final case class Literal[S, A](value: A, schema: Schema[A])
    ...
    private[this] val dynamicResult = new Right(
      schema.toDynamicValue(value) :: Nil
    )
  }

  final case class Optic[A, B](optic: zio.blocks.schema.Optic[A, B])
    ...
    def evalDynamic(input: A): Either[OpticCheck, Seq[DynamicValue]] =
      optic match {
        ...
        case p: Prism[?, ?] =>
          p.getOrFail(input) match {
            case Right(x: B @scala.unchecked) =>
              new Right(toDynamicValue(x) :: Nil)
            case left =>
              left.asInstanceOf[Either[OpticCheck, Seq[DynamicValue]]]
          }
        case o: Optional[?, ?] =>
          o.getOrFail(input) match {
    ...
    private[this] val toDynamicValue: B => DynamicValue =
      optic.focus.toDynamicValue
  }
  ...
  final case class Relational[A, B](
      left: SchemaExpr[A, B],
    ...
    def eval(input: A): Either[OpticCheck, Seq[Boolean]] =
      if (
        operator == RelationalOperator.Equal || operator == RelationalOperator.NotEqual
      ) {
        for {
          xs <- left.eval(input)
          ys <- right.eval(input)
        ...
        for {
          ...
          if (operator == RelationalOperator.LessThan)
            for { x <- xs; y <- ys } yield x < y
          else if (operator == RelationalOperator.LessThanOrEqual)
            for { x <- xs; y <- ys } yield x <= y
          else if (operator == RelationalOperator.GreaterThan)
            for { x <- xs; y <- ys } yield x > y
          else for { x <- xs; y <- ys } yield x >= y
        }
      }
    ...
    def evalDynamic(input: A): Either[OpticCheck, Seq[DynamicValue]] =
      for {
        ...
        case RelationalOperator.LessThan =>
          for { x <- xs; y <- ys } yield toDynamicValue(x < y)
  ...
  final case class Not[A](expr: SchemaExpr[A, Boolean])
      extends UnaryOp[A, Boolean] {
    def eval(input: A): Either[OpticCheck, Seq[Boolean]] =
      for {
        xs <- expr.eval(input)
      } yield xs.map(!_)
  ...
  final case class Arithmetic[S, A](
      ...
      for {
        ...
        operator match {
          case ArithmeticOperator.Add =>
            for { x <- xs; y <- ys } yield n.plus(x, y)
          case ArithmeticOperator.Subtract =>
            for { x <- xs; y <- ys } yield n.minus(x, y)
          case ArithmeticOperator.Multiply =>
            for { x <- xs; y <- ys } yield n.times(x, y)
        }
      }
    ...
    def evalDynamic(input: S): Either[OpticCheck, Seq[DynamicValue]] =
      for {
        ...
        operator match {
          case ArithmeticOperator.Add =>
            for { x <- xs; y <- ys } yield toDynamicValue(n.plus(x, y))
          case ArithmeticOperator.Subtract =>
            for { x <- xs; y <- ys } yield toDynamicValue(n.minus(x, y))
          case ArithmeticOperator.Multiply =>
            for { x <- xs; y <- ys } yield toDynamicValue(n.times(x, y))
        }
      }

    private[this] val toDynamicValue: A => DynamicValue =
      isNumeric.primitiveType.toDynamicValue
  }
  ...
  final case class StringConcat[A](
  ...
  final case class StringRegexMatch[A](
  ...
  final case class StringLength[A](string: SchemaExpr[A, String])
...
// --- DefaultValue support (used by schema migrations; see issue #519) ---

  import scala.reflect.Selectable.reflectiveSelectable

  /** Convert a string message into an [[OpticCheck]].
    *
    * The shape of `OpticCheck` is still evolving in `zio-blocks`, so we build
    * it via a tiny reflective call that matches the common companion
    * `apply(String)` constructor.
    */
  private def defaultValueCheck(msg: String): OpticCheck =
    OpticCheck
      .asInstanceOf[{ def apply(message: String): OpticCheck }]
      .apply(msg)

  /** Marker the user can write in the DSL. MUST be replaced by macros with
    * DefaultValueFromSchema(fieldSchema).
    */
  case object DefaultValueMarker extends SchemaExpr[Any, Nothing] {
    def eval(input: Any): Either[OpticCheck, Seq[Nothing]] =
      throw new IllegalStateException(
        "SchemaExpr.DefaultValue is a marker and must be expanded by migration macros"
      )
  ...
  /** The real default-value expression required by #519: stores the
    * macro-captured field schema, calls schema.defaultValue, and converts it to
    * DynamicValue.
    */
  final case class DefaultValueFromSchema[A](schema: Schema[A])
      extends SchemaExpr[Any, A] {
    def eval(input: Any): Either[OpticCheck, Seq[A]] =
      schemaDefaultValue(schema) match {
        case Right(a)  => Right(a :: Nil)
        case Left(msg) => Left(defaultValueCheck(msg))
      }

    def evalDynamic(input: Any): Either[OpticCheck, Seq[DynamicValue]] =
      schemaDefaultValue(schema) match {
        case Right(a)  => Right(schema.toDynamicValue(a) :: Nil)
        case Left(msg) => Left(defaultValueCheck(msg))
      }
  }
