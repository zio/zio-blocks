...
object SchemaExpr {
  final case class DefaultValue[S, A](schema: Schema[A]) extends SchemaExpr[S, A] {
    def eval(input: S): Either[OpticCheck, Seq[A]] = schema.getDefaultValue match {
      ...
      case None    => Left(OpticCheck(::(OpticCheck.WrappingError(DynamicOptic.root, DynamicOptic.root, SchemaError.message("No default value available for schema", DynamicOptic.root)), Nil)))
    }
