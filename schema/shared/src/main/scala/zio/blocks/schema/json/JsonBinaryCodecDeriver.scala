...
class JsonBinaryCodecDeriver private[json] (
  ...
  override def deriveDynamic[F[_, _]](
    ...
    Lazy(
      deriveCodec(
        new Reflect.Dynamic(
          binding,
          TypeId.nominal("DynamicValue", Owner.parse("zio.blocks.schema"), Nil, TypeDefKind.Enum(Nil)),
          doc,
          modifiers
        )
      )
  ...
  private[this] def option[F[_, _], A](variant: Reflect.Variant[F, A]): Option[Reflect[F, ?]] = {
    ...
    val cases  = variant.cases
    if (
      typeId.owner.asString == "scala" && typeId.name == "Option" &&
  ...
  private[this] def isTuple[F[_, _], A](reflect: Reflect[F, A]): Boolean = reflect.isRecord && {
    val typeId = reflect.typeId
    typeId.owner.asString == "scala" && typeId.name.startsWith("Tuple")
  }
}
