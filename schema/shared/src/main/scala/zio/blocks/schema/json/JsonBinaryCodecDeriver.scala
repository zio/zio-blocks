...
class JsonBinaryCodecDeriver private[json] (
  ...
  override def deriveDynamic[F[_, _]](
    ...
    Lazy(deriveCodec(new Reflect.Dynamic(binding, TypeId.derive[DynamicValue], doc, modifiers)))
              ...
              if (in.isNextToken('{')) {
                ...
                while (idx < len) {
                  if (
                    ...
                    val field = fieldInfos(idx)
                    if (field.defaultValueConstructor ne None) {
  ...
  private[this] def isEnumeration[F[_, _], A](variant: Reflect.Variant[F, A]): Boolean =
    enumValuesAsStrings && variant.cases.forall(_.value.asRecord.exists(_.fields.isEmpty))
  ...
  private[this] def option[F[_, _], A](variant: Reflect.Variant[F, A]): Option[Reflect[F, ?]] = {
    ...
    if (
      (typeId match {
        case TypeId.Nominal("Option", owner, _) => owner == Owner.parse("scala")
        case _                                  => false
      }) &&
  ...
  private[this] def isTuple[F[_, _], A](reflect: Reflect[F, A]): Boolean = reflect.isRecord && {
    val typeId = reflect.typeId
    typeId match {
      case TypeId.Nominal(name, owner, _) => owner == Owner.parse("scala") && name.startsWith("Tuple")
      case _                              => false
    }
  }
