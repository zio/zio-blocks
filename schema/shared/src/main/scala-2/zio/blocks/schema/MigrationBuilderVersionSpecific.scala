...
trait MigrationBuilderVersionSpecific[A, B] {
  ...
  def addField(target: B => Any, default: SchemaExpr[DynamicValue, Any]): MigrationBuilder[A, B] =
...
object MigrationBuilderMacros {
  def addFieldImpl[A, B](
    c: whitebox.Context
  )(target: c.Tree, default: c.Tree)(implicit tagA: c.WeakTypeTag[A], tagB: c.WeakTypeTag[B]): c.Tree = {
    import c.universe._
    val _         = (tagA, tagB)
  ...
  def dropFieldImpl[A, B](
    c: whitebox.Context
  )(source: c.Tree)(implicit tagA: c.WeakTypeTag[A], tagB: c.WeakTypeTag[B]): c.Tree = {
    import c.universe._
    val _         = (tagA, tagB)
  ...
  def renameFieldImpl[A, B](
    c: whitebox.Context
  )(from: c.Tree, to: c.Tree)(implicit tagA: c.WeakTypeTag[A], tagB: c.WeakTypeTag[B]): c.Tree = {
    import c.universe._
    val _        = (tagA, tagB)
