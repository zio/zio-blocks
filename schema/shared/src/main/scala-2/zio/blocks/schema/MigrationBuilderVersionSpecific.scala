...
trait MigrationBuilderVersionSpecific[A, B] { self: MigrationBuilder[A, B] =>
  ...
  def addField(target: B => Any, default: DynamicValue): MigrationBuilder[A, B] = macro
    MigrationMacros.addFieldImpl[A, B]
