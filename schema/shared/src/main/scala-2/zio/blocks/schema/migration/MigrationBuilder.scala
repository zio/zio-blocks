...
final class MigrationBuilder[A, B](
  ...
  def addField(target: B => Any, default: DynamicValue): MigrationBuilder[A, B] =
  ...
  def transformField(from: A => Any, to: B => Any, transform: DynamicValue): MigrationBuilder[A, B] =
    macro MigrationBuilderMacros.transformFieldImpl[A, B]

  def mandateField(source: A => Any, default: DynamicValue): MigrationBuilder[A, B] =
    macro MigrationBuilderMacros.mandateFieldImpl[A, B]

  def optionalizeField(source: A => Any): MigrationBuilder[A, B] =
    macro MigrationBuilderMacros.optionalizeFieldImpl[A, B]

  def changeFieldType(source: A => Any, converter: DynamicValue): MigrationBuilder[A, B] =
