...
final class MigrationBuilder[A, B](
  ...
  def addField[T](target: B => T, default: T)(implicit targetFieldSchema: Schema[T]): MigrationBuilder[A, B] = macro
    MigrationBuilderMacros.addFieldImpl[A, B, T]

  /** Add a new field to the target with a DynamicValue default. */
  def addFieldDynamic(target: B => Any, default: DynamicValue): MigrationBuilder[A, B] = macro
    MigrationBuilderMacros.addFieldDynamicImpl[A, B]

  /** Drop a field from the source. */
  def dropField[T](source: A => T, defaultForReverse: T)(implicit
    sourceFieldSchema: Schema[T]
  ): MigrationBuilder[A, B] = macro MigrationBuilderMacros.dropFieldImpl[A, B, T]

  /** Drop a field from the source with a DynamicValue for reverse. */
  def dropFieldDynamic(source: A => Any, defaultForReverse: DynamicValue): MigrationBuilder[A, B] = macro
    MigrationBuilderMacros.dropFieldDynamicImpl[A, B]
  ...
  def transformFieldLiteral[T](at: A => T, newValue: T)(implicit fieldSchema: Schema[T]): MigrationBuilder[A, B] = macro
    MigrationBuilderMacros.transformFieldLiteralImpl[A, B, T]

  /** Convert an optional field to required. */
  def mandateField[T](source: A => Option[T], target: B => T, default: T)(implicit
    fieldSchema: Schema[T]
  ): MigrationBuilder[A, B] = macro MigrationBuilderMacros.mandateFieldImpl[A, B, T]

  /** Convert a required field to optional. */
  def optionalizeField[T](source: A => T, target: B => Option[T]): MigrationBuilder[A, B] = macro
    MigrationBuilderMacros.optionalizeFieldImpl[A, B, T]
