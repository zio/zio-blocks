...
object SchemaFieldsMacros {
  ...
  def derivedImpl[A: c.WeakTypeTag](c: whitebox.Context)(
    schema: c.Expr[Schema[A]]
  ): c.Expr[SchemaFields[A]] = {
    import c.universe._

    // We can't inspect the Schema value at compile time in Scala 2,
    // so we use runtime extraction with a type placeholder
    c.Expr[SchemaFields[A]](q"""
      {
        val s = $schema
        val names = _root_.zio.blocks.schema.migration.SchemaFields.extractFieldNames(s)
        new _root_.zio.blocks.schema.migration.SchemaFields[${weakTypeOf[A]}] {
          type Fields = _root_.zio.blocks.schema.migration.FieldSet.HNil
          def fieldNames: _root_.scala.List[_root_.java.lang.String] = names
        }
      }
    """)
  }
