 ...
 * @param tpeRepr Field type (stored as Any to be version-independent in signatures, but is c.Type at runtime)
...
private[schema] object FieldMapper {
   ...
   * 
   * @param c Compilation context
   * @param sourceFields Fields from source type
   * @param targetFields Fields from target type
   * @return Right with mapping (sourceIndex -> targetIndex) or Left with error message
   */
