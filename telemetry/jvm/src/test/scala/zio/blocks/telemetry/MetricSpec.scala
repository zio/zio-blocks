...
object MetricSpec extends ZIOSpecDefault {
  ...
  def spec = suite("Metrics")(
    ...
    suite("Counter")(
      ...
      test("varargs add without attributes records against empty attributes") {
        val counter = Counter("requests", "Total requests", "1")
        counter.add(5L)
        val points = sumPoints(counter.collect())
        assertTrue(points.size == 1 && points.head.value == 5L && points.head.attributes.isEmpty)
      }
    ),
    suite("UpDownCounter")(
    ...
    suite("Histogram")(
      ...
      test("varargs record without attributes records against empty attributes") {
        val histogram = Histogram("latency", "Request latency", "ms")
        histogram.record(42.5)
        val points = histogramPoints(histogram.collect())
        assertTrue(points.size == 1 && points.head.count == 1L && points.head.attributes.isEmpty)
      }
    ),
    suite("Gauge")(
      ...
      test("varargs record without attributes records against empty attributes") {
        ...
        gauge.record(42.0)
        val points = gaugePoints(gauge.collect())
        assertTrue(points.size == 1 && points.head.value == 42.0 && points.head.attributes.isEmpty)
      }
    ),
