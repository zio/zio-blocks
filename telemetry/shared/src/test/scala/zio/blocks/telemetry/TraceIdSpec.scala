...
object TraceIdSpec extends ZIOSpecDefault {
  ...
  def spec = suite("TraceId")(
    ...
    suite("equality")(
      test("fixed ids roundtrip through hex encoding") {
        val hi     = 123L
        val lo     = 456L
        val parsed = TraceId.fromHex(TraceId.toHex(hi, lo))
        assertTrue(parsed.contains((hi, lo)))
      },
      test("different ids produce different hex strings") {
        val first  = TraceId.toHex(123L, 456L)
        val second = TraceId.toHex(123L, 457L)
