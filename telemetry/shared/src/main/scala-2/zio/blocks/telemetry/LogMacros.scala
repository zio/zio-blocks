...
private[telemetry] object LogMacros {
  ...
  def traceEveryImpl(
    c: blackbox.Context
  )(every: c.Expr[Int], message: c.Expr[String], enrichments: c.Expr[Any]*): c.Expr[Unit] =
  ...
  def traceAtMostImpl(
    c: blackbox.Context
  )(intervalMillis: c.Expr[Long], message: c.Expr[String], enrichments: c.Expr[Any]*): c.Expr[Unit] =
    ...
    val emitTree: Tree = if (hasFallbacks) {
      ...
      q"""
        val record = _root_.zio.blocks.telemetry.LogRecord(
          ...
          spanId = sId,
          traceFlags = tFlags,
          resource = state.logger.resource,
    ...
    c.Expr[Unit](q"""
      {
        if ($sevNum >= _root_.zio.blocks.telemetry.GlobalLogState.globalMinLevel) {
          ...
          if (state != null && $severity.number >= state.effectiveLevel($namespace)) {
            val now = _root_.zio.blocks.telemetry.EpochClock.epochNanos()
            ...
            val (tIdHi: _root_.scala.Long, tIdLo: _root_.scala.Long, sId: _root_.scala.Long, tFlags: _root_.scala.Byte) = spanCtx match {
              case _root_.scala.Some(sc) => (sc.traceIdHi, sc.traceIdLo, sc.spanId.value, sc.traceFlags.byte)
              case _root_.scala.None => (0L, 0L, 0L, 0.toByte)
            }
