  ...
  class AttributesBuilder private[Attributes] () {
    ...
    def build: Attributes =
      if (_len == 0) Attributes.empty
        ...
        val sq =
          if (hasActiveSeqs) java.util.Arrays.copyOf(_seqs, _len)
          else null
    ...
    def buildAndReset(): Attributes =
      if (_len == 0) Attributes.empty
        ...
        val sq  = if (hasActiveSeqs) _seqs else null
        val a   = new Attributes(_keys, _types, _longs, _strings, sq, _len)
        val cap = 8
