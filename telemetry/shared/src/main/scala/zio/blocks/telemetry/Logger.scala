...
final class Logger(
  ...
  /**
   * Returns the current SpanContext from the context storage, if any. Used by
   * macro-generated code for trace correlation.
   */
  def currentSpanContext(): Option[SpanContext] = contextStorage.get()
