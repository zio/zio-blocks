...
object MergeInnerErrorSpec extends StreamsBaseSpec {
  ...
  private def coordinatedFailures[A](
    success: Stream[Nothing, A],
    failure: CyclicBarrier => Stream[String, A]
  ): Boolean = {
    val barrier = new CyclicBarrier(4)
    val streams = Stream.fromIterable(
      (0 until 8).map(i =>
        if ((i & 1) == 0) failure(barrier)
        else success
      )
    )
    Stream.mergeAll(8)(streams).runCollect.isLeft
  }
  ...
  def spec: Spec[TestEnvironment, Any] = suite("MergeInnerErrorSpec")(
    ...
    suite("multiple concurrent inner errors do not hang")(
      test("generic: four coordinated errors terminate") {
        ZIO.attemptBlocking(
          assertTrue(
            coordinatedFailures(Stream("value"), barrier => Stream.suspend { barrier.await(); Stream.fail(errorMsg) })
          )
        )
      } @@ TestAspect.timeout(30.seconds),
      test("Int: four coordinated errors terminate") {
        ZIO.attemptBlocking(
          assertTrue(
            coordinatedFailures(
              Stream(1),
              barrier => Stream.range(0, 0) ++ Stream.suspend { barrier.await(); Stream.fail(errorMsg) }
            )
          )
        )
      } @@ TestAspect.timeout(30.seconds),
      test("Long: four coordinated errors terminate") {
        ZIO.attemptBlocking(
          assertTrue(
            coordinatedFailures(
              Stream(1L),
              barrier => Stream(1L).take(0) ++ Stream.suspend { barrier.await(); Stream.fail(errorMsg) }
            )
          )
        )
      } @@ TestAspect.timeout(30.seconds),
      test("Float: four coordinated errors terminate") {
        ZIO.attemptBlocking(
          assertTrue(
            coordinatedFailures(
              Stream(1.0f),
              barrier => Stream(1.0f).take(0) ++ Stream.suspend { barrier.await(); Stream.fail(errorMsg) }
            )
          )
        )
      } @@ TestAspect.timeout(30.seconds),
      test("Double: four coordinated errors terminate") {
        ZIO.attemptBlocking(
          assertTrue(
            coordinatedFailures(
              Stream(1.0d),
              barrier => Stream(1.0d).take(0) ++ Stream.suspend { barrier.await(); Stream.fail(errorMsg) }
            )
          )
        )
      } @@ TestAspect.timeout(30.seconds)
