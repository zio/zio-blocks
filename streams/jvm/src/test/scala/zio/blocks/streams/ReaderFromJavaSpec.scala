...
object ReaderFromJavaSpec extends StreamsBaseSpec {
  ...
  def spec: Spec[TestEnvironment, Any] = suite("Reader.fromInputStream / fromReader")(
    ...
    suite("Stream-level IOException surfacing")(
      ...
      test("Java input adapters replay one StreamError identity on every later pull") {
        val brokenStream = new InputStream {
          override def read(): Int = throw new IOException("broken stream")
        }
        val input      = Reader.fromInputStream(brokenStream)
        val inputFirst = scala.util.Try(input.readByte()).failed.toOption.orNull
        val inputBulk  = scala.util.Try(input.readBytes(new Array[Byte](0), 0, 0)).failed.toOption.orNull
        val inputChunk = scala.util.Try(input.readN[Any](0)).failed.toOption.orNull

        val brokenReader = new JReader {
          override def read(buf: Array[Char], off: Int, len: Int): Int = throw new IOException("broken reader")
          override def close(): Unit                                   = ()
        }
        val chars      = Reader.fromReader(brokenReader)
        val charsFirst = scala.util.Try(chars.read[Any](null)).failed.toOption.orNull
        chars.close()
        val charsNext = scala.util.Try(chars.readChar(-1)).failed.toOption.orNull

        assertTrue(
          inputFirst.isInstanceOf[StreamError],
          inputBulk eq inputFirst,
          inputChunk eq inputFirst,
          charsFirst.isInstanceOf[StreamError],
          charsNext eq charsFirst
        )
      },
      ...
      test("fromInputStream replays a close-only failure and closes exactly once") {
        val closeFailure = new IOException("input close")
        var closeCount   = 0
        ...
        val reader = Stream.fromInputStream(is).compile(0).expectedSync
      ...
      test("fromInputStream keeps read failure primary and suppresses close failure") {
        val readFailure  = new IOException("input read")
        val closeFailure = new IOException("input close")
        var closeCount   = 0
        val is           = new InputStream {
          override def read(): Int   = throw readFailure
          override def close(): Unit = {
            closeCount += 1
            throw closeFailure
          }
        }
        val caught = try { Stream.fromInputStream(is).runDrain; null }
        catch { case t: Throwable => t }
      ...
      test("fromJavaReader replays a close-only failure and closes exactly once") {
        val closeFailure = new IOException("reader close")
        var closeCount   = 0
        val javaReader   = new JReader {
          override def read(buf: Array[Char], off: Int, len: Int): Int = -1
          override def close(): Unit                                   = {
            closeCount += 1
            throw closeFailure
          }
        }
        val reader = Stream.fromJavaReader(javaReader).compile(0).expectedSync
        val first  = try { reader.close(); null }
        catch { case t: Throwable => t }
        val second = try { reader.close(); null }
        catch { case t: Throwable => t }
      ...
      test("fromJavaReader keeps read failure primary and suppresses close failure") {
        val readFailure  = new IOException("reader read")
        val closeFailure = new IOException("reader close")
        var closeCount   = 0
        val javaReader   = new JReader {
          override def read(buf: Array[Char], off: Int, len: Int): Int = throw readFailure
          override def close(): Unit                                   = {
            closeCount += 1
            throw closeFailure
          }
        }
        val caught = try { Stream.fromJavaReader(javaReader).runDrain; null }
        catch { case t: Throwable => t }
