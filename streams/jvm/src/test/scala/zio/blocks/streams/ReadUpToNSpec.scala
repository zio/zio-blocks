  ...
  def spec: Spec[TestEnvironment, Any] = suite("Reader#readUpToN")(
    suite("FromChunk[String] (generic)")(
      test("readUpToN(0) returns empty chunk") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk("a", "b", "c")))
      ...
      test("readUpToN(n < total) returns exactly n elements") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk("a", "b", "c", "d", "e")))
      ...
      test("readUpToN on exhausted reader returns empty chunk") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk("a", "b")))
    ...
    suite("FromChunkInt")(
      test("readUpToN(0) returns empty chunk") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(1, 2, 3)))
      ...
      test("readUpToN(n < total) returns exactly n elements") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(1, 2, 3, 4, 5)))
      ...
      test("successive readUpToN calls advance position") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(10, 20, 30, 40, 50)))
    ...
    suite("FromChunkLong")(
      test("readUpToN(0) returns empty chunk") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(1L, 2L, 3L)))
      ...
      test("readUpToN(n < total) returns exactly n elements") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(10L, 20L, 30L, 40L, 50L)))
        assertTrue(reader.readUpToN[Long](3) == Chunk(10L, 20L, 30L))
      },
      test("readUpToN(n > total) returns all remaining elements") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(1L, 2L)))
    ...
    suite("FromChunkDouble")(
      test("readUpToN(0) returns empty chunk") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(1.0, 2.0, 3.0)))
      ...
      test("readUpToN(n < total) returns exactly n elements") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(1.5, 2.5, 3.5, 4.5)))
        assertTrue(reader.readUpToN[Double](2) == Chunk(1.5, 2.5))
      },
      test("readUpToN(n > total) returns all remaining elements") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(1.0, 2.0)))
    ...
    suite("FromChunkFloat")(
      test("readUpToN(0) returns empty chunk") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(1.0f, 2.0f, 3.0f)))
      ...
      test("readUpToN(n < total) returns exactly n elements") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(1.1f, 2.2f, 3.3f, 4.4f)))
        assertTrue(reader.readUpToN[Float](2) == Chunk(1.1f, 2.2f))
      },
      test("readUpToN(n > total) returns all remaining elements") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(1.0f, 2.0f)))
    ...
    suite("FromRange")(
      test("readUpToN(0) returns empty chunk") {
        val reader = compileToSyncReader(Stream.range(0, 10))
        assertTrue(reader.readUpToN[Int](0) == Chunk.empty)
      },
      test("readUpToN(1) returns single element") {
        val reader = compileToSyncReader(Stream.range(0, 10))
    ...
    suite("ConcatReader (boundary spanning)")(
      test("readUpToN spans concatenation boundary") {
        ...
        val reader = Sink.toSyncReader(Stream.compileToReader(s1 ++ s2))
        val c1     = reader.readUpToN[Int](3)
        val c2     = reader.readUpToN[Int](3)
    ...
    suite("Taken")(
      test("readUpToN respects take limit") {
        val reader = compileToSyncReader(Stream.range(0, 100).take(5))
      ...
      test("readUpToN(3) on take(3) returns exactly 3 elements") {
        val reader = compileToSyncReader(Stream.range(0, 100).take(3))
    ...
    suite("TakenWhile")(
      test("readUpToN respects takeWhile predicate") {
        val reader = compileToSyncReader(Stream.range(0, 100).takeWhile(_ < 5))
        var all    = Chunk.empty: Chunk[Int]
        var done   = false
        while (!done) {
          val chunk = reader.readUpToN[Int](10)
      ...
      test("readUpToN on takeWhile that immediately fails returns empty") {
        val reader = compileToSyncReader(Stream.range(10, 20).takeWhile(_ < 5))
        assertTrue(reader.readUpToN[Int](5) == Chunk.empty)
      }
    ),
    suite("SkipLimitReader")(
      test("readUpToN respects skip and limit") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk.fromIterable(0 until 20)).drop(5).take(5))
        var all    = Chunk.empty: Chunk[Int]
        var done   = false
        while (!done) {
          val chunk = reader.readUpToN[Int](10)
      ...
      test("readUpToN(2) on skip+limit reader returns at most 2 elements") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk.fromIterable(0 until 20)).drop(3).take(10))
    ...
    suite("ConcurrentBufferedReader (JVM)")(
      test("readUpToN returns available elements") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(1, 2, 3, 4, 5)).buffer(4))
        var all    = Chunk.empty: Chunk[Int]
        var done   = false
        while (!done) {
          val chunk = reader.readUpToN[Int](10)
      ...
      test("readUpToN(0) returns empty on buffered reader") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(1, 2, 3)).buffer(4))
    ...
    suite("ConcurrentMergeReader (JVM)")(
      test("readUpToN returns elements from merged streams") {
        ...
        val compiled = Stream.compileToReader(merged)
        val reader   = compiled match {
          case sync: Reader.SyncReader[Int @unchecked]   => sync.toAsync
          case async: Reader.AsyncReader[Int @unchecked] => async
        }
        def drain(all: Chunk[Int]): Async[Chunk[Int]] =
          reader.readUpToN[Int](10).flatMap(chunk => if (chunk.isEmpty) Async.succeed(all) else drain(all ++ chunk))
        ZIO
          .fromFuture(_ => drain(Chunk.empty).toFuture)
          .ensuring(ZIO.fromFuture(_ => reader.close().toFuture).orDie)
          .map { all =>
