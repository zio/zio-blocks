  ...
  def spec: Spec[TestEnvironment, Any] = suite("IsClosedRegressionSpec")(
    suite("mapPar isClosed must not be true while elements remain (P1-A regression)")(
      test("Int mapPar: isClosed false until all elements consumed") {
        ZIO.attemptBlocking {
          val n      = 10_000
          val reader = Stream.range(0, n).mapPar(4)(identity).compile(0, Stream.DefaultBufferSize).expectedSync
          var count  = 0
          val s      = Long.MinValue
          var v      = reader.readInt(s)
          while (v != s) {
            count += 1
            v = reader.readInt(s)
          }
      ...
      test("mapPar: readable() is consistent with isClosed during consumption") {
        ZIO.attemptBlocking {
          val n                  = 1000
          val reader             = Stream.range(0, n).mapPar(2)(identity).compile(0, Stream.DefaultBufferSize).expectedSync
