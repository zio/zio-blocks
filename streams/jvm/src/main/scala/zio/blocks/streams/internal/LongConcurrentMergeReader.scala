 ...
 * `flatMapPar`).
 *
 * Its per-drainer SPSC channels represent empty and done out of band, so all
 * `Long` values are transported without boxing or sentinel collisions.
 */
private[streams] final class LongConcurrentMergeReader(outerReader: Reader.SyncReader[?], maxOpen: Int, bufferSize: Int)
    extends Reader.SyncReader[Long] {
  ...
  private val scalarScratch        = new Array[Long](1)

  override def readLong(sentinel: Long)(implicit ev: Long <:< Long): Long =
    if (readLongs(scalarScratch, 0, 1) < 0) sentinel else scalarScratch(0)

  def read[A1 >: Long](sentinel: A1): A1 =
    if (readLongs(scalarScratch, 0, 1) < 0) sentinel
    else Long.box(scalarScratch(0)).asInstanceOf[A1]
  ...
  override def readLongs(dest: Array[Long], offset: Int, length: Int)(implicit ev: Long <:< Long): Int = {
    ...
    if (length == 0) 0
      ...
      val first = readOne(dest, offset, wait = true)
      if (first < 0) -1
      else {
        var count = 1
        var next  = 1
        while (count < length && next > 0) {
          next = readOne(dest, offset + count, wait = false)
          if (next > 0) count += 1
        }
        count
      }
  ...
  override def readUpToN[A1 >: Long](n: Int): Chunk[A1] = {
    ...
    val first = readLongs(scalarScratch, 0, 1)
    if (first < 0) return Chunk.empty
    b.addOne(scalarScratch(0))
    var i    = 1
    var read = 1
    while (i < n && read > 0) {
      read = readOne(scalarScratch, 0, wait = false)
      if (read > 0) { b.addOne(scalarScratch(0)); i += 1 }
    }
  ...
  private def drainInner(innerStream: Stream[Any, Long], drainerIdx: Int): Unit = {
    var innerReader: Reader.SyncReader[Long] = null
    try {
      innerReader = innerStream.compile(0, bufferSize) match {
        case sync: Reader.SyncReader[Long @unchecked]   => sync
        case async: Reader.AsyncReader[Long @unchecked] => async.toSync
      }
      activeReaders.set(drainerIdx, new ConcurrentShutdown.CloseOnce(innerReader))
      var running = true
      val input   = new Array[Long](1)
      while (running && !consumerClosed && !Thread.currentThread().isInterrupted) {
        val count = innerReader.readLongsPhysical(input, 0, 1)
        if (count < 0) {
          running = false
        } else {
          val v       = input(0)
  ...
  private def readOne(dest: Array[Long], offset: Int, wait: Boolean): Int = {
    while (true) {
      var i = 0
      while (i < maxOpen) {
