...
private[streams] final class ProducerReader[A](bufferSize: Int) extends Reader[A] {
  ...
  private[streams] def isCancelled: Boolean = cancelled.get()
  ...
  val sink: ProducerSink[A, Any] = new ProducerSink[A, Any] {
    ...
    override def emit(chunk: Chunk[A @uncheckedVariance]): Boolean = {
  ...
  def close(): Unit =
    if (!closed) {
      ...
      if (cancelled.compareAndSet(false, true)) {
        ...
        if (cb != null) {
          // Cancel callback invoked synchronously on consumer thread.
          // The Stream contract requires that close() has run the cancel
          // callback before returning so callers observe cancellation
          // immediately. Exceptions are swallowed; queue closure already
          // unblocks producer/consumer.
          try cb()
