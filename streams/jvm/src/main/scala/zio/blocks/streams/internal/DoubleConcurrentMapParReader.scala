 ...
 * out of band, so every raw `Double` bit pattern and `Long` value is preserved.
 *
 * The per-worker output queue is selected at construction time from `outType`:
 *   - `JvmType.Boolean` / `Byte` / `Char` / `Short` / `Int` →
 *     `IntSpscRingBuffer`
...
private[streams] final class DoubleConcurrentMapParReader[B](
  upstream: Reader.SyncReader[Double],
  n: Int,
  f: Double => B,
  bufferSize: Int,
  outType: JvmType
  ...
  private val doubleToBooleanFn: (Double => Boolean) =
    if (outType eq JvmType.Boolean) f.asInstanceOf[Double => Boolean] else null
  private val doubleToByteFn: (Double => Byte) =
    if (outType eq JvmType.Byte) f.asInstanceOf[Double => Byte] else null
  private val doubleToCharFn: (Double => Char) =
    if (outType eq JvmType.Char) f.asInstanceOf[Double => Char] else null
  ...
  private val doubleToShortFn: (Double => Short) =
    if (outType eq JvmType.Short) f.asInstanceOf[Double => Short] else null
    ...
    def run(): Unit = {
      ...
      try {
        ...
        while (running && !consumerClosed && !Thread.currentThread().isInterrupted) {
          val count = upstream.readDoublesPhysical(coordBuf, 0, bufferSize)
          if (count <= 0) {
            running = false
          } else {
            var j = 0
            while (j < count && running && !consumerClosed && !Thread.currentThread().isInterrupted) {
              ...
              while (!dispatched && !consumerClosed && !Thread.currentThread().isInterrupted) {
                ...
                while (i < n && !dispatched) {
                  ...
                  if (inputQueues(qIdx).offer(java.lang.Double.doubleToRawLongBits(v))) {
  ...
  private def workerLoop(idx: Int): Unit = {
    ...
    while (keepRunning && !consumerClosed && !self.isInterrupted) {
      ...
      if (status == FullDomainSpscRingBuffer.Done) {
        ...
        val input: Double = java.lang.Double.longBitsToDouble(inputBits(0))
        try {
          if (doubleToBooleanFn ne null) {
            val out = doubleToBooleanFn(input)
            offerToIntOutput(idx, if (out) 1 else 0, self) || { keepRunning = false; true }; ()
          } else if (doubleToByteFn ne null) {
            val out = doubleToByteFn(input)
            offerToIntOutput(idx, out.toInt & 0xff, self) || { keepRunning = false; true }; ()
          } else if (doubleToCharFn ne null) {
            val out = doubleToCharFn(input)
            offerToIntOutput(idx, out.toInt, self) || { keepRunning = false; true }; ()
          } else if (doubleToIntFn ne null) {
          ...
          } else if (doubleToShortFn ne null) {
            val out = doubleToShortFn(input)
            offerToIntOutput(idx, out.toInt, self) || { keepRunning = false; true }; ()
          } else {
            val result          = f(input)
