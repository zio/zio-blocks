...
object IntSpecializationSpec extends StreamsBaseSpec {
  ...
  val singletonIntSuite = suite("SingletonInt")(
    ...
    test("Stream.succeed(Int) uses SingletonInt") {
      ...
      runAsync(Stream.succeed(42).runCollectAsync).map { result =>
  ...
  val specializedCombinatorSuite = suite("Specialized combinators")(
    test("range.map(+1) via readInt matches runCollect") {
      runAsync(Stream.range(0, 100).map(_ + 1).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk.fromIterable(1 to 100)))

      }
    },
    test("range.filter(even) via readInt matches runCollect") {
      runAsync(Stream.range(0, 100).filter(_ % 2 == 0).runCollectAsync).map { result =>
    ...
    test("range.map.filter pipeline produces specialized chain") {
      runAsync(Stream.range(0, 10).map(_ * 2).filter(_ > 5).runCollectAsync).map { result =>
    ...
    test("range.take via TakenInt") {
      runAsync(Stream.range(0, 100).take(10).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk.fromIterable(0 until 10)))

      }
    },
    test("range.takeWhile via TakenWhileInt") {
      runAsync(Stream.range(0, 100).takeWhile(_ < 10).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk.fromIterable(0 until 10)))

      }
    },
    test("range.drop via DroppedInt") {
      runAsync(Stream.range(0, 20).drop(10).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk.fromIterable(10 until 20)))

      }
    },
    test("range ++ range via ConcatWithInt") {
      runAsync((Stream.range(0, 5) ++ Stream.range(5, 10)).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk.fromIterable(0 until 10)))
    ...
    test("chained Int maps fuse into a blocking Long fold") {
      def mapped(stream: Stream[Nothing, Int]) =
        stream.map(_ + 1).map(_ * 2).map(_ - 3).map(_ * 5)

      val expected = (0 until 10).map(value => (((value + 1) * 2) - 3) * 5).map(_.toLong).sum
      assertTrue(
        mapped(Stream.range(0, 10)).runFoldLongBlocking(0L, _ + _) == Right(expected),
        mapped(Stream.fromIterable(0 until 10)).runFoldLongBlocking(0L, _ + _) == Right(expected)
      )
    },
    test("filter then map then fold Long") {
      runAsync(
        Stream.range(0, 100).filter(_ % 2 == 0).map(_ + 1).runFoldAsync(0L)((acc, v) => Async.succeed(acc + v))
      ).map { result =>
  ...
  val specializedSinkSuite = suite("Specialized sinks")(
    test("runFold(Long) matches runFold[Z] for Int streams") {
      val n = 1000
      runAsync(Stream.range(0, n).runFoldAsync[Long](0L)((acc, i) => Async.succeed(acc + i))).flatMap { generic =>
        runAsync(Stream.range(0, n).runFoldAsync(0L)((acc, i) => Async.succeed(acc + i))).map { specialized =>
    ...
    test("runFold(Int) matches manual sum") {
      ...
      runAsync(Stream.range(0, n).runFoldAsync(0)((acc, i) => Async.succeed(acc + i))).map { result =>
        assertTrue(result == Right(expected))

      }
    },
    test("Sink.sumInt uses specialized path") {
      runAsync(Stream.range(0, 100).runAsync(Sink.sumInt)).map { result =>
    ...
    test("drain on Int stream uses drainInt path") {
      runAsync(Stream.range(0, 10000).runDrainAsync).map { result =>
    ...
    test("runFold(Double) matches manual sum for Int streams") {
      ...
      runAsync(Stream.range(0, n).runFoldAsync(0.0)((acc, i) => Async.succeed(acc + i))).map { result =>
        assertTrue(result == Right(expected))
    ...
    test("runFold(Double) matches runFold[Z] for Int streams") {
      val n = 1000
      runAsync(Stream.range(0, n).runFoldAsync[Double](0.0)((acc, i) => Async.succeed(acc + i))).flatMap { generic =>
        runAsync(Stream.range(0, n).runFoldAsync(0.0)((acc, i) => Async.succeed(acc + i))).map { specialized =>
          assertTrue(generic == specialized)
    ...
    test("runFold(Double) works on Long streams") {
      ...
      runAsync(Stream.fromChunk(data).runFoldAsync(0.0)((acc, l) => Async.succeed(acc + l))).map { result =>
        assertTrue(result == Right(expected))
    ...
    test("runFold(Double) works on Float streams") {
      ...
      runAsync(Stream.fromChunk(data).runFoldAsync(0.0)((acc, f) => Async.succeed(acc + f))).map { result =>
        assertTrue(result == Right(expected))
    ...
    test("runFold(Double) works on Double streams") {
      ...
      val expected = (1 to 50).map(_.toDouble).sum
      runAsync(Stream.fromChunk(data).runFoldAsync(0.0)((acc, d) => Async.succeed(acc + d))).map { result =>
        assertTrue(result == Right(expected))
    ...
    test("runFold(Double) works on generic streams") {
      ...
      runAsync(Stream.fromChunk(data).runFoldAsync(0.0)((acc, s) => Async.succeed(acc + s.length))).map { result =>
    ...
    test("Sink.sumFloat uses specialized path for Float streams") {
      ...
      runAsync(Stream.fromChunk(data).runAsync(Sink.sumFloat)).map { result =>
        assertTrue(result == Right(expected))
  ...
  val flatMapSpecializationSuite = suite("FlatMap specialization")(
    test("flatMap with generic chunk inner stream") {
      ...
      runAsync(Stream.succeed(42).flatMap(_ => Stream.fromChunk(genericChunk)).runCollectAsync).map { result =>
    ...
    test("flatMap with boxed function") {
      ...
      runAsync(Stream.succeed(5).flatMap(boxedF.asInstanceOf[Int => Stream[Nothing, Int]]).runCollectAsync).map {
        result =>
    ...
    test("flatMap like StreamLawsSpec") {
      ...
      check(
        ...
        for {
          left  <- runAsync(Stream.succeed(a).flatMap(f).runCollectAsync)
          right <- runAsync(f(a).runCollectAsync)
        } yield assertTrue(left.fold(_ => Chunk.empty, identity) == right.fold(_ => Chunk.empty, identity))
      }
    },
    test("flatMap(succeed) matches manual computation") {
      runAsync(Stream.range(0, 100).flatMap(i => Stream.succeed(i * 2)).runCollectAsync).map { result =>
    ...
    test("flatMap with runFold(Long) uses specialized path") {
      runAsync(
        Stream.range(0, 100).flatMap(i => Stream.succeed(i * 2)).runFoldAsync(0L)((acc, i) => Async.succeed(acc + i))
      ).map { result =>
    ...
    test("flatMap with multi-element inner streams") {
      runAsync(Stream.range(0, 3).flatMap(i => Stream.range(i * 10, i * 10 + 3)).runCollectAsync).map { result =>
  ...
  val edgeCaseSuite = suite("Edge cases")(
    test("Int.MinValue as stream element is handled correctly") {
      ...
      runAsync(Stream.succeed(Int.MinValue).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk(Int.MinValue)))

      }
    },
    test("Int.MinValue through map") {
      runAsync(Stream.succeed(Int.MinValue).map(_ + 1).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk(Int.MinValue + 1)))

      }
    },
    test("Int.MinValue through filter") {
      runAsync(Stream.succeed(Int.MinValue).filter(_ == Int.MinValue).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk(Int.MinValue)))

      }
    },
    test("empty range produces no elements via specialized path") {
      runAsync(Stream.range(0, 0).map(_ + 1).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk.empty))

      }
    },
    test("repeated stream via specialized path") {
      runAsync(Stream.range(0, 3).repeated.take(9).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk(0, 1, 2, 0, 1, 2, 0, 1, 2)))

      }
    },
    test("drop then take via specialized path") {
      runAsync(Stream.range(0, 100).drop(10).take(5).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk(10, 11, 12, 13, 14)))

      }
    },
    test("mapError preserves elements via specialized path") {
      runAsync(Stream.range(0, 5).mapError((_: Nothing) => "err").runCollectAsync).map { result =>
  ...
  val resetSuite = suite("reset")(
    ...
    test("InputStreamReader reset throws") {
      ...
      val dq: Reader.SyncReader[Byte] = Reader.fromInputStream(is)
      assertTrue(
        try { dq.reset(); false }
