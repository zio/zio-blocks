...
object StreamStateSpec extends ZIOSpecDefault {
  def spec: Spec[Any, Nothing] = suite("StreamState")(
    ...
    test("max index values (15-bit = 32767)") {
      val s = StreamState(32767, 32767, 32767, 32767, 4)
      assertTrue(
        StreamState.stageStart(s) == 32767,
        StreamState.incomingLen(s) == 32767,
        StreamState.stageEnd(s) == 32767,
        StreamState.outgoingLen(s) == 32767,
