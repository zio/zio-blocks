...
object StreamMetadataSpec extends StreamsBaseSpec {
  ...
  private val knownLengthPropagationSuite = suite("knownLength propagation")(
    ...
    test("take with negative n produces empty stream") {
      runAsync(Stream.fromChunk(chunk5).take(-1).runCollectAsync)
        .map(result => assertTrue(result == Right(Chunk.empty[Int])))
    },
    ...
    test("drop with negative n produces all elements") {
      runAsync(Stream.fromChunk(chunk5).drop(-1).runCollectAsync).map(result => assertTrue(result == Right(chunk5)))
    },
  ...
  private val fromArraySuite = suite("fromArray")(
    ...
    test("roundtrip collects correctly") {
      val arr = Array[Byte](1, 2, 3)
      runAsync(Stream.fromArray(arr).runCollectAsync).map(result => assertTrue(result == Right(Chunk.fromArray(arr))))
    }
  )
  ...
  private val fromIterableMetadataSuite = suite("fromIterable metadata")(
    ...
    test("roundtrip collects correctly") {
      runAsync(Stream.fromIterable(List(1, 2, 3)).runCollectAsync)
  ...
  private val fromRangeMetadataSuite = suite("fromRange metadata")(
    ...
    test("roundtrip collects correctly") {
      runAsync(Stream.fromRange(1 to 3).runCollectAsync).map(result => assertTrue(result == Right(Chunk(1, 2, 3))))
    }
  )
}
