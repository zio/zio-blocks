...
object ByteSpecializationSpec extends StreamsBaseSpec {
  ...
  val streamRoundTripSuite = suite("Stream round-trip")(
    test("Stream.fromChunk(Chunk[Byte]).runCollectAsync round-trips correctly") {
      val chunk = Chunk[Byte](1, 2, 3)
      runAsync(Stream.fromChunk(chunk).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk[Byte](1, 2, 3)))

      }
    },
    test("empty Chunk[Byte] round-trips to empty") {
      runAsync(Stream.fromChunk(Chunk.empty[Byte]).runCollectAsync).map { result =>
    ...
    test("Stream.fromChunk(Chunk[Byte]).take works") {
      ...
      runAsync(Stream.fromChunk(chunk).take(3).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk[Byte](1, 2, 3)))
    ...
    test("Stream.fromChunk(Chunk[Byte]).drop works") {
      val chunk = Chunk[Byte](1, 2, 3, 4, 5)
      runAsync(Stream.fromChunk(chunk).drop(2).runCollectAsync).map { result =>
  ...
  val edgeCaseSuite = suite("Edge cases")(
    test("Byte.MinValue (-128) as stream element is handled correctly") {
      runAsync(Stream.fromChunk(Chunk[Byte](Byte.MinValue)).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk[Byte](Byte.MinValue)))

      }
    },
    test("Byte.MaxValue (127) as stream element is handled correctly") {
      runAsync(Stream.fromChunk(Chunk[Byte](Byte.MaxValue)).runCollectAsync).map { result =>
    ...
    test("all 256 byte values round-trip correctly") {
      ...
      runAsync(Stream.fromChunk(allBytes).runCollectAsync).map { result =>
  ...
  val sinkByteSuite = suite("Sink byte dispatch")(
    test("Sink.collectAll collects all bytes") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).runAsync(Sink.collectAll[Byte])).map { result =>
        assertTrue(result == Right(Chunk[Byte](1, 2, 3)))

      }
    },
    test("Sink.collectAll on empty byte stream") {
      runAsync(Stream.fromChunk(Chunk.empty[Byte]).runAsync(Sink.collectAll[Byte])).map { result =>
        assertTrue(result == Right(Chunk.empty[Byte]))

      }
    },
    test("Sink.count counts byte elements") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).runAsync(Sink.count)).map { result =>
        assertTrue(result == Right(3L))

      }
    },
    test("Sink.count on empty byte stream") {
      runAsync(Stream.fromChunk(Chunk.empty[Byte]).runAsync(Sink.count)).map { result =>
        assertTrue(result == Right(0L))

      }
    },
    test("Sink.drain consumes all bytes") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).runAsync(Sink.drain)).map { result =>
        assertTrue(result == Right(()))

      }
    },
    test("Sink.exists finds matching byte") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).runAsync(Sink.exists[Byte](_ == 2.toByte))).map { result =>
        assertTrue(result == Right(true))

      }
    },
    test("Sink.exists returns false when no match") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).runAsync(Sink.exists[Byte](_ == 10.toByte))).map { result =>
        assertTrue(result == Right(false))

      }
    },
    test("Sink.find locates matching byte") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).runAsync(Sink.find[Byte](_ == 2.toByte))).map { result =>
        assertTrue(result == Right(Some(2.toByte)))

      }
    },
    test("Sink.find returns None when no match") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).runAsync(Sink.find[Byte](_ == 10.toByte))).map { result =>
        assertTrue(result == Right(None))

      }
    },
    test("Sink.forall returns true when all match") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).runAsync(Sink.forall[Byte](_ < 10.toByte))).map { result =>
        assertTrue(result == Right(true))

      }
    },
    test("Sink.forall returns false when one fails") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).runAsync(Sink.forall[Byte](_ < 2.toByte))).map { result =>
        assertTrue(result == Right(false))
    ...
    test("Sink.foreach processes all bytes") {
      var sum = 0
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).runAsync(Sink.foreach[Byte](b => sum += b))).map { result =>
        assertTrue(result == Right(()), sum == 6)

      }
    },
    test("Sink.foldLeft accumulates bytes") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).runAsync(Sink.foldLeft[Byte, Int](0)(_ + _))).map { result =>
        assertTrue(result == Right(6))

      }
    },
    test("Sink.foldLeft on empty byte stream") {
      runAsync(Stream.fromChunk(Chunk.empty[Byte]).runAsync(Sink.foldLeft[Byte, Int](0)(_ + _))).map { result =>
        assertTrue(result == Right(0))

      }
    },
    test("Sink.head returns first byte") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).runAsync(Sink.head[Byte])).map { result =>
        assertTrue(result == Right(Some(1.toByte)))

      }
    },
    test("Sink.head returns None on empty") {
      runAsync(Stream.fromChunk(Chunk.empty[Byte]).runAsync(Sink.head[Byte])).map { result =>
        assertTrue(result == Right(None))

      }
    },
    test("Sink.last returns last byte") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).runAsync(Sink.last[Byte])).map { result =>
        assertTrue(result == Right(Some(3.toByte)))

      }
    },
    test("Sink.last returns None on empty") {
      runAsync(Stream.fromChunk(Chunk.empty[Byte]).runAsync(Sink.last[Byte])).map { result =>
        assertTrue(result == Right(None))

      }
    },
    test("Sink.take(n) takes first n bytes") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3, 4, 5)).runAsync(Sink.take[Byte](3))).map { result =>
        assertTrue(result == Right(Chunk[Byte](1, 2, 3)))

      }
    },
    test("Sink.take(n) on shorter stream") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2)).runAsync(Sink.take[Byte](5))).map { result =>
    ...
    test("Byte.MinValue handled by all sinks") {
      ...
      runAsync(Stream.fromChunk(chunk).runAsync(Sink.collectAll[Byte])).flatMap { collectResult =>
        runAsync(Stream.fromChunk(chunk).runAsync(Sink.count)).flatMap { countResult =>
          runAsync(Stream.fromChunk(chunk).runAsync(Sink.head[Byte])).flatMap { headResult =>
            runAsync(Stream.fromChunk(chunk).runAsync(Sink.last[Byte])).map { lastResult =>
              assertTrue(
  ...
  val combinatorByteSuite = suite("Stream combinator byte dispatch")(
    ...
    test("grouped groups byte elements into chunks") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3, 4, 5)).grouped(2).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk(Chunk[Byte](1, 2), Chunk[Byte](3, 4), Chunk[Byte](5))))

      }
    },
    test("grouped on empty byte stream") {
      runAsync(Stream.fromChunk(Chunk.empty[Byte]).grouped(3).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk.empty))

      }
    },
    test("grouped with n larger than stream") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2)).grouped(5).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk(Chunk[Byte](1, 2))))

      }
    },
    test("grouped preserves Byte.MinValue and Byte.MaxValue") {
      runAsync(Stream.fromChunk(Chunk[Byte](Byte.MinValue, 0, Byte.MaxValue)).grouped(2).runCollectAsync).map { result =>
    ...
    test("sliding produces correct byte windows") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3, 4, 5)).sliding(3, 1).runCollectAsync).map { result =>
        assertTrue(
          result == Right(
            Chunk(
              Chunk[Byte](1, 2, 3),
    ...
    test("sliding with step > 1") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3, 4, 5)).sliding(2, 2).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk(Chunk[Byte](1, 2), Chunk[Byte](3, 4), Chunk[Byte](5))))

      }
    },
    test("sliding on empty byte stream") {
      runAsync(Stream.fromChunk(Chunk.empty[Byte]).sliding(3).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk.empty))

      }
    },
    test("sliding with step > window size") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3, 4, 5)).sliding(2, 3).runCollectAsync).map { result =>
    ...
    test("scan accumulates byte values") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).scan(0.toByte)((a, b) => (a + b).toByte).runCollectAsync).map {
        result =>
    ...
    test("scan on empty byte stream emits only init") {
      runAsync(Stream.fromChunk(Chunk.empty[Byte]).scan(10.toByte)((a, b) => (a + b).toByte).runCollectAsync).map {
        result =>
    ...
    test("scan preserves negative byte accumulation") {
      runAsync(
        Stream.fromChunk(Chunk[Byte](Byte.MaxValue, 1)).scan(0.toByte)((a, b) => (a + b).toByte).runCollectAsync
      ).map { result =>
    ...
    test("intersperse with byte separator") {
      runAsync(Stream.fromChunk(Chunk[Byte](1, 2, 3)).intersperse(0.toByte).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk[Byte](1, 0, 2, 0, 3)))

      }
    },
    test("intersperse on single byte element") {
      runAsync(Stream.fromChunk(Chunk[Byte](42)).intersperse(0.toByte).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk[Byte](42)))

      }
    },
    test("intersperse on empty byte stream") {
      runAsync(Stream.fromChunk(Chunk.empty[Byte]).intersperse(0.toByte).runCollectAsync).map { result =>
        assertTrue(result == Right(Chunk.empty))
