  ...
  def spec: Spec[TestEnvironment, Any] = suite("Reader#readN")(
    suite("Byte branch (InputStreamReader)")(
      test("reads bytes from InputStream") {
        ...
        val reader = compileToSyncReader(Stream.fromInputStreamUnmanaged(is))
    ...
    suite("FromRange override")(
      ...
      test("FromRange readN with step > 1") {
        val reader = compileToSyncReader(Stream.fromRange(Range(0, 20, 3)))
    ...
    suite("Int branch (FromRange)")(
      ...
      test("readN(100) on 5 elements returns all remaining elements") {
        val reader = compileToSyncReader(Stream.range(0, 5))
    ...
    suite("Long branch (FromChunkLong)")(
      test("reads long values from specialized chunk reader") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(1L, 2L, 3L, 4L, 5L)))
    ...
    suite("AnyRef branch (FromIterable)")(
      test("reads reference values from iterable reader") {
        val reader = compileToSyncReader(Stream.fromIterable(List("a", "b", "c")))
    ...
    suite("FromChunk* overrides")(
      ...
      test("FromChunkLong readN returns correct typed Chunk[Long]") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk(10L, 20L, 30L, 40L)))
      ...
      test("readN respects setLimit") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk.fromIterable(0 until 10)))
        reader.setLimit(3)
      ...
      test("readN on zero-length chunk returns empty") {
        val reader = compileToSyncReader(Stream.fromChunk(Chunk.empty: Chunk[Int]))
      ...
      test("setLimit then relative setSkip preserves the limit window and reset across every chunk lane") {
        def verify[A](values: Chunk[A])(implicit jt: JvmType.Infer[A]): Boolean = {
          val reader  = Reader.fromChunk(values)
          val limited = reader.setLimit(5)
          val skipped = reader.setSkip(3)
          val first   = reader.readN[A](10)
          reader.reset()
          val replay = reader.readN[A](10)
          limited && skipped && first == values.slice(3, 8) && replay == first
        }
        assertTrue(
          verify(Chunk.fromIterable((0 until 10).map(_.toString))),
          verify(Chunk.fromIterable((0 until 10).map(i => (i & 1) == 0))),
          verify(Chunk.fromIterable((0 until 10).map(_.toByte))),
          verify(Chunk.fromIterable((0 until 10).map(_.toChar))),
          verify(Chunk.fromIterable((0 until 10).map(_.toShort))),
          verify(Chunk.fromIterable(0 until 10)),
          verify(Chunk.fromIterable((0 until 10).map(_.toLong))),
          verify(Chunk.fromIterable((0 until 10).map(_.toFloat))),
          verify(Chunk.fromIterable((0 until 10).map(_.toDouble)))
        )
      }
    )
  )
