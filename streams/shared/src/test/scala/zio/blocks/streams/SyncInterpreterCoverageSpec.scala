...
/**
 * Supplementary SyncInterpreter tests covering gaps in SyncInterpreterSpec:
 *   - All 25 addMap lane crossings (13 previously untested)
 *   - wrapLastRead
 *   - appendRead on a pipeline with existing ops
 */
object SyncInterpreterCoverageSpec extends StreamsBaseSpec {
  ...
  def spec = suite("SyncInterpreter coverage")(
  ...
  // ---- drain helpers (same as SyncInterpreterSpec) ----
  ...
  val allMapCrossingsSuite = suite("All 25 addMap lane crossings")(
    ...
    test("Long->Int (tag 5)") {
      val p = SyncInterpreter(Reader.fromChunk(Chunk(10L, 20L, 30L)))
    ...
    test("Float->Int (tag 10)") {
      val p = SyncInterpreter(Reader.fromChunk(Chunk(1.9f, 2.1f, 3.7f)))
      p.addMap[Float, Int](LANE_F, OUT_I)((_: Float).toInt)
      assertTrue(drainInts(p) == List(1, 2, 3))
    },
    test("Float->Long (tag 11)") {
      val p = SyncInterpreter(Reader.fromChunk(Chunk(1.0f, 2.0f, 3.0f)))
    ...
    test("Double->Int (tag 15)") {
      val p = SyncInterpreter(Reader.fromChunk(Chunk(1.9, 2.1, 3.7)))
      p.addMap[Double, Int](LANE_D, OUT_I)((_: Double).toInt)
      assertTrue(drainInts(p) == List(1, 2, 3))
    },
    test("Double->Long (tag 16)") {
      val p = SyncInterpreter(Reader.fromChunk(Chunk(1.0, 2.0, 3.0)))
      p.addMap[Double, Long](LANE_D, OUT_L)((_: Double).toLong)
      assertTrue(drainLongs(p) == List(1L, 2L, 3L))
    },
    test("Double->Float (tag 17)") {
      val p = SyncInterpreter(Reader.fromChunk(Chunk(1.5, 2.5)))
    ...
    test("AnyRef->Int (tag 20)") {
      val p = SyncInterpreter(Reader.fromIterable(List("hello", "ab", "x")))
    ...
    test("AnyRef->Long (tag 21)") {
      val p = SyncInterpreter(Reader.fromIterable(List("hello", "ab")))
  ...
  val wrapLastReadSuite = suite("wrapLastRead")(
    test("wrapLastRead transforms the source reader") {
      val p = SyncInterpreter.unsealed(Reader.fromRange(0 until 10))
      p.addMap[Int, Int](LANE_I, OUT_I)((_: Int) + 1)
      // Wrap the source reader to limit it to 3 elements
      p.wrapLastRead((r: Reader.SyncReader[Any]) => Reader.fromRange(0 until 3).asInstanceOf[Reader.SyncReader[Any]])
      p.seal()
    ...
    test("wrapLastRead on pipeline with no ops") {
      val p = SyncInterpreter.unsealed(Reader.fromRange(0 until 100))
      p.wrapLastRead((r: Reader.SyncReader[Any]) => Reader.fromRange(0 until 2).asInstanceOf[Reader.SyncReader[Any]])
      p.seal()
      val result = drainInts(p)
    ...
    test("wrapLastRead targets the last Read op in the pipeline") {
      ...
      val p = SyncInterpreter.unsealed(Reader.fromRange(0 until 5))
      p.addFilter[Int](LANE_I)((_: Int) % 2 == 0)
      p.wrapLastRead((_: Reader.SyncReader[Any]) =>
        Reader.fromChunk(Chunk(10, 20, 30)).asInstanceOf[Reader.SyncReader[Any]]
      )
      p.seal()
      val result = drainInts(p)
  ...
  private def failingCloseReader(ex: Throwable): Reader.SyncReader[Int] = new Reader.SyncReader[Int] {
  ...
  val closeSuppressedSuite = suite("close() suppressed exceptions")(
    test("single reader close exception is thrown") {
      ...
      val p  = SyncInterpreter(failingCloseReader(ex))
    ...
    test("multiple reader close exceptions: first thrown, rest suppressed") {
      ...
      val p = SyncInterpreter.unsealed(failingCloseReader(ex1))
  ...
  val appendReadSuite = suite("appendRead")(
    test("unsealed creates pipeline via appendRead, outputLane is Int") {
      val p = SyncInterpreter.unsealed(Reader.fromRange(0 until 3))
      p.seal()
    ...
    test("unsealed with Long reader sets outputLane to Long") {
      val p = SyncInterpreter.unsealed(Reader.fromChunk(Chunk(1L, 2L)))
      p.seal()
    ...
    test("unsealed with Double reader sets outputLane to Double") {
      val p = SyncInterpreter.unsealed(Reader.fromChunk(Chunk(1.0, 2.0, 3.0)))
      p.seal()
    ...
    test("unsealed with Float reader sets outputLane to Float") {
      val p = SyncInterpreter.unsealed(Reader.fromChunk(Chunk(1.0f, 2.0f)))
      p.seal()
    ...
    test("unsealed with AnyRef reader sets outputLane to AnyRef") {
      val p = SyncInterpreter.unsealed(Reader.fromIterable(List("a", "b")))
      p.seal()
