...
object StreamRenderSpec extends StreamsBaseSpec {
  ...
  def spec: Spec[TestEnvironment, Any] = suite("StreamRenderSpec")(
    ...
    suite("transforms")(
      ...
      test("asynchronous transforms render their exact operator names") {
        val source = Stream(1)
        assertTrue(
          Stream.unwrap(Async.succeed(source)).render == "Stream.unwrap(...)",
          source.mapAsync(n => Async.succeed(n)).render == "Stream(1).mapAsync(...)",
          source.filterAsync(_ => Async.succeed(true)).render == "Stream(1).filterAsync(...)",
          source.collectAsync(n => Async.succeed(Some(n))).render == "Stream(1).collectAsync(...)",
          source.tapEachAsync(_ => Async.succeed(())).render == "Stream(1).tapEachAsync(...)",
          source.distinctByAsync(Async.succeed).render == "Stream(1).distinctByAsync(...)",
          source.mapAccumAsync(0)((s, n) => Async.succeed((s + n, n))).render == "Stream(1).mapAccumAsync(...)",
          source.scanAsync(0)((s, n) => Async.succeed(s + n)).render == "Stream(1).scanAsync(...)",
          source.takeWhileAsync(_ => Async.succeed(true)).render == "Stream(1).takeWhileAsync(...)",
          source.mapErrorAsync((e: Nothing) => Async.succeed(e)).render == "Stream(1).mapErrorAsync(...)",
          source.ensuringAsync(Async.succeed(())).render == "Stream(1).ensuringAsync(...)"
        )
      }
    ),
    ...
    suite("error ops")(
      ...
      test("catchAll renders as .catchAll(...)") {
        assert(Stream.fail("err").catchAll[Nothing, Nothing, Nothing](_ => Stream.empty).render)(
          equalTo("Stream.fail(...).catchAll(...)")
        )
      },
      test("catchDefect renders as .catchDefect(...)") {
        assert(Stream.empty.catchDefect[Nothing, Nothing, Nothing, Nothing] { case _ => Stream.empty }.render)(
