        ...
        if (et eq JvmType.Byte) {
          new Reader[Chunk[A]] {
            ...
            def read[A1 >: Chunk[A]](sentinel: A1): A1 = {
              ...
              var i       = 0
              var v       = source.readInt(Long.MinValue)(unsafeEvidence)
              while (v != Long.MinValue && i < n) {
                builder.addOne(v.toByte)
                i += 1
                if (i < n) v = source.readInt(Long.MinValue)(unsafeEvidence)
              }
          ...
          new Reader[Chunk[A]] {
            ...
            def read[A1 >: Chunk[A]](sentinel: A1): A1 = {
              ...
              var i       = 0
              var v       = source.readLong(Long.MaxValue)(unsafeEvidence)
              while (v != Long.MaxValue && i < n) {
                builder.addOne(v)
                i += 1
                if (i < n) v = source.readLong(Long.MaxValue)(unsafeEvidence)
              }
              if (i == 0) sentinel else builder.result().asInstanceOf[A1]
            }
            def close() = source.close()
          ...
          new Reader[Chunk[A]] {
            ...
            def read[A1 >: Chunk[A]](sentinel: A1): A1 = {
              ...
              var i       = 0
              var v       = source.readFloat(Double.MaxValue)(unsafeEvidence)
              while (v != Double.MaxValue && i < n) {
                builder.addOne(v.toFloat)
                i += 1
                if (i < n) v = source.readFloat(Double.MaxValue)(unsafeEvidence)
              }
              if (i == 0) sentinel else builder.result().asInstanceOf[A1]
            }
            def close() = source.close()
          ...
          new Reader[Chunk[A]] {
            ...
            def read[A1 >: Chunk[A]](sentinel: A1): A1 = {
              ...
              var i       = 0
              var v       = source.readDouble(Double.MaxValue)(unsafeEvidence)
              while (v != Double.MaxValue && i < n) {
                builder.addOne(v)
                i += 1
                if (i < n) v = source.readDouble(Double.MaxValue)(unsafeEvidence)
              }
              if (i == 0) sentinel else builder.result().asInstanceOf[A1]
            }
            def close() = source.close()
    ...
    new Stream.FromReader[E, Chunk[A]](
      () => {
        ...
        def skipElements(count: Int): Boolean = {
          ...
          if (et eq JvmType.Byte) {
            while (i < count) {
              if (source.readInt(Long.MinValue)(unsafeEvidence) == Long.MinValue) return false; i += 1
            }
          } else if (et eq JvmType.Int) {
            while (i < count) {
              if (source.readInt(Long.MinValue)(unsafeEvidence) == Long.MinValue) return false; i += 1
            ...
            while (i < count) {
              if (source.readLong(Long.MaxValue)(unsafeEvidence) == Long.MaxValue) return false; i += 1
            }
          } else if (et eq JvmType.Float) {
            while (i < count) {
              if (source.readFloat(Double.MaxValue)(unsafeEvidence) == Double.MaxValue) return false; i += 1
            }
          } else if (et eq JvmType.Double) {
            while (i < count) {
              if (source.readDouble(Double.MaxValue)(unsafeEvidence) == Double.MaxValue) return false; i += 1
            }
          } else {
            while (i < count) {
...
object Stream {
  ...
  private[streams] final class CatchAllReader[E, A](upstream: Reader[A], f: E => Stream[Any, A]) extends Reader[A] {
    ...
    override def readInt(sentinel: Long)(implicit ev: A <:< Int): Long =
      if (!switched) {
        try current.readInt(sentinel)(unsafeEvidence)
        catch { case e: StreamError => doSwitch(e); current.readInt(sentinel)(unsafeEvidence) }
      } else current.readInt(sentinel)(unsafeEvidence)

    override def readLong(sentinel: Long)(implicit ev: A <:< Long): Long =
      if (!switched) {
        try current.readLong(sentinel)(unsafeEvidence)
        catch { case e: StreamError => doSwitch(e); current.readLong(sentinel)(unsafeEvidence) }
      } else current.readLong(sentinel)(unsafeEvidence)

    override def readFloat(sentinel: Double)(implicit ev: A <:< Float): Double =
      if (!switched) {
        try current.readFloat(sentinel)(unsafeEvidence)
        catch { case e: StreamError => doSwitch(e); current.readFloat(sentinel)(unsafeEvidence) }
      } else current.readFloat(sentinel)(unsafeEvidence)

    override def readDouble(sentinel: Double)(implicit ev: A <:< Double): Double =
      if (!switched) {
        try current.readDouble(sentinel)(unsafeEvidence)
        catch { case e: StreamError => doSwitch(e); current.readDouble(sentinel)(unsafeEvidence) }
      } else current.readDouble(sentinel)(unsafeEvidence)
  ...
  private[streams] final class CatchDefectReader[E, A](
    ...
    override def readInt(sentinel: Long)(implicit ev: A <:< Int): Long =
      if (!switched) {
        try current.readInt(sentinel)(unsafeEvidence)
        catch { case t: Throwable => if (trySwitch(t)) current.readInt(sentinel)(unsafeEvidence) else throw t }
      } else current.readInt(sentinel)(unsafeEvidence)

    override def readLong(sentinel: Long)(implicit ev: A <:< Long): Long =
      if (!switched) {
        try current.readLong(sentinel)(unsafeEvidence)
        catch { case t: Throwable => if (trySwitch(t)) current.readLong(sentinel)(unsafeEvidence) else throw t }
      } else current.readLong(sentinel)(unsafeEvidence)

    override def readFloat(sentinel: Double)(implicit ev: A <:< Float): Double =
      if (!switched) {
        try current.readFloat(sentinel)(unsafeEvidence)
        catch { case t: Throwable => if (trySwitch(t)) current.readFloat(sentinel)(unsafeEvidence) else throw t }
      } else current.readFloat(sentinel)(unsafeEvidence)

    override def readDouble(sentinel: Double)(implicit ev: A <:< Double): Double =
      if (!switched) {
        try current.readDouble(sentinel)(unsafeEvidence)
        catch { case t: Throwable => if (trySwitch(t)) current.readDouble(sentinel)(unsafeEvidence) else throw t }
      } else current.readDouble(sentinel)(unsafeEvidence)
  ...
  private[streams] final class ErrorMappedReader[E, A](upstream: Reader[A], f: E => Any) extends Reader[A] {
    ...
    override def readInt(sentinel: Long)(implicit ev: A <:< Int): Long =
      try upstream.readInt(sentinel)(unsafeEvidence)
      catch { case e: StreamError => throw new StreamError(f(e.value.asInstanceOf[E])) }
    override def readLong(sentinel: Long)(implicit ev: A <:< Long): Long =
      try upstream.readLong(sentinel)(unsafeEvidence)
      catch { case e: StreamError => throw new StreamError(f(e.value.asInstanceOf[E])) }
    override def readFloat(sentinel: Double)(implicit ev: A <:< Float): Double =
      try upstream.readFloat(sentinel)(unsafeEvidence)
      catch { case e: StreamError => throw new StreamError(f(e.value.asInstanceOf[E])) }
    override def readDouble(sentinel: Double)(implicit ev: A <:< Double): Double =
      try upstream.readDouble(sentinel)(unsafeEvidence)
      catch { case e: StreamError => throw new StreamError(f(e.value.asInstanceOf[E])) }
