...
/**
 * A type expression that can represent complex types like `List[Int]` or `Map[String, Int]`.
 * 
 * TypeRepr is the "type level AST" that describes how types are constructed.
 * This is a minimal MVP implementation - extensible for future Structural, Union, etc.
 */
...
object TypeRepr {
  ...
  /**
   * Reference to a named type (could be unapplied type constructor or proper type).
   * 
   * Examples:
   * - `Ref(intId)` for `Int`
   ...
   * 
   * Used in alias and opaque type definitions, e.g., in `type MyList[A] = List[A]`,
   * the RHS contains a ParamRef to A.
   */
   ...
   * - `Applied(Ref(mapId), List(Ref(stringId), Ref(intId)))` for `Map[String, Int]`
   */
