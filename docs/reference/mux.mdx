...
Each stream has separate inbound and outbound queues (default depth `Mux.DefaultStreamQueueCapacity`, 256 per queue; tunable via `streamQueueCapacity`, see [Mux](#mux)). If the protocol producer is faster than the consumer, the queue fills and `offerInbound` or `send` returns `QueueFull`:
...
**`QueueFull(queueCapacity: Int)`** — Returned by `send` (outbound queue full) or `offerInbound` (inbound queue full) when the per-stream message queue has exhausted its capacity (the configured `streamQueueCapacity`, 256 by default). Drain the queue (by calling `takeOutbound()` or `receive()`) to resume sending/receiving.
...
**Stream queue capacity (optional)**

Each stream carries an inbound and an outbound queue. Their depth is set once per multiplexer with the optional `streamQueueCapacity` argument (omit it to use the default `Mux.DefaultStreamQueueCapacity`, 256):

```scala
import zio.blocks.mux._

val mux = Mux[Int, String, String](capacity = 100, streamQueueCapacity = 512)
```

The depth must be a positive power of two — the backend stores each queue in a fixed ring buffer — otherwise construction throws `IllegalArgumentException`. Sizing guidance: worst-case retained memory per stream is roughly twice `streamQueueCapacity` message references, so size it at or above the maximum number of messages you expect to be in flight on one stream between polls. When a queue is full, `send`/`offerInbound` return `MuxError.QueueFull` instead of blocking (non-blocking contract): retry, shed load, or open fewer streams rather than spinning.
