...
Chunk integrates deeply with ZIO Blocks' schema system through the [Reflect](./zio-blocks-schema/reflect.md) module. When deriving schemas for collection types, `Chunk` is recognized as a key sequence type alongside `List`, `Vector`, and `Set`.
...
Chunks work naturally with the [Codec](./zio-blocks-schema/codec.md) system for serialization and deserialization:
...
The [DynamicValue](./zio-blocks-schema/dynamic-value.md) system also works with Chunk, allowing schema-driven navigation of chunk data.
