...
`Stream.fromProducer` bridges a push-based producer (running on any thread) into a
...
// JVM-only constructors on the Stream companion (streams/jvm/);
// ProducerStreams remains as a deprecated alias for compatibility.
object Stream {
  ...
  // Managed lifecycle: auto-end, defects on exception, virtual thread.
...
thread, automatically calling `end()` on normal return and surfacing thrown exceptions as
defects (rethrown). On JDK 17, `Platform.startVirtualThread` falls back to a regular daemon
thread, since virtual threads require JDK 21+.

### Defect vs typed error

Thrown exceptions from `register` / `produce` and from the producer thread are **defects**:
they are rethrown on the consumer thread and do not appear as `Left[E]`. Use
`sink.fail(E)` for the typed `Left[E]` path (`Stream[E, A]` failure). This matches the
`Stream` contract where untyped defects propagate as thrown exceptions and only `StreamError`
/ `SinkError` become `Left`.

## Usage Example

```scala
import zio.blocks.streams.{ProducerSink, Stream}

val stream: Stream[Nothing, Int] = Stream.fromProducer { sink =>
...
| `fail(e)`       | signals typed error `Left[E]` | no-op |
...
- `emit` after any terminal state returns `false` — the producer should treat this as a signal
  to stop work.
- Thrown exceptions from the producer are defects (rethrown), not `Left[E]`; call
  `sink.fail(e)` for typed errors.
- The cancel callback is invoked exactly once, off the consumer thread (to avoid blocking
  close), during early termination. It is **not** called when the stream ends normally via
  `end()`.
...
```scala
Stream.fromProducer(register, knownLength = Some(fileSize))
```
...
- On consumer cancel, `queue.close()` is called, which unblocks both the producer and consumer
  so they can observe the cancellation and exit cleanly. The cancel callback is dispatched
  off-thread so `close()` never blocks on user code.
...
val byteStream: Stream[Nothing, Byte] = Stream.fromProducerBytesSimple(
  produce = { sink =>
    var active = true
