...
Applies a function to every element, producing a new element type.
...
Applies a partial function: only elements for which the function is defined pass through, and they are transformed to the output type. This combines filtering and mapping in one step.
...
Passes through at most the first `n` elements, then stops.
...
// Right(Chunk(0, 1, 2, 3, 4))
```

### `Pipeline.drop[A]` — Skip First N Elements

Skips the first `n` elements, then passes through the rest.
...
// Right(Chunk("row1", "row2"))
```

### `Pipeline.identity[A]` — Pass-Through

The identity pipeline that passes all elements through unchanged. This is the neutral element for `andThen` composition.
...
Composes two pipelines into one, applying `this` first and `that` second.
...
val result = Stream(-5, 3, -1, 7, 2, 0, 9, 4, 8, 6, 1, 10)
  .via(normalize)
  .runCollect
// Right(Chunk(6, 14, 4, 18, 8, 16, 12, 2, 20))
```
...
The primary way to use a pipeline is through `Stream.via`:
...
You can also call `applyToStream` directly. This is equivalent to `via` but reads left-to-right from the pipeline's perspective:
...
The dual of `via`: instead of transforming a stream's output, you pre-process a sink's input.
...
`andThenSink` is an alias for `applyToSink`. Prefer `andThenSink` for readability.
...
This example demonstrates all six Pipeline factory methods: `map`, `filter`, `collect`, `take`, `drop`, and `identity`.
...
This example shows how to compose pipelines with `andThen`, apply them to multiple streams, and build pipelines conditionally.
...
This example demonstrates applying pipelines to sinks with `andThenSink`, showing the equivalence between `stream.via(pipe).run(sink)` and `stream.run(pipe.andThenSink(sink))`.
