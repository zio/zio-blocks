...
+- **Threading models**: Ring buffers work on raw JVM threads, virtual threads (Loom), or platform-specific threads. The virtual-thread path falls back through a `MethodHandle` when `Thread.ofVirtual()` is unavailable. Pair with `ZIO.fork` or `Thread` as needed.
