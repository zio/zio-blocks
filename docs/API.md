...
**Note on Variance:** The trait is defined as `Into[A, B]` (invariant) rather than `Into[-A, +B]` to ensure type safety with macro derivation. Variance annotations can cause issues with macro type inference and are not necessary for the conversion semantics.
