...
pulls them through the stream interface. A lock-free `SpscRingBuffer` (VarHandle-based) sits
between the two, with spin-wait + park blocking wrappers: the producer parks when the buffer
is full and the consumer parks when it's empty.

**SPSC contract:** Callers of `fromProducer` must ensure that only one thread calls
`emit` at a time. The internal `SpscRingBuffer` is a single-producer single-consumer data
...
trait ProducerSink[-A, -E] {
  ...
  def emit(chunk: Chunk[A]): Boolean // emit a whole chunk; false on close
...
`fromProducerSimple` uses `Thread.ofVirtual()` to run the producer on a virtual thread,
automatically calling `end()` on normal return and `fail()` on exception.
...
- **Producer thread**: pushes elements via `ProducerSink`. Parks (via `LockSupport.parkNanos`)
...
- On consumer cancel, a **poison pill** is offered to unblock the consumer's `blockingTake()`,
  and `close()` also unparks the producer thread so it can observe the cancellation.
...
reference, and bytes are pulled without boxing via `readByte()` and `readBytes()`.
