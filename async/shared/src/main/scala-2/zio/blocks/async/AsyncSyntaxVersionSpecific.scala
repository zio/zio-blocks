...
private[async] trait AsyncSyntaxVersionSpecific {
  ...
  implicit class AsyncOps[A](private val fa: Async[A]) {
    ...
    def ensuring(finalizer: Async[Any]): Async[A] = {
      ...
      if (r.isInstanceOf[Pollable[_]])
        ...
        else if (fin.isInstanceOf[Pollable[_]])
     ...
     * `Out` values, so we skip the per-value `left`/`right` projections (no
     * `map` wrappers) and just recover with a cast — for a ready `fa` this
     * collapses to a no-op. Only the disjoint `Either` widening actually
     * projects each side through `c.left` / `c.right`.
     */
    def orElse[B](that: => Async[B])(implicit c: Concat[A, B]): Async[c.Out] =
      if (c.isIdentityLike)
    ...
    def foldCause[B](onFailure: Throwable => B)(onSuccess: A => B): Async[B] = {
      ...
      if (r.isInstanceOf[Failure])
        ...
        Async.slowPath.foldCauseAsync[A, B](r, onFailure, onSuccess)
      } else {
        val a = AsyncEncoding.deliverSuccess[A](r)
  ...
  implicit class AsyncFlattenOps[A](private val ffa: Async[Async[A]]) {
    ...
    def flatten: Async[A] = {
      ...
      if (any.isInstanceOf[Pollable[_]]) Async.slowPath.flattenAsync[A](any)
