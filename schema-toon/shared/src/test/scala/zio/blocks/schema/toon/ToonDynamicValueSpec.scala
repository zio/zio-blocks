 ...
 * - encode: for types that encode correctly but may not decode to exact same type
 */
object ToonDynamicValueSpec extends ZIOSpecDefault {

  private def roundTrip(value: DynamicValue)(implicit deriver: ToonBinaryCodecDeriver = ToonBinaryCodecDeriver): TestResult = {
  ...
  private def encode(value: DynamicValue, contains: String)(implicit deriver: ToonBinaryCodecDeriver = ToonBinaryCodecDeriver): TestResult = {
    val codec = Schema.dynamic.derive(deriver)
    val encoded = codec.encodeToString(value)
  ...
  def spec = suite("ToonDynamicValueSpec")(
    suite("DynamicValue encoding/decoding")(
      ...
      test("record types roundTrip") {
        roundTrip(DynamicValue.Record(Vector(
      ...
      test("sequence types roundTrip") {
        roundTrip(DynamicValue.Sequence(Vector(
