...
object ContextStorageSpec extends ZIOSpecDefault {
  ...
  def spec: Spec[Any, Nothing] = suite("ContextStorage")(
    ...
    suite("set")(
      test("updates value visible to get") {
        val storage = ContextStorage.create("initial")
        storage.set("updated")
        assertTrue(storage.get() == "updated")
      },
      test("set multiple times returns last value") {
        val storage = ContextStorage.create(0)
        storage.set(1)
        storage.set(2)
        storage.set(3)
        assertTrue(storage.get() == 3)
      }
    ),
    suite("scoped")(
