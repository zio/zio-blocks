...
object BranchCoverageSpec extends ZIOSpecDefault {
  ...
  def spec = suite("BranchCoverage")(
    severitySuite, spanSuite, tracerSuite, meterSuite,
    loggerSuite, attributesSuite, b3Suite, w3cSuite, ctxSuite, batchSuite
  )

  private val severitySuite = suite("Severity.fromNumber all arms")(
    test("fromNumber covers all 24 levels and invalid") {
      val expected = Seq(
        (1, Some(Severity.Trace)), (2, Some(Severity.Trace2)), (3, Some(Severity.Trace3)), (4, Some(Severity.Trace4)),
        (5, Some(Severity.Debug)), (6, Some(Severity.Debug2)), (7, Some(Severity.Debug3)), (8, Some(Severity.Debug4)),
        (9, Some(Severity.Info)), (10, Some(Severity.Info2)), (11, Some(Severity.Info3)), (12, Some(Severity.Info4)),
        (13, Some(Severity.Warn)), (14, Some(Severity.Warn2)), (15, Some(Severity.Warn3)), (16, Some(Severity.Warn4)),
        (17, Some(Severity.Error)), (18, Some(Severity.Error2)), (19, Some(Severity.Error3)), (20, Some(Severity.Error4)),
        (21, Some(Severity.Fatal)), (22, Some(Severity.Fatal2)), (23, Some(Severity.Fatal3)), (24, Some(Severity.Fatal4)),
        (0, None), (25, None), (-1, None)
      )
      assertTrue(expected.forall { case (n, e) => Severity.fromNumber(n) == e })
    },
    test("all severity sub-level number and text") {
      val all = Seq(Severity.Trace2, Severity.Trace3, Severity.Trace4, Severity.Debug2, Severity.Debug3, Severity.Debug4,
        Severity.Info2, Severity.Info3, Severity.Info4, Severity.Warn2, Severity.Warn3, Severity.Warn4,
        Severity.Error2, Severity.Error3, Severity.Error4, Severity.Fatal2, Severity.Fatal3, Severity.Fatal4)
  ...
  private def mkSampler(decision: SamplingDecision, attrs: Attributes, ts: String = ""): Sampler = new Sampler {
    def shouldSample(pc: Option[SpanContext], tid: TraceId, n: String, k: SpanKind, a: Attributes, l: Seq[SpanLink]): SamplingResult =
  ...
  private val tracerSuite = suite("Tracer branch coverage")(
    test("RecordOnly with all attribute types") {
      val attrs = Attributes.builder.put(AttributeKey.string("s"), "v").put(AttributeKey.long("l"), 1L).put(AttributeKey.double("d"), 2.0).put(AttributeKey.boolean("b"), true).build
      val processor = new TestProcessor
      val tracer = TracerProvider.builder.setSampler(mkSampler(SamplingDecision.RecordOnly, attrs)).addSpanProcessor(processor).build().get("t")
      val spanAttrs = Attributes.builder.put(AttributeKey.string("as"), "av").put(AttributeKey.long("al"), 10L).put(AttributeKey.double("ad"), 20.0).put(AttributeKey.boolean("ab"), false).build
      tracer.span("ro", SpanKind.Internal, spanAttrs)(span => assertTrue(span.isRecording))
    },
    test("RecordAndSample with all attribute types") {
      val attrs = Attributes.builder.put(AttributeKey.string("s"), "v").put(AttributeKey.long("l"), 1L).put(AttributeKey.double("d"), 2.0).put(AttributeKey.boolean("b"), true).build
      val processor = new TestProcessor
      val tracer = TracerProvider.builder.setSampler(mkSampler(SamplingDecision.RecordAndSample, attrs)).addSpanProcessor(processor).build().get("t")
      val spanAttrs = Attributes.builder.put(AttributeKey.string("as"), "av").put(AttributeKey.long("al"), 10L).put(AttributeKey.double("ad"), 20.0).put(AttributeKey.boolean("ab"), false).build
      tracer.span("ras", SpanKind.Internal, spanAttrs)(span => assertTrue(span.isRecording))
    },
    test("RecordOnly with seq attribute hits catch-all") {
      val processor = new TestProcessor
      val tracer = TracerProvider.builder.setSampler(mkSampler(SamplingDecision.RecordOnly, Attributes.builder.put(AttributeKey.stringSeq("seq"), Seq("a")).build)).addSpanProcessor(processor).build().get("t")
      tracer.span("ro-seq", SpanKind.Internal, Attributes.builder.put(AttributeKey.longSeq("lseq"), Seq(1L, 2L)).build)(span => assertTrue(span.isRecording))
    },
    test("RecordAndSample with seq attribute hits catch-all") {
      val processor = new TestProcessor
      val tracer = TracerProvider.builder.setSampler(mkSampler(SamplingDecision.RecordAndSample, Attributes.builder.put(AttributeKey.doubleSeq("seq"), Seq(1.0)).build)).addSpanProcessor(processor).build().get("t")
      tracer.span("ras-seq", SpanKind.Internal, Attributes.builder.put(AttributeKey.booleanSeq("bseq"), Seq(true)).build)(span => assertTrue(span.isRecording))
    }
  )
  ...
  private val loggerSuite = suite("Logger attribute types")(
    test("log with Boolean attribute") {
      ...
      LoggerProvider.builder.addLogRecordProcessor(p).build().get("t").info("m", "b" -> AttributeValue.BooleanValue(true))
      assertTrue(p.emitted.head.attributes.toMap("b") == AttributeValue.BooleanValue(true))
    },
    test("log with Double attribute") {
      val p = new TestLogProcessor
      LoggerProvider.builder.addLogRecordProcessor(p).build().get("t").info("m", "d" -> AttributeValue.DoubleValue(3.14))
      assertTrue(p.emitted.head.attributes.toMap("d") == AttributeValue.DoubleValue(3.14))
    },
    test("log with seq attribute hits catch-all") {
      val p = new TestLogProcessor
      LoggerProvider.builder.addLogRecordProcessor(p).build().get("t").info("m", "s" -> AttributeValue.StringSeqValue(Seq("a")))
  ...
  private val attributesSuite = suite("Attributes branch coverage")(
    test("++ merges with conflict") {
      val merged = Attributes.builder.put("k1", "v1").put("k2", "v2").build ++ Attributes.builder.put("k2", "v2b").put("k3", "v3").build
    ...
    test("get BooleanSeq via valueToType") {
      val k = AttributeKey.booleanSeq("bs"); assertTrue(Attributes.of(k, Seq(true, false)).get(k).contains(Seq(true, false)))
    },
    ...
    test("of with Long") { assertTrue(Attributes.of(AttributeKey.long("l"), 42L).get(AttributeKey.long("l")).contains(42L)) },
    test("of with Double") { assertTrue(Attributes.of(AttributeKey.double("d"), 3.14).get(AttributeKey.double("d")).contains(3.14)) },
    test("of with Boolean") { assertTrue(Attributes.of(AttributeKey.boolean("b"), true).get(AttributeKey.boolean("b")).contains(true)) }
  )
  ...
  private val ctxSuite = suite("ContextStorage branch coverage")(
    test("hasLoom matches implementationName") {
      assertTrue((ContextStorage.hasLoom && ContextStorage.implementationName == "ScopedValue") ||
    ...
    test("scoped restores on exception") {
      val s = ContextStorage.create("initial")
      try s.scoped("scoped")(throw new RuntimeException("boom")) catch { case _: RuntimeException => () }
  ...
  private val batchSuite = suite("BatchProcessor branch coverage")(
    test("enqueue below capacity does not drop") {
      ...
      val p = new BatchProcessor[String](_ => { exported = _ :: exported; ExportResult.Success },
        maxQueueSize = 10, maxBatchSize = 5, maxRetries = 1, flushIntervalMillis = 60000L)
      p.enqueue("a"); p.enqueue("b"); p.forceFlush(); p.shutdown()
