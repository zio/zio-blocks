  ...
  def spec: Spec[TestEnvironment, Any] = suite("SchemaVersionSpecificSpec")(
    suite("Reflect.Record")(
      test("derives schema for named tuples") {
        ...
        assert(NamedTuple4.schema.reflect.typeId.toString)(containsString("NamedTuple")) &&
      ...
      test("derives schema for complex named tuples") {
        ...
        assert(schema1.reflect.typeId.toString)(containsString("NamedTuple")) &&
        assert(schema1.reflect.asRecord.map(_.fields.map(_.name)))(isSome(equalTo(Vector("i", "s")))) &&
        assert(schema12.reflect.typeId.toString)(containsString("NamedTuple")) &&
        assert(schema14.reflect.typeId.toString)(containsString("NamedTuple")) &&
        assert(schema14.reflect.asRecord.map(_.fields.length))(isSome(equalTo(0))) &&
        assert(schema15.reflect.asRecord.map(_.fields.length))(isSome(equalTo(0))) &&
      ...
      test("derives schema for complex generic tuples") {
        ...
        assert(schema1.reflect.typeId.toString)(containsString("Tuple2")) &&
        assert(schema1.reflect.asRecord.map(_.fields.map(_.name)))(isSome(equalTo(Vector("_1", "_2")))) &&
        assert(schema2.reflect.asRecord.map(_.fields.map(_.name)))(isSome(equalTo(Vector("_1", "_2")))) &&
        assert(schema1.fromDynamicValue(schema1.toDynamicValue(value1)))(isRight(equalTo(value1))) &&
      ...
      test("derives schema for generic named tuples") {
        ...
        assert(NamedTupleOfIntAndString.schema.reflect.typeId.toString)(containsString("NamedTuple")) &&
        assert(record.map(_.fields.map(_.name)))(isSome(equalTo(Vector("a", "b"))))
      },
      test("derives schema for higher-kind named tuples") {
        ...
        assert(NamedTupleOfIntAndStringLists.schema.reflect.typeId.toString)(containsString("NamedTuple")) &&
        assert(record.map(_.fields.map(_.name)))(isSome(equalTo(Vector("a", "b"))))
      },
