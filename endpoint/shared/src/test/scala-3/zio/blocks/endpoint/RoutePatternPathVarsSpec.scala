...
object RoutePatternPathVarsSpec extends ZIOSpecDefault {
  ...
  def spec: Spec[Any, Nothing] = suite("RoutePattern/PathCodec ordered PathVars propagation (Scala 3)")(
    test("Method.GET / int / literal / string produces an ordered, concatenated PathVars tuple") {
      ...
      implicitly[pattern.PathVars =:= (PathVar["userId", Int] *: PathVar["postId", String] *: EmptyTuple)]
      assertCompletes
    },
    test("PathCodec ++ propagates the same ordered PathVars as the RoutePattern chain it backs") {
      ...
      implicitly[codec.PathVars =:= (PathVar["userId", Int] *: PathVar["postId", String] *: EmptyTuple)]
      assertCompletes
    },
    test("a single captured segment produces a one-element PathVars tuple end-to-end") {
      ...
      implicitly[pattern.PathVars =:= (PathVar["id", Int] *: EmptyTuple)]
      assertCompletes
    },
    test("literal-only routes produce an empty PathVars tuple end-to-end") {
    ...
    test("Method.GET / int / literal / string.unused produces an ordered tuple with a PathVar.Ignored entry") {
      ...
      implicitly[pattern.PathVars =:= (PathVar["userId", Int] *: PathVar.Ignored["postId", String] *: EmptyTuple)]
      assertCompletes
    }
  )
}
