...
final class CoordinatorImpl(id: String) extends Coordinator {
  private val workerPlan = AgentClient.plan[Worker].asInstanceOf[golem.runtime.plan.AgentClientPlan[Worker, (String, Int)]]

  private def workerMethod[In, Out](name: String): Option[ClientMethodPlan[Worker, In, Out]] =
    workerPlan.methods.collectFirst { case p if p.metadata.name == name => p.asInstanceOf[ClientMethodPlan[Worker, In, Out]] }
