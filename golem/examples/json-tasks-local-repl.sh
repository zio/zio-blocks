...
if ! command -v golem-cli >/dev/null 2>&1; then
  echo "[json-tasks-local-repl] error: golem-cli not found on PATH" >&2
  exit 1
fi
...
if [[ "$is_cloud" -eq 0 ]]; then
  ...
  if ! timeout 1 bash -lc "cat < /dev/null > /dev/tcp/$host/$port" 2>/dev/null; then
    echo "[json-tasks-local-repl] Local router not reachable at $host:$port." >&2
    echo "[json-tasks-local-repl] Start it in another terminal, then rerun:" >&2
...
if ! ( cd "$PWD" && sbt -batch -no-colors -Dsbt.supershell=false "zioGolemExamplesJS/fastLinkJS" ) >"$build_log" 2>&1; then
  cat "$build_log" >&2
  echo "[json-tasks-local-repl] sbt failed; see output above." >&2
  exit 1
fi
...
if echo "$out" | grep -F -q 'CustomError(' || \
  ...
  echo "[json-tasks-local-repl] ERROR: repl output contains an error:" >&2
  echo "$out" >&2
  exit 1
fi
