...
if ! command -v golem-cli >/dev/null 2>&1; then
  echo "[hitl-local-repl] error: golem-cli not found on PATH" >&2
  exit 1
fi
...
if [[ "$is_cloud" -eq 0 ]]; then
  ...
  if ! timeout 1 bash -lc "cat < /dev/null > /dev/tcp/$host/$port" 2>/dev/null; then
    echo "[hitl-local-repl] Local router not reachable at $host:$port." >&2
    echo "[hitl-local-repl] Start it in another terminal, then rerun:" >&2
...
if ! ( cd "$PWD" && sbt -batch -no-colors -Dsbt.supershell=false "zioGolemExamplesJS/fastLinkJS" ) >"$build_log" 2>&1; then
  cat "$build_log" >&2
  echo "[hitl-local-repl] sbt failed; see output above." >&2
  exit 1
fi

workflow_id="demo-$(date +%s)-$RANDOM"
tmp_script="$(mktemp)"
trap 'rm -f "$tmp_script"' EXIT
sed -e "s/\"demo\"/\"$workflow_id\"/g" "$script_file" > "$tmp_script"
...
if echo "$out" | grep -F -q 'CustomError(' || \
  ...
  echo "[hitl-local-repl] ERROR: repl output contains an error:" >&2
  echo "$out" >&2
  exit 1
fi
