...
# - build + wire into `.golem-apps/` (`golemWire`)
# - golem-cli as the driver (deploy + repl)
...
if [[ -f "$bundle_file" ]] && ! grep -q "__golemInternalScalaAgents" "$bundle_file"; then
  echo "[agent2agent-local-repl] Scala shim export missing from wired bundle; re-running golemWire..." >&2
  sbt -batch -no-colors -Dsbt.supershell=false "zioGolemExamplesJS/golemWire"
fi
