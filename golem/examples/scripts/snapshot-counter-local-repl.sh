...
echo "[snapshot-counter-local-repl] 1) Build Scala.js"
...
script_file="$app_dir/samples/snapshot-counter/repl-snapshot-counter.rib"

echo "[snapshot-counter-local-repl] 2) Deploy app"
( cd "$app_dir" && env -u ARGV0 golem-cli "${flags[@]}" --yes --app-manifest-path "$app_dir/golem.yaml" deploy )
echo "[snapshot-counter-local-repl] 3) Invoke via repl"
