...
import cloud.golem.data.SchemaHelpers.{singleElementSchema, singleElementValue}
...
import cloud.golem.runtime.annotations.{agentDefinition, description, mode, prompt}
import cloud.golem.runtime.annotations.DurabilityMode
import cloud.golem.runtime.macros.{AgentClientMacro, AgentImplementationMacro, AgentMacros}
import cloud.golem.runtime.plan.{AsyncMethodPlan, ClientInvocation}
import cloud.golem.tools.AgentTypeJsonEncoder
...
final class RustAgentParitySpec extends AnyFunSuite {
  ...
  test("EchoAgent combine method keeps parameter ordering and schema") {
    ...
    method.input match {
      case StructuredSchema.Tuple(elements) =>
        ...
        assert(elements.head.schema == ElementSchema.Component(cloud.golem.data.DataType.StringType))
        assert(elements(1).schema == ElementSchema.Component(cloud.golem.data.DataType.IntType))
      case other =>
  ...
  object EnglishOnly {
    @cloud.golem.runtime.annotations.languageCode("en")
  ...
  object VisionOnly {
    @cloud.golem.runtime.annotations.mimeType("image/png")
