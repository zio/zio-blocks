...
object DataInteropSpec extends ZIOSpecDefault {
  ...
  implicit val userIdSchema: Schema[UserId] = Schema[Long].transform(UserId(_), _.value)
  ...
  private def invokeDynamicToDataValue[A](schema: Schema[A], dynamic: DynamicValue): Either[Throwable, DataValue] = {
    val method =
      DataInterop.getClass.getDeclaredMethods
        .find(_.getName.contains("dynamicToDataValue"))
        .getOrElse(
          throw new RuntimeException("dynamicToDataValue method not found")
        )
    method.setAccessible(true)
    scala.util.Try(method.invoke(DataInterop, schema.reflect, dynamic).asInstanceOf[DataValue]).toEither
  }
