...
object AgentImplementationMacroImpl {
  ...
  private def buildMethodPlan(c: blackbox.Context)(
    ...
    if (isAsync) {
      q"""
        _root_.cloud.golem.runtime.plan.AsyncMethodPlan[$traitType, $inputType, $outputType](
      ...
      q"""
        _root_.cloud.golem.runtime.plan.SyncMethodPlan[$traitType, $inputType, $outputType](
          metadata = $methodMetadataExpr,
          inputSchema = $inputSchemaExpr,
          outputSchema = $outputSchemaInstance,
          handler = $handlerExpr
  ...
  private def multiParamSchemaExpr(c: blackbox.Context)(
    ...
    q"""
      new _root_.cloud.golem.data.GolemSchema[List[Any]] {
        ...
        override def encode(value: List[Any]): Either[String, _root_.cloud.golem.data.StructuredValue] = {
        ...
        override def decode(value: _root_.cloud.golem.data.StructuredValue): Either[String, List[Any]] =
          value match {
