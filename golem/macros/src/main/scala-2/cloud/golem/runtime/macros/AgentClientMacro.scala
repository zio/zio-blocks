...
object AgentClientMacroImpl {
  def planImpl[Trait: c.WeakTypeTag](c: blackbox.Context): c.Expr[AgentClientPlan[Trait, _]] = {
    ...
    c.Expr[AgentClientPlan[Trait, _]](q"""
      _root_.cloud.golem.runtime.plan.AgentClientPlan[$traitType, $constructorType](
        ...
        traitName = $traitName,
        constructor = $constructorPlanExpr.asInstanceOf[_root_.cloud.golem.runtime.plan.ConstructorPlan[$constructorType]],
  ...
  private def buildConstructorPlan(c: blackbox.Context)(traitType: c.universe.Type): (c.universe.Type, c.Tree) = {
    ...
    val planExpr = q"_root_.cloud.golem.runtime.plan.ConstructorPlan[$inputType]($schemaExpr)"
    (inputType, planExpr)
  }
  ...
  private def buildMethodPlan(
    ...
    q"""
      _root_.cloud.golem.runtime.plan.ClientMethodPlan[$traitType, $inputType, $outputType](
  ...
  private def methodInputSchema(c: blackbox.Context)(method: c.universe.MethodSymbol): c.Tree = {
    ...
    if (params.isEmpty) {
      ...
      val elements = params.map { case (name, tpe) =>
        ...
        q"_root_.cloud.golem.data.NamedElementSchema(${name.toString}, $schemaExpr)"
      }
  ...
  private def multiParamSchemaExpr(
    ...
    q"""
      new _root_.cloud.golem.data.GolemSchema[Vector[Any]] {
        private val params = Array[(String, _root_.cloud.golem.data.GolemSchema[Any])](..$paramEntries)

        override val schema: _root_.cloud.golem.data.StructuredSchema = {
          val builder = List.newBuilder[_root_.cloud.golem.data.NamedElementSchema]
          var idx = 0
          while (idx < params.length) {
            val (paramName, codec) = params(idx)
            codec.schema match {
              case _root_.cloud.golem.data.StructuredSchema.Tuple(elements) if elements.length == 1 =>
                builder += _root_.cloud.golem.data.NamedElementSchema(paramName, elements.head.schema)
              case other =>
          ...
          _root_.cloud.golem.data.StructuredSchema.Tuple(builder.result())
        }

        override def encode(value: Vector[Any]): Either[String, _root_.cloud.golem.data.StructuredValue] = {
          if (value.length != params.length)
            ...
            val builder = List.newBuilder[_root_.cloud.golem.data.NamedElementValue]
            var idx = 0
            ...
            while (idx < params.length && error.isEmpty) {
              ...
              codec.encode(value(idx)) match {
                ...
                case Right(_root_.cloud.golem.data.StructuredValue.Tuple(elements)) =>
                  elements match {
                    case _root_.cloud.golem.data.NamedElementValue(_, elementValue) :: Nil =>
                      builder += _root_.cloud.golem.data.NamedElementValue(paramName, elementValue)
                    case _ =>
            ...
            error.fold[Either[String, _root_.cloud.golem.data.StructuredValue]](
              Right(_root_.cloud.golem.data.StructuredValue.Tuple(builder.result()))
            )(Left(_))
          }
        }

        override def decode(value: _root_.cloud.golem.data.StructuredValue): Either[String, Vector[Any]] =
          value match {
            case _root_.cloud.golem.data.StructuredValue.Tuple(elements) =>
              if (elements.length != params.length)
                ...
                while (idx < params.length && error.isEmpty) {
                  ...
                  if (element.name != paramName)
                    ...
                    codec.decode(_root_.cloud.golem.data.StructuredValue.single(element.value)) match {
                      case Left(err) =>
