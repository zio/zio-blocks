...
import cloud.golem.data.{GolemSchema, NamedElementSchema, StructuredSchema}
import cloud.golem.runtime.annotations.{description, mode, prompt}
...
object AgentDefinitionMacro {
  ...
  private def impl[T: Type](using Quotes): Expr[AgentMetadata] = {
    ...
    val agentTypeName =
      agentDefinitionTypeName(typeSymbol).getOrElse {
  ...
  private def elementSchemaExpr(using
    Quotes
  )(paramName: String, tpe: quotes.reflect.TypeRepr): Expr[cloud.golem.data.ElementSchema] = {
    import quotes.reflect.*
