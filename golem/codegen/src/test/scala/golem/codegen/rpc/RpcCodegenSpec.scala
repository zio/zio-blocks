...
class RpcCodegenSpec extends munit.FunSuite {
  ...
  test("generates multiple companions") {
    val result = RpcCodegen.generate(
      agents = List(
        ...
        agent("b.AgentB", "b", "AgentB", params = List(ParamSurface("name", "String")),
  ...
  test("Constructor and bindRemote are private") {
    val result = RpcCodegen.generate(
      agents = List(agent("demo.Test", "demo", "Test",
        methods = List(MethodSurface("foo", Nil, "Future[Unit]", Nil)))),
      existingObjects = Seq.empty
    )
  ...
  test("generates XRemote trait with per-method classes") {
    ...
    assert(content.contains("def apply(id: Int, msg: String): _root_.scala.concurrent.Future[String]"), s"missing apply in:\n$content")
    assert(content.contains("def trigger(id: Int, msg: String): _root_.scala.concurrent.Future[_root_.scala.Unit]"), s"missing trigger in:\n$content")
    assert(content.contains("def scheduleAt(id: Int, msg: String, when: _root_.golem.Datetime)"), s"missing scheduleAt in:\n$content")
  }
  ...
  test("no-param method generates apply/trigger/scheduleAt with no args") {
    ...
    assert(content.contains("def trigger(): _root_.scala.concurrent.Future[_root_.scala.Unit]"), s"missing trigger() in:\n$content")
  ...
  test("multi-param method packs args as Vector") {
    val result = RpcCodegen.generate(
      agents = List(agent(
        "example.MyAgent", "example", "MyAgent",
        methods = List(MethodSurface("put", List(ParamSurface("key", "String"), ParamSurface("value", "Int")), "Future[Unit]", List(false, false)))
      )),
      existingObjects = Seq.empty
    )
  ...
  test("principal params are filtered out from remote methods") {
    val result = RpcCodegen.generate(
      agents = List(agent(
        "example.MyAgent", "example", "MyAgent",
        methods = List(
          MethodSurface("process", List(ParamSurface("caller", "Principal"), ParamSurface("data", "String")), "Future[String]", List(true, false))
        )
      )),
      existingObjects = Seq.empty
    )
  ...
  test("all-principal method generates no-arg apply") {
    ...
    val content = result.files.head.content
    assert(content.contains("def apply(): _root_.scala.concurrent.Future[String]"), s"missing no-arg apply in:\n$content")
  ...
  test("mode-aware constructors use correct resolve methods") {
    ...
    assert(content.contains("AgentClientRuntime.resolveWithConfig["), s"getWithConfig should use resolveWithConfig:\n$content")
    assert(content.contains("resolveWithPhantom["), s"getPhantom should use resolveWithPhantom:\n$content")
    assert(content.contains("resolveWithPhantomAndConfig["), s"getPhantomWithConfig should use resolveWithPhantomAndConfig:\n$content")
  }
  ...
  test("no mode-aware constructors when no methods") {
    ...
    assert(!content.contains("def getWithConfig("), s"should not have mode-aware constructors without methods:\n$content")
