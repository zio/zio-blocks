...
object RpcCodegen {
  ...
  private def generateCompanion(agent: AgentSurface): String = {
    ...
    // type Constructor (private — implementation detail)
    sb.append(s"  private type Constructor = $constructorType\n\n")

    // typeName
    sb.append(s"""  val typeName: _root_.scala.Predef.String = "$typeName"\n\n""")

    // agentType
    sb.append(s"  lazy val agentType: _root_.golem.runtime.agenttype.AgentType[$simpleName, Constructor] =\n")
    sb.append(s"    _root_.golem.runtime.rpc.AgentClient.agentType[$simpleName]\n")
    sb.append(s"      .asInstanceOf[_root_.golem.runtime.agenttype.AgentType[$simpleName, Constructor]]\n\n")
  ...
  private def generateModeAwareConstructors(
    ...
    def resolveCall(phantom: Option[String], config: Option[String]): String =
      (phantom, config) match {
        case (None, None) =>
          s"_root_.golem.runtime.rpc.AgentClientRuntime.resolve[$simpleName, Constructor](agentType, $packExpr)"
        case (Some(ph), None) =>
          s"_root_.golem.runtime.rpc.AgentClientRuntime.resolveWithPhantom[$simpleName, Constructor](\n" +
