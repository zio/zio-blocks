    ...
    extends BinaryFormat(
      ...
      new Deriver[BinaryCodec] {
        override def derivePrimitive[F[_, _], A](
          ...
          Lazy({
            val avroSchema = AvroSchemaCodec.toAvroSchema(
              new Schema(
                Reflect.Primitive(
            ...
            primitiveType match {
              ...
              case _ =>
                new AvroBinaryCodec(avroSchema)
            }
        ...
        override def deriveRecord[F[_, _], A](
          ...
          Lazy {
            ...
            val schema     = new Schema(record)
            val avroSchema = AvroSchemaCodec.toAvroSchema(schema)
            binding match {
              case Binding.Record(constructor, deconstructor, _, _) =>
                val fieldCodecs = new Array[AvroBinaryCodec[?, ?]](fields.length)
                var idx         = -1
                fields.foreach { field =>
                  idx += 1
                  val reflect = field.value
                  fieldCodecs(idx) = if (reflect.isPrimitive) {
                    val primitiveReflect = reflect.asPrimitive.get.asInstanceOf[Reflect.Primitive[F, Any]]
                    this
                      .derivePrimitive(
                        primitiveReflect.primitiveType,
                        primitiveReflect.typeName,
                        primitiveReflect.binding,
                        primitiveReflect.doc,
                        primitiveReflect.modifiers
                      )
                      .force
                      .asInstanceOf[AvroBinaryCodec[?, ?]]
                  } else if (reflect.isRecord) {
                    val recordReflect = reflect.asRecord.get.asInstanceOf[Reflect.Record[F, Any]]
                    this
                      .deriveRecord(
                        recordReflect.fields,
                        recordReflect.typeName,
                        recordReflect.binding,
                        recordReflect.doc,
                        recordReflect.modifiers
                      )
                      .force
                      .asInstanceOf[AvroBinaryCodec[?, ?]]
                  } else ???
                }
                new AvroBinaryCodec[A, IndexedRecord](
                  avroSchema,
                  (a: A) => {
                      ...
                      if (reflect.isPrimitive) {
                        ...
                        primitiveType match {
                          ...
                          case _: PrimitiveType.Boolean   =>
                            builder
                              .set(fieldName, encoder.asInstanceOf[Boolean => AnyRef](registers.getBoolean(offset, 0)))
                          ...
                          case _: PrimitiveType.Double =>
                            builder
                              .set(fieldName, encoder.asInstanceOf[Double => AnyRef](registers.getDouble(offset, 0)))
                          ...
                          case _ =>
                            builder
                              .set(fieldName, encoder.asInstanceOf[AnyRef => AnyRef](registers.getObject(offset, 0)))
                    ...
                    fields.foreach { field =>
                      ...
                      if (reflect.isPrimitive) {
                        ...
                        primitiveType match {
                          ...
                          case _ =>
                            registers
                              .setObject(offset, 0, decoder.asInstanceOf[AnyRef => AnyRef](indexedRecord.get(idx)))
                            offset = RegisterOffset.add(offset, RegisterOffset(objects = 1))
                        }
                      } else {
              ...
              case _ => sys.error("expected record binding")
            }
          }
...
private case class AvroBinaryCodec[A, B](
  ...
  encoder: A => B = (x: A) => x.asInstanceOf[B],
  decoder: B => A = (x: B) => x.asInstanceOf[A]
) extends BinaryCodec[A] {
  private[this] val reader = new GenericDatumReader[B](schemaAvro)
  private[this] val writer = new GenericDatumWriter[B](schemaAvro)
