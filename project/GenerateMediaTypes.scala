...
object GenerateMediaTypes {
  ...
  case class MediaTypeInfo(
    ...
    def render: String = {
      val extList = if (extensions.isEmpty) "" 
    ...
    while (remaining.nonEmpty) {
      ...
      if (keyStart < 0) {
        ...
        if (objStart >= 0) {
          ...
          val compressible = if (objJson.contains("\"compressible\":true") || objJson.contains("\"compressible\": true")) Some(true)
                            else if (objJson.contains("\"compressible\":false") || objJson.contains("\"compressible\": false")) Some(false)
                            else None
