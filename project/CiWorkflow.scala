 ...
 * deploy-preview.yml is generated by the plugin itself (`ciEnableDeployPreview`
 * in build.sbt) rather than from here; it consumes the `website-artifact` and
 * `pr-metadata` artifacts uploaded by buildDocs, and matches this workflow by
 * its name, "CI"; both are load-bearing.
 */
object CiWorkflow {
