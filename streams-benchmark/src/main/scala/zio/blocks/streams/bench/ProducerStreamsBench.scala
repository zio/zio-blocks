    ...
    zbByteFold(
      ProducerStreams.fromProducerBytesSimple[Nothing](
        produce = { sink =>
          ...
          var emitted = 0
          while (emitted + chunkSize <= N) { sink.emit(chunk); emitted += chunkSize }
          val remaining = N - emitted
          if (remaining > 0) sink.emit(Chunk.fromArray(Array.fill[Byte](remaining)(1)))
        },
        bufferSize = 256
      )
    )
