  ...
  def spec: Spec[TestEnvironment, Any] = suite("Header")(
    suite("ContentType header")(
      test("parse round-trip for application/json") {
        ...
        assertTrue(
          result == Right(Header.ContentType(zio.http.ContentType(MediaTypes.application.`json`))),
      ...
      test("parse round-trip for text/plain with charset") {
        ...
        assertTrue(
          result == Right(
            Header.ContentType(zio.http.ContentType(MediaTypes.text.`plain`, charset = Some(Charset.UTF8)))
          ),
      ...
      test("render delegates to ContentType.render") {
        val h = Header.ContentType(zio.http.ContentType(MediaTypes.application.`json`))
