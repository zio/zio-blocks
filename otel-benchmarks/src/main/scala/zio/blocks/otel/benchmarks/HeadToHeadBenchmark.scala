...
/**
 * Head-to-head benchmark: zio-blocks-otel vs Scribe. Both write to files using
 * their native file writers. 1000 messages per invocation (matches Scribe's own
 * benchmark).
 *
...
class HeadToHeadBenchmark {
  ...
  def setup(): Unit = {
    ...
    // --- zio-blocks-otel: install with TextLogFormatter + FileLogWriter ---
