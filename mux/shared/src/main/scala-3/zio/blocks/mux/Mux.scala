...
trait Mux[Id, In, Out] {
   ...
   *   Stream identifier
   * @return
   *   A new MuxStream or an error (e.g., capacity exceeded, mux closed)
   */
...
trait MuxStream[Id, In, Out] {
   ...
   * available yet. Returns an error if the stream is closed.
   *
   * @return
   ...
   *   Message to deliver
   * @return
   *   Success or an error if the stream is closed
   */
   ...
   *
   * @return
   *   Some(msg) if a message is available, None if empty, or an error if closed
   */
