 ...
 * -------------------------------------------- This suite verifies the
 * mathematical correctness of the codec. Instead of checking specific values
 * (e.g. 1, "abc"), it generates random usage scenarios (Fuzz Testing) to find
 * edge cases. Principle: decode(encode(x)) == x (Round Trip Law)
 */
