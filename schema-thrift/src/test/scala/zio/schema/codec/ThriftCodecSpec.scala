...
import zio.schema.{CaseSet, DeriveSchema, DynamicValue, Schema, StandardType, TypeId, DeriveGen}
...
/**
 * Updated for ZIO Schema V2 (1.6.1+) - Full 1200+ Lines Integrated
 */
object ThriftCodecSpec extends ZIOSpecDefault {

  import Schema._

  // --- Local Helper to fix missing SchemaGen and DynamicValueGen (Sized fixed) ---
  object DynamicValueGen {
    def anyPrimitiveDynamicValue[A](standardType: StandardType[A]): Gen[Sized, DynamicValue] =
      DeriveGen.gen(Schema.Primitive(standardType)).map(v => DynamicValue.Primitive(v, standardType))

    def anyDynamicValueOfSchema[A](schema: Schema[A]): Gen[Sized, DynamicValue] =
      DeriveGen.gen(schema).map(schema.toDynamic)

    def anyDynamicTupleValue[A, B](schemaA: Schema[A], schemaB: Schema[B]): Gen[Sized, DynamicValue] =
      for {
        a <- anyDynamicValueOfSchema(schemaA)
        b <- anyDynamicValueOfSchema(schemaB)
  ...
  object SchemaGen {
    val anyRecursiveTypeAndValue: Gen[Sized, (Schema[Any], Any)] =
      Gen.const((schemaIntList.asInstanceOf[Schema[Any]], IntList(List(1, 2, 3)).asInstanceOf[Any]))

    val anyDeepRecursiveTypeAndValue: Gen[Sized, (Schema[Any], Any)] =
      Gen.const(
        (
          sequenceOfProductSchema.asInstanceOf[Schema[Any]],
          SequenceOfProduct("test", List(Record("R", 1)), RichSum.LongWrapper(10L)).asInstanceOf[Any]
        )
      )

    case class JsonLike(name: String, age: Int, active: Boolean)
    object JsonLike {
      implicit val schema: Schema[JsonLike] = DeriveSchema.gen[JsonLike]
    }

    object Json {
      val schema: Schema[Any] = JsonLike.schema.asInstanceOf[Schema[Any]]
    }

    val anyRecord: Gen[Sized, Schema[Any]] = Gen.const(Record.schemaRecord.asInstanceOf[Schema[Any]])
  }
    ...
    suite("Should successfully encode and decode")(
      ...
      test("recursive data types") {
        check(SchemaGen.anyRecursiveTypeAndValue) { case (schema, value) =>
          for {
            ed  <- encodeAndDecode(schema.asInstanceOf[Schema[Any]], value)
            ed2 <- encodeAndDecodeNS(schema.asInstanceOf[Schema[Any]], value)
          } yield assert(ed)(equalTo(Chunk(value))) && assert(ed2)(equalTo(value))
        }
      },
  ...
  val complexEitherSchema: Schema.Either[Record, OneOf]       = Schema.Either(Record.schemaRecord, schemaOneOf)
  val complexEitherSchema2: Schema.Either[MyRecord, MyRecord] = Schema.Either(myRecord, myRecord)
  ...
  val complexFallbackSchema: Schema.Fallback[Record, OneOf]       = Schema.Fallback(Record.schemaRecord, schemaOneOf)
  val complexFallbackSchema2: Schema.Fallback[MyRecord, MyRecord] = Schema.Fallback(myRecord, myRecord)
  ...
  def encode[A](schema: Schema[A], input: A): ZIO[Any, Nothing, Chunk[Byte]] =
    ZStream.succeed(input).via(ThriftCodec.thriftCodec(schema).streamEncoder).run(ZSink.collectAll)
  ...
  def decode[A](schema: Schema[A], hex: String): ZIO[Any, DecodeError, Chunk[A]] =
    ZStream.fromChunk(fromHex(hex)).via(ThriftCodec.thriftCodec(schema).streamDecoder).run(ZSink.collectAll)
  ...
  def encodeAndDecodeNS[A](schema: Schema[A], input: A, print: Boolean = false): ZIO[Any, DecodeError, A] = {
    val _ = print
    ZIO
      .succeed(ThriftCodec.thriftCodec(schema).encode(input))
  ...
  def encodeAndDecodeNS[A](encodeSchema: Schema[A], decodeSchema: Schema[A], input: A): ZIO[Any, DecodeError, A] =
    ZIO
      .succeed(ThriftCodec.thriftCodec(encodeSchema).encode(input))
