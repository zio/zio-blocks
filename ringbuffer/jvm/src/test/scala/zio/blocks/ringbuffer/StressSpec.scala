 ...
 * blocking / non-blocking usage. Every test verifies:
...
object StressSpec extends ZIOSpecDefault {
  ...
  def spec = suite("StressSpec")(
    ...
    suite("5. Blocking stress tests")(
      test("BlockingSpscRingBuffer: 100K items through capacity=4") {
        ZIO.attemptBlocking {
          val count       = 100_000
          val capacity    = 4
          val rb          = new BlockingSpscRingBuffer[java.lang.Integer](capacity)
          val expectedSum = count.toLong * (count - 1) / 2
      ...
      test("BlockingMpscRingBuffer: 4P + 1C, 100K items, capacity=8") {
        ZIO.attemptBlocking {
          val numProducers     = 4
          val itemsPerProducer = 25_000
          val totalItems       = numProducers * itemsPerProducer
          val capacity         = 8
          val rb               = new BlockingMpscRingBuffer[java.lang.Long](capacity)
          val expectedSum      = {
            var s = 0L
      ...
      test("BlockingSpmcRingBuffer: 1P + 4C, 100K items, capacity=8") {
        ZIO.attemptBlocking {
          val numConsumers = 4
          val totalItems   = 100_000
          val capacity     = 8
          val rb           = new BlockingSpmcRingBuffer[java.lang.Long](capacity)
          val expectedSum  = totalItems.toLong * (totalItems - 1) / 2
      ...
      test("BlockingMpmcRingBuffer: 4P + 4C, 50K items, capacity=8") {
        ZIO.attemptBlocking {
          val numProducers = 4
          val numConsumers = 4
          val totalItems   = 50_000
          val capacity     = 8
          val rb           = new BlockingMpmcRingBuffer[java.lang.Long](capacity)
    ...
    suite("6. Mixed blocking + non-blocking")(
      test("BlockingMpmcRingBuffer: mixed offer/take and tryOffer/tryTake, 4P + 4C, 50K items") {
        ZIO.attemptBlocking {
          val numProducers = 4
          val numConsumers = 4
          val totalItems   = 50_000
          ...
          // Even-numbered producers use offer() (blocking), odd-numbered use tryOffer()
          val producers = (0 until numProducers).map { pId =>
            val start = pId.toLong * itemsPerProducer
            val end   = if (pId == numProducers - 1) totalItems.toLong else start + itemsPerProducer
            new Thread(
              () => {
                ...
                if (pId % 2 == 0) {
                  ...
                  while (i < end) {
                    if (rb.tryOffer(java.lang.Long.valueOf(i))) {
                      i += 1
                    } else {
                      Thread.onSpinWait()
                    }
              ...
              s"stress-mixed-p-$pId"
            )
          }

          // Even-numbered consumers use tryTake(), odd-numbered use tryTake()
          val consumers = (0 until numConsumers).map { cId =>
            new Thread(
              () => {
                var localSum   = 0L
                ...
                var running    = true
                if (cId % 2 == 0) {
                  // Blocking consumer: use tryTake() but must stop when all items consumed
                  while (running) {
                    val v = rb.tryTake()
                    if (v != null) {
                      localSum += v.longValue()
