...
object SpscRingBufferSpec extends ZIOSpecDefault {

  def spec = suite("SpscRingBuffer")(
    suite("constructor")(
      test("capacity 0 throws") {
        assertTrue(throws(new SpscRingBuffer[String](0)))
      },
      test("capacity 3 (non-power-of-2) throws") {
        assertTrue(throws(new SpscRingBuffer[String](3)))
      },
      test("negative capacity throws") {
        assertTrue(throws(new SpscRingBuffer[String](-1)))
      },
      test("capacity 1 succeeds") {
        val rb = new SpscRingBuffer[String](1)
        assertTrue(rb.capacity == 1)
      },
      test("capacity 2 succeeds") {
        val rb = new SpscRingBuffer[String](2)
    ...
    suite("single-element offer/take")(
      test("offer then take returns the same element") {
        val rb      = new SpscRingBuffer[String](4)
    ...
    suite("FIFO ordering")(
      test("elements are taken in offer order") {
        val rb    = new SpscRingBuffer[String](8)
      ...
      test("partial fill and drain preserves order") {
        val rb    = new SpscRingBuffer[String](16)
    ...
    suite("capacity=1 edge case")(
      test("offer and take with capacity 1") {
        val rb       = new SpscRingBuffer[String](1)
        val offered  = rb.offer("only")
        val full     = rb.isFull
      ...
      test("repeated offer/take cycles with capacity 1") {
        val rb         = new SpscRingBuffer[String](1)
        var allCorrect = true
    ...
    suite("companion object")(
      test("SpscRingBuffer.apply creates a buffer") {
        val rb = SpscRingBuffer[String](8)
        assertTrue(rb.capacity == 8, rb.isEmpty)
      }
    ),
    suite("drain")(
      test("drain from empty returns 0") {
        val rb = new SpscRingBuffer[String](4)
        val n  = rb.drain(_ => (), 10)
        assertTrue(n == 0)
      },
      test("drain returns correct elements in FIFO order") {
        val rb = new SpscRingBuffer[String](8)
        rb.offer("a")
        rb.offer("b")
        rb.offer("c")
        val result = scala.collection.mutable.ArrayBuffer.empty[String]
        val n      = rb.drain(e => result += e, 10)
        assertTrue(n == 3, result.toVector == Vector("a", "b", "c"))
      },
      test("drain negative limit throws") {
        val rb = new SpscRingBuffer[String](4)
        assertTrue(throws(rb.drain(_ => (), -1)))
      },
      test("drain with limit=0 returns 0 without consuming") {
        val rb = new SpscRingBuffer[String](4)
        rb.offer("a")
        val n = rb.drain(_ => (), 0)
    ...
    suite("fill")(
      test("fill fills up to limit") {
        val rb                     = new SpscRingBuffer[String](8)
        var counter                = 0
        val supplier: () => String = () => { val v = s"v$counter"; counter += 1; v }
        val n                      = rb.fill(supplier, 3)
        assertTrue(n == 3, rb.size == 3, rb.take() == "v0", rb.take() == "v1", rb.take() == "v2")
      },
      test("fill stops when buffer is full") {
        val rb                     = new SpscRingBuffer[String](4)
        var counter                = 0
        val supplier: () => String = () => { val v = s"v$counter"; counter += 1; v }
        val n                      = rb.fill(supplier, 100)
        assertTrue(n == 4, rb.isFull)
      },
      test("fill negative limit throws") {
        val rb = new SpscRingBuffer[String](4)
        assertTrue(throws(rb.fill(() => "x", -1)))
      },
      test("fill with limit=0 returns 0 without inserting") {
        val rb = new SpscRingBuffer[String](4)
        val n  = rb.fill(() => "x", 0)
        assertTrue(n == 0, rb.isEmpty)
      },
      test("fill rejects null from supplier") {
        val rb = new SpscRingBuffer[String](4)
        assertTrue(throwsNPE(rb.fill(() => null, 1)))
      }
    )
  )
