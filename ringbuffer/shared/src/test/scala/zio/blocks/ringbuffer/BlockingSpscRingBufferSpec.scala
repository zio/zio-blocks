...
object BlockingSpscRingBufferSpec extends ZIOSpecDefault {

  def spec = suite("BlockingSpscRingBuffer")(
    suite("constructor")(
      test("capacity 0 throws") {
        assertTrue(throws(new BlockingSpscRingBuffer[String](0)))
      },
      test("capacity 3 (non-power-of-2) throws") {
        assertTrue(throws(new BlockingSpscRingBuffer[String](3)))
      },
      test("negative capacity throws") {
        assertTrue(throws(new BlockingSpscRingBuffer[String](-1)))
      },
      ...
      test("capacity 1024 succeeds") {
        val rb = new BlockingSpscRingBuffer[String](1024)
    ...
    suite("FIFO ordering via tryOffer/tryTake")(
      test("elements are taken in tryOffer order") {
        val rb    = new BlockingSpscRingBuffer[String](8)
    ...
    suite("companion object")(
      test("BlockingSpscRingBuffer.apply creates a buffer") {
        val rb = BlockingSpscRingBuffer[String](8)
