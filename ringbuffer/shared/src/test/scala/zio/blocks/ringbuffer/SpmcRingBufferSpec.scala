...
object SpmcRingBufferSpec extends ZIOSpecDefault {

  def spec = suite("SpmcRingBuffer")(
    suite("constructor")(
      test("capacity 0 throws") {
        assertTrue(throws(new SpmcRingBuffer[String](0)))
      },
      test("capacity 3 (non-power-of-2) throws") {
        assertTrue(throws(new SpmcRingBuffer[String](3)))
      },
      test("negative capacity throws") {
        assertTrue(throws(new SpmcRingBuffer[String](-1)))
      },
      test("capacity 1 succeeds") {
        val rb = new SpmcRingBuffer[String](1)
        assertTrue(rb.capacity == 1)
      },
      test("capacity 2 succeeds") {
        val rb = new SpmcRingBuffer[String](2)
        assertTrue(rb.capacity == 2)
      },
      test("capacity 1024 succeeds") {
        val rb = new SpmcRingBuffer[String](1024)
        assertTrue(rb.capacity == 1024)
      }
    ),
    suite("single-element offer/take")(
      test("offer then take returns the same element") {
        val rb      = new SpmcRingBuffer[String](4)
    ...
    suite("FIFO ordering")(
      test("elements are taken in offer order") {
        val rb    = new SpmcRingBuffer[String](8)
      ...
      test("partial fill and drain preserves order") {
        val rb    = new SpmcRingBuffer[String](16)
    ...
    suite("companion object")(
      test("SpmcRingBuffer.apply creates a buffer") {
        val rb = SpmcRingBuffer[String](8)
