...
object MpscRingBufferSpec extends ZIOSpecDefault {

  def spec = suite("MpscRingBuffer")(
    suite("constructor")(
      test("capacity 0 throws") {
        assertTrue(throws(new MpscRingBuffer[String](0)))
      },
      test("capacity 3 (non-power-of-2) throws") {
        assertTrue(throws(new MpscRingBuffer[String](3)))
      },
      test("negative capacity throws") {
        assertTrue(throws(new MpscRingBuffer[String](-1)))
      },
      test("capacity 1 succeeds") {
        val rb = new MpscRingBuffer[String](1)
        assertTrue(rb.capacity == 1)
      },
      test("capacity 2 succeeds") {
        val rb = new MpscRingBuffer[String](2)
        assertTrue(rb.capacity == 2)
      },
      test("capacity 1024 succeeds") {
        val rb = new MpscRingBuffer[String](1024)
        assertTrue(rb.capacity == 1024)
      }
    ),
    suite("single-element offer/take")(
      test("offer then take returns the same element") {
        val rb      = new MpscRingBuffer[String](4)
    ...
    suite("FIFO ordering")(
      ...
      test("partial fill and drain preserves order") {
        val rb    = new MpscRingBuffer[String](16)
    ...
    suite("companion object")(
      test("MpscRingBuffer.apply creates a buffer") {
        val rb = MpscRingBuffer[String](8)
