...
object BlockingMpmcRingBufferSpec extends ZIOSpecDefault {

  def spec = suite("BlockingMpmcRingBuffer")(
    suite("constructor")(
      test("capacity 0 throws") {
        assertTrue(throws(new BlockingMpmcRingBuffer[String](0)))
      },
      test("capacity 3 (non-power-of-2) throws") {
        assertTrue(throws(new BlockingMpmcRingBuffer[String](3)))
      },
      test("negative capacity throws") {
        assertTrue(throws(new BlockingMpmcRingBuffer[String](-1)))
      },
      test("capacity 1 throws (requires >= 2 for sequence buffer algorithm)") {
        assertTrue(throws(new BlockingMpmcRingBuffer[String](1)))
      },
      test("capacity 2 succeeds") {
        val rb = new BlockingMpmcRingBuffer[String](2)
        assertTrue(rb.capacity == 2)
      },
      test("capacity 1024 succeeds") {
        val rb = new BlockingMpmcRingBuffer[String](1024)
    ...
    suite("tryOffer/tryTake (non-blocking)")(
      ...
      test("FIFO ordering preserved") {
        val rb    = new BlockingMpmcRingBuffer[String](8)
    ...
    suite("companion object")(
      test("BlockingMpmcRingBuffer.apply creates a buffer") {
        val rb = BlockingMpmcRingBuffer[String](8)
