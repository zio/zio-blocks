  ...
  def spec: Spec[TestEnvironment, Any] = suite("Reference")(
    suite("Reference object")(
      ...
      test("Reference round-trips through DynamicValue") {
        ...
        val result = Schema[Reference].fromDynamicValue(Schema[Reference].toDynamicValue(ref))
        assertTrue(
          result.isRight,
    ...
    suite("ReferenceOr[A]")(
      test("can hold a Reference via Ref case") {
        val reference: ReferenceOr[String] = ReferenceOr.Ref(Reference(`$ref` = "#/components/schemas/User"))
        reference match {
          case ReferenceOr.Ref(ref) => assertTrue(ref.`$ref` == "#/components/schemas/User")
          case ReferenceOr.Value(_) => assertTrue(false)
        }
      },
      test("can hold a concrete value via Value case") {
        val value: ReferenceOr[String] = ReferenceOr.Value("Hello, World!")
        value match {
          case ReferenceOr.Ref(_)        => assertTrue(false)
          case ReferenceOr.Value(actual) => assertTrue(actual == "Hello, World!")
        }
      },
      test("Ref extends ReferenceOr[Nothing] for variance") {
        val ref: ReferenceOr.Ref                       = ReferenceOr.Ref(Reference(`$ref` = "#/components/schemas/User"))
      ...
      test("ReferenceOr[String] with Ref round-trips through DynamicValue") {
        val ref = ReferenceOr.Ref(
          Reference(
            `$ref` = "#/components/schemas/User",
            summary = Some(doc("User reference"))
          )
        )
        val result = Schema[ReferenceOr[String]].fromDynamicValue(Schema[ReferenceOr[String]].toDynamicValue(ref))
        assertTrue(
          result.isRight,
          result.exists {
      ...
      test("ReferenceOr[String] with Value round-trips through DynamicValue") {
        val value  = ReferenceOr.Value("concrete value")
        val result = Schema[ReferenceOr[String]].fromDynamicValue(Schema[ReferenceOr[String]].toDynamicValue(value))
        assertTrue(
          result.isRight,
          result.exists {
      ...
      test("ReferenceOr[Info] with concrete Info value round-trips") {
        val value  = ReferenceOr.Value(Info(title = "Test API", version = "1.0.0"))
        val result = Schema[ReferenceOr[Info]].fromDynamicValue(Schema[ReferenceOr[Info]].toDynamicValue(value))
        assertTrue(
          result.isRight,
          result.exists {
      ...
      test("ReferenceOr[Info] with Ref round-trips") {
        val ref    = ReferenceOr.Ref(Reference(`$ref` = "#/components/info/AppInfo"))
        val result = Schema[ReferenceOr[Info]].fromDynamicValue(Schema[ReferenceOr[Info]].toDynamicValue(ref))
        assertTrue(
          result.isRight,
          result.exists {
            case ReferenceOr.Ref(reference) => reference.`$ref` == "#/components/info/AppInfo"
            case _                          => false
          }
        )
      },
      test("ReferenceOr works with complex nested types") {
        ...
        val value  = ReferenceOr.Value(ComplexType("test", 5, List("tag1", "tag2")))
        val result =
          Schema[ReferenceOr[ComplexType]].fromDynamicValue(Schema[ReferenceOr[ComplexType]].toDynamicValue(value))
        assertTrue(
          result.isRight,
          result.exists {
            case ReferenceOr.Value(c) => c.name == "test" && c.count == 5 && c.tags == List("tag1", "tag2")
            case _                    => false
          }
        )
      },
