  ...
  def spec: Spec[TestEnvironment, Any] = suite("OpenAPIRoundTripSpec")(
    suite("JSON round-trip tests")(
      test("minimal.json round-trips through JSON") {
        ...
        assertTrue(Json.parse(jsonString).isRight)
      },
    ...
    suite("Basic types round-trip through DynamicValue")(
      test("Info round-trips with all fields") {
        ...
        val result = Schema[Info].fromDynamicValue(Schema[Info].toDynamicValue(original))
      ...
      test("Contact round-trips with all fields") {
        ...
        val result = Schema[Contact].fromDynamicValue(Schema[Contact].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      ...
      test("License round-trips with identifier") {
        ...
        val result = Schema[License].fromDynamicValue(Schema[License].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
    ...
    suite("Server types round-trip through DynamicValue")(
      test("Server round-trips with all fields") {
        ...
        val result = Schema[Server].fromDynamicValue(Schema[Server].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      ...
      test("ServerVariable round-trips with enum") {
        ...
        val result = Schema[ServerVariable].fromDynamicValue(Schema[ServerVariable].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
    ...
    suite("Tag and ExternalDocumentation round-trip")(
      test("Tag round-trips with all fields") {
        ...
        val result = Schema[Tag].fromDynamicValue(Schema[Tag].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      ...
      test("ExternalDocumentation round-trips") {
        ...
        val result =
          Schema[ExternalDocumentation].fromDynamicValue(Schema[ExternalDocumentation].toDynamicValue(original))
        assertTrue(result == Right(original))
      }
    ),
    suite("Reference and ReferenceOr round-trip")(
      test("Reference round-trips") {
        ...
        val result = Schema[Reference].fromDynamicValue(Schema[Reference].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      test("ReferenceOr.Ref round-trips") {
        ...
        val result = Schema[ReferenceOr[String]].fromDynamicValue(Schema[ReferenceOr[String]].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
    ...
    suite("Schema-related types round-trip")(
      test("SchemaObject round-trips with all fields") {
        ...
        val result = Schema[SchemaObject].fromDynamicValue(Schema[SchemaObject].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      test("OpenAPIDiscriminator round-trips") {
        ...
        val result = Schema[Discriminator].fromDynamicValue(Schema[Discriminator].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      test("XML round-trips with all fields") {
        ...
        val result = Schema[XML].fromDynamicValue(Schema[XML].toDynamicValue(original))
        assertTrue(result == Right(original))
      }
    ),
    suite("Operation and Parameter types round-trip")(
      test("Operation round-trips with all fields") {
        ...
        val result = Schema[Operation].fromDynamicValue(Schema[Operation].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      test("Parameter round-trips with query location") {
        ...
        val result = Schema[Parameter].fromDynamicValue(Schema[Parameter].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      ...
      test("Header round-trips") {
        ...
        val result = Schema[Header].fromDynamicValue(Schema[Header].toDynamicValue(original))
        assertTrue(result == Right(original))
      }
    ),
    suite("Request and Response types round-trip")(
      test("RequestBody round-trips") {
        ...
        val result = Schema[RequestBody].fromDynamicValue(Schema[RequestBody].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      test("MediaType round-trips with all fields") {
        ...
        val result = Schema[MediaType].fromDynamicValue(Schema[MediaType].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      test("Encoding round-trips") {
        val original = Encoding(
          contentType = Some("application/json"),
          headers = ChunkMap("X-Custom" -> ReferenceOr.Value(Header(description = Some(doc("Custom header"))))),
          style = Some("form"),
          explode = Some(false),
          allowReserved = true,
          extensions = ChunkMap("x-enc-id" -> Json.Number(1))
        )
        val result = Schema[Encoding].fromDynamicValue(Schema[Encoding].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      test("Responses round-trips") {
        ...
        val result = Schema[Responses].fromDynamicValue(Schema[Responses].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      test("Response round-trips") {
        ...
        val result = Schema[Response].fromDynamicValue(Schema[Response].toDynamicValue(original))
        assertTrue(result == Right(original))
      }
    ),
    suite("Example and Link types round-trip")(
      test("Example round-trips with value") {
        ...
        val result = Schema[Example].fromDynamicValue(Schema[Example].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      ...
      test("Link round-trips with operationRef") {
        ...
        val result = Schema[Link].fromDynamicValue(Schema[Link].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      test("Link round-trips with operationId") {
        val original = Link(
          operationRef = Some("getUserById"),
    ...
    suite("Callback round-trip")(
      test("Callback round-trips") {
        ...
        val result = Schema[Callback].fromDynamicValue(Schema[Callback].toDynamicValue(original))
        assertTrue(result == Right(original))
      }
    ),
    suite("Security types round-trip")(
      test("SecurityScheme.APIKey round-trips") {
        ...
        val result = Schema[SecurityScheme].fromDynamicValue(Schema[SecurityScheme].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      ...
      test("OAuthFlows round-trips with all flows") {
        ...
        val result = Schema[OAuthFlows].fromDynamicValue(Schema[OAuthFlows].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      test("OAuthFlow round-trips") {
        ...
        val result = Schema[OAuthFlow].fromDynamicValue(Schema[OAuthFlow].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      test("SecurityRequirement round-trips") {
        val original = SecurityRequirement(
          requirements = ChunkMap("oauth2" -> Chunk("read", "write"), "api_key" -> Chunk.empty)
        )
        val result = Schema[SecurityRequirement].fromDynamicValue(Schema[SecurityRequirement].toDynamicValue(original))
        assertTrue(result == Right(original))
      }
    ),
    suite("Container types round-trip")(
      test("Components round-trips") {
        val original = Components(
          ...
          links = ChunkMap("next" -> ReferenceOr.Value(Link(operationId = Some("getNext")))),
        ...
        val result = Schema[Components].fromDynamicValue(Schema[Components].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      test("Paths round-trips") {
        ...
        val result = Schema[Paths].fromDynamicValue(Schema[Paths].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
      test("PathItem round-trips") {
        ...
        val result = Schema[PathItem].fromDynamicValue(Schema[PathItem].toDynamicValue(original))
        assertTrue(result == Right(original))
      }
    ),
    suite("OpenAPI root object round-trip")(
      test("OpenAPI round-trips with minimal fields") {
        val original = OpenAPI(openapi = "3.1.0", info = Info(title = "Test API", version = "1.0.0"))
        val result   = Schema[OpenAPI].fromDynamicValue(Schema[OpenAPI].toDynamicValue(original))
        assertTrue(result == Right(original))
      },
    ...
    suite("Extension preservation on all extensible types")(
      ...
      test("extensions preserved on Operation") {
        val original = Operation(extensions = ChunkMap("x-rate-limit" -> Json.Number(100)))
        val result   = Schema[Operation].fromDynamicValue(Schema[Operation].toDynamicValue(original))
        assertTrue(result.exists(_.extensions == original.extensions))
      },
