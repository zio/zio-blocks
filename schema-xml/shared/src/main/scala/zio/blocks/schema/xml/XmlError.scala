...
class XmlError(
  ...
  override def getMessage: String = line match {
    case Some(l) =>
      column match {
        case Some(c) => s"$message (at line $l, column $c)"
        case _       => s"$message (at line $l)"
      }
    case _ => message
  }

  /** Returns the path as a DynamicOptic. */
  def path: DynamicOptic = new DynamicOptic(spans.reverse.toIndexedSeq)

  /** Creates a new XmlError with an additional span prepended. */
  def atSpan(span: DynamicOptic.Node): XmlError = new XmlError(span :: spans, message, line, column)
}
...
object XmlError {
  ...
  def apply(message: String, spans: List[DynamicOptic.Node]): XmlError = new XmlError(spans, message)

  /** Creates an XmlError with a message and line/column position. */
  def apply(message: String, line: Int, column: Int): XmlError =
    new XmlError(Nil, message, new Some(line), new Some(column))

  /** Parse error - malformed XML. */
  def parseError(message: String, line: Int, column: Int): XmlError =
    new XmlError(Nil, s"Parse error: $message", new Some(line), new Some(column))

  /** Validation error - invalid content. */
  def validationError(message: String): XmlError = new XmlError(Nil, s"Validation error: $message")

  /** Encoding error - cannot encode value. */
  def encodingError(message: String): XmlError = new XmlError(Nil, s"Encoding error: $message")

  /** Selection error - path not found. */
  def selectionError(message: String): XmlError = new XmlError(Nil, s"Selection error: $message")

  /** Patch error - patch operation failed. */
  def patchError(message: String): XmlError = new XmlError(Nil, s"Patch error: $message")
}
