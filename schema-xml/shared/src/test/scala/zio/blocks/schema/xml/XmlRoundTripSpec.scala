...
object XmlRoundTripSpec extends ZIOSpecDefault {
  def spec: Spec[TestEnvironment, Any] = suite("XmlRoundTripSpec")(
    test("RSS feed round-trip") {
      ...
      val parsed1Result = Try(XmlReader.read(rssXml)).toEither
      val parsed1       = parsed1Result match {
        case Right(xml) => xml
        case Left(err)  => throw new Exception(s"Failed to parse RSS XML: ${err.getMessage}")
      }
      val written       = XmlWriter.write(parsed1)
      val parsed2Result = Try(XmlReader.read(written)).toEither
    ...
    test("Atom feed round-trip") {
      ...
      val parsed1Result = Try(XmlReader.read(atomXml)).toEither
      val parsed1       = parsed1Result match {
        case Right(xml) => xml
    ...
    test("Sitemap round-trip") {
      ...
      val parsed1Result = Try(XmlReader.read(sitemapXml)).toEither
      val parsed1       = parsed1Result match {
        case Right(xml) => xml
