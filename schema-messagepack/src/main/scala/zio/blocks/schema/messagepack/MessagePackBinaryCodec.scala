...
import org.msgpack.core.{MessagePack, MessageUnpacker, MessageBufferPacker}
...
abstract class MessagePackBinaryCodec[A](val valueType: Int = MessagePackBinaryCodec.objectType)
  ...
  private[this] def decode(unpacker: MessageUnpacker): Either[SchemaError, A] =
    try {
      ...
      if (unpacker.hasNext()) {
        // In some cases we might just want to consume one object, but generally for a full decode we expect consumption
        // For strict checking we could assert !unpacker.hasNext() if it was a single object decode
      }
      new Right(res)
    } catch {
