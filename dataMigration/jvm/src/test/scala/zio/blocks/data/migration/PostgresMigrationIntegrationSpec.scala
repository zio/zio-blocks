...
import zio.blocks.sql.{DbCodec, DbCodecDeriver, DbTx, Dialect, Frag, JdbcTransactor, Repo, SqlDialect, Table}
  ...
  private def withPgDb[A](f: JdbcTransactor => A): A = {
    ...
    try {
      tx.transact { (c: DbTx) ?=>
        ...
        Frag
          .literal(
            "CREATE TABLE IF NOT EXISTS v2 (id INTEGER NOT NULL, name TEXT NOT NULL, age INTEGER NOT NULL DEFAULT 0, PRIMARY KEY (id))"
          )
          .update
      }
      f(tx)
      ...
      suite("PostgresMigrationIntegration")(
        ...
        test("LargeMigrator fence/drain/complete protocol") {
          withPgDb { tx =>
            ...
            tx.transact { (c2: DbTx) ?=>
              Frag
                .literal("INSERT INTO v1 (id, name) VALUES (1, 'a'), (2, 'b'), (3, 'c') ON CONFLICT (id) DO NOTHING")
                .update
              Frag
                .literal(
                  "INSERT INTO v2 (id, name, age) VALUES (1, 'a', 0), (2, 'b', 0), (3, 'c', 0) ON CONFLICT (id) DO NOTHING"
                )
                .update
            }
            val migrator = LargeMigrator[V1, V2, Int, Int](
