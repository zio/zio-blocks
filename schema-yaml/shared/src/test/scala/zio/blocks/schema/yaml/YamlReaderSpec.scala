  ...
  def spec: Spec[TestEnvironment, Any] = suite("YamlReader")(
    ...
    suite("document markers")(
      test("document marker followed by content") {
        val input = "---\nname: John"
        assertTrue(YamlReader.read(input) == Yaml.Mapping.fromStringKeys("name" -> Yaml.Scalar("John")))
      },
    ...
    suite("block mappings")(
      ...
      test("multiple key-value pairs") {
        ...
        assertTrue(
          result ==
            Yaml.Mapping(Chunk((Yaml.Scalar("name"), Yaml.Scalar("Alice")), (Yaml.Scalar("age"), Yaml.Scalar("30"))))
        )
      },
      test("nested mapping") {
        val input  = "person:\n  name: Bob\n  age: 25"
        val result = YamlReader.read(input)
        assertTrue(
          result == Yaml.Mapping.fromStringKeys(
      ...
      test("key with null value (empty after colon)") {
        val input  = "key:"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("key" -> Yaml.NullValue))
      },
      test("key with null value (nothing after colon, more entries follow)") {
        val input  = "key:\nother: value"
        val result = YamlReader.read(input)
        assertTrue(
          result == Yaml.Mapping.fromStringKeys("key" -> Yaml.NullValue, "other" -> Yaml.Scalar("value"))
        )
      },
      test("mapping with inline flow value") {
        val input  = "items: {a: 1, b: 2}"
        val result = Try(YamlReader.read(input)).toEither
    ...
    suite("block sequences")(
      test("simple sequence") {
        val input  = "- apple\n- banana\n- cherry"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Sequence(Chunk(Yaml.Scalar("apple"), Yaml.Scalar("banana"), Yaml.Scalar("cherry"))))
      },
      test("sequence of mappings") {
        val input  = "- name: Alice\n  age: 30\n- name: Bob\n  age: 25"
        val result = YamlReader.read(input)
        assertTrue(
          result == Yaml.Sequence(
            Chunk(
      ...
      test("sequence with empty item (dash only)") {
        val input  = "-\n- value"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Sequence(Chunk(Yaml.NullValue, Yaml.Scalar("value"))))
      },
      test("sequence with dash only at end") {
        val input  = "- a\n-"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Sequence(Chunk(Yaml.Scalar("a"), Yaml.NullValue)))
      },
      test("sequence item with nested block value") {
        val input  = "-\n  name: test"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Sequence(Chunk(Yaml.Mapping.fromStringKeys("name" -> Yaml.Scalar("test")))))
      },
    ...
    suite("flow mappings")(
      test("empty flow mapping") {
        ...
        assertTrue(result == Yaml.Mapping.empty)
      },
      test("simple flow mapping") {
        val result = YamlReader.read("{name: Alice, age: 30}")
        assertTrue(
          result == Yaml.Mapping(
            Chunk((Yaml.Scalar("name"), Yaml.Scalar("Alice")), (Yaml.Scalar("age"), Yaml.Scalar("30")))
          )
        )
      },
      test("nested flow mapping") {
        ...
        assertTrue(
          result == Yaml.Mapping(
            Chunk((Yaml.Scalar("outer"), Yaml.Mapping(Chunk((Yaml.Scalar("inner"), Yaml.Scalar("value"))))))
          )
        )
      },
    ...
    suite("flow sequences")(
      test("empty flow sequence") {
        val result = YamlReader.read("[]")
        assertTrue(result == Yaml.Sequence.empty)
      },
      test("simple flow sequence") {
        val result = YamlReader.read("[a, b, c]")
        assertTrue(result == Yaml.Sequence(Chunk(Yaml.Scalar("a"), Yaml.Scalar("b"), Yaml.Scalar("c"))))
      },
      ...
      test("flow sequence with commas inside double-quoted strings") {
        val result = YamlReader.read("[\"a,b\", c]")
        assertTrue(result == Yaml.Sequence(Chunk(Yaml.Scalar("a,b"), Yaml.Scalar("c"))))
      },
      test("flow mapping with commas inside double-quoted value") {
        val result = YamlReader.read("{a: \"x,y\"}")
        assertTrue(result == Yaml.Mapping(Chunk((Yaml.Scalar("a"), Yaml.Scalar("x,y")))))
      },
    ...
    suite("quoted strings")(
      test("single-quoted string") {
        ...
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("name" -> Yaml.Scalar("hello world")))
      },
      test("single-quoted with escaped single quote") {
        val input  = "name: 'it''s'"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("name" -> Yaml.Scalar("it's")))
      },
      ...
      test("double-quoted with escape sequences") {
        ...
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("value" -> Yaml.Scalar("line1\nline2\ttab")))
      },
      test("double-quoted with backslash escape") {
        val input  = """value: "path\\to\\file""""
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("value" -> Yaml.Scalar("path\\to\\file")))
      },
      test("double-quoted with quote escape") {
        val input  = """value: "say \"hello\"""""
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("value" -> Yaml.Scalar("say \"hello\"")))
      },
      test("double-quoted with carriage return escape") {
        val input  = """value: "a\rb""""
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("value" -> Yaml.Scalar("a\rb")))
      },
      test("double-quoted with bell escape") {
        val input  = """value: "a\ab""""
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("value" -> Yaml.Scalar("a\u0007b")))
      },
      ...
      test("double-quoted with backspace escape") {
        ...
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("value" -> Yaml.Scalar("a\bb")))
      },
      ...
      test("double-quoted with unicode escape") {
        val input  = "value: \"" + "\\u0041" + "\""
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("value" -> Yaml.Scalar("A")))
      },
      ...
      test("key starting with quote does not find mapping colon") {
        ...
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Scalar("quoted value"))
      },
    ...
    suite("literal blocks")(
      test("literal block (|)") {
        val input  = "desc: |\n  line1\n  line2"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("desc" -> Yaml.Scalar("line1\nline2")))
      },
      test("folded block (>)") {
        val input  = "desc: >\n  line1\n  line2"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("desc" -> Yaml.Scalar("line1 line2")))
      },
      test("literal block at top level") {
        val input  = "|\n  line1\n  line2"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Scalar("line1\nline2"))
      },
      test("folded block at top level") {
        val input  = ">\n  line1\n  line2"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Scalar("line1 line2"))
      },
      test("literal block with empty content") {
        val input  = "desc: |"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("desc" -> Yaml.Scalar("")))
      },
      ...
      test("literal block content with trailing newline is trimmed") {
        val input  = "|\n  hello\n"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Scalar("hello"))
      },
      test("mapping value with > indicator") {
        val input  = "key: >\n  folded line"
        val result = YamlReader.read(input)
        assertTrue(
          result == Yaml.Mapping.fromStringKeys("key" -> Yaml.Scalar("folded line"))
        )
      },
    ...
    suite("comments")(
      test("inline comment on value") {
        ...
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("name" -> Yaml.Scalar("Alice")))
      },
      test("comment-only line between entries") {
        val input  = "a: 1\n# comment\nb: 2"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("a" -> Yaml.Scalar("1"), "b" -> Yaml.Scalar("2")))
      },
      test("hash in single-quoted string is not a comment") {
        val input  = "val: 'not # a comment'"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("val" -> Yaml.Scalar("not # a comment")))
      },
      ...
      test("hash without preceding space is not comment") {
        ...
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("val" -> Yaml.Scalar("foo#bar")))
      }
    ),
    suite("CRLF line endings")(
      test("CRLF is normalized") {
        val input  = "name: Alice\r\nage: 30"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Mapping.fromStringKeys("name" -> Yaml.Scalar("Alice"), "age" -> Yaml.Scalar("30")))
      },
    ...
    suite("booleans and special values")(
      test("true scalar") {
        ...
        assertTrue(result == Yaml.Scalar("true"))
      },
      test("false scalar") {
        val result = YamlReader.read("false")
        assertTrue(result == Yaml.Scalar("false"))
      }
    ),
    suite("readFromBytes")(
      test("reads from byte array") {
        ...
        assertTrue(result == Yaml.Mapping.fromStringKeys("name" -> Yaml.Scalar("Test")))
      }
    ),
    ...
    suite("edge cases")(
      ...
      test("block sequence with inline scalar") {
        val input  = "- hello"
        val result = YamlReader.read(input)
        assertTrue(result == Yaml.Sequence(Chunk(Yaml.Scalar("hello"))))
      }
    )
  )
}
