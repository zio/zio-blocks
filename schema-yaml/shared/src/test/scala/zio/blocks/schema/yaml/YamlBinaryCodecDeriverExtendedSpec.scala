...
object YamlBinaryCodecDeriverExtendedSpec extends YamlBaseSpec {
  ...
  def spec: Spec[TestEnvironment, Any] = suite("YamlBinaryCodecDeriverExtended")(
    suite("primitive field types in records")(
      test("round-trip Byte field") {
        val codec  = Schema[WithByte].derive(YamlBinaryCodecDeriver)
      ...
      test("round-trip Short field") {
        val codec  = Schema[WithShort].derive(YamlBinaryCodecDeriver)
      ...
      test("round-trip BigDecimal field") {
        val codec  = Schema[WithBigDecimal].derive(YamlBinaryCodecDeriver)
      ...
      test("round-trip all primitive types in single record") {
        val codec  = Schema[AllPrimitives].derive(YamlBinaryCodecDeriver)
    ...
    suite("java.time primitive codecs")(
      ...
      test("round-trip LocalDate") {
        val codec  = Schema[LocalDate].derive(YamlBinaryCodecDeriver)
      ...
      test("round-trip LocalDateTime") {
        val codec  = Schema[LocalDateTime].derive(YamlBinaryCodecDeriver)
      ...
      test("round-trip OffsetTime") {
        val codec  = Schema[OffsetTime].derive(YamlBinaryCodecDeriver)
      ...
      test("round-trip ZoneId") {
        val codec  = Schema[ZoneId].derive(YamlBinaryCodecDeriver)
      ...
      test("round-trip Currency") {
        val codec  = Schema[Currency].derive(YamlBinaryCodecDeriver)
      ...
      test("round-trip UUID") {
        val codec  = Schema[UUID].derive(YamlBinaryCodecDeriver)
    ...
    suite("DynamicValue codec")(
      test("encode/decode DynamicValue.Primitive String") {
        val codec   = Schema[DynamicValue].derive(YamlBinaryCodecDeriver)
    ...
    suite("error cases in deriver")(
      test("missing required field returns error") {
        val codec  = Schema[Wrapper].derive(YamlBinaryCodecDeriver)
        val result = codec.decodeValue(Yaml.Mapping.empty)
        assertTrue(result.isLeft)
      },
      ...
      test("wrong YAML type for variant returns error") {
        ...
        val codec  = Schema[Color].derive(YamlBinaryCodecDeriver)
        val result = codec.decodeValue(Yaml.Scalar("not a mapping"))
        assertTrue(result.isLeft)
      },
      test("non-string key for variant returns error") {
        ...
        val codec = Schema[Shape].derive(YamlBinaryCodecDeriver)
        import zio.blocks.chunk.Chunk
      ...
      test("sequence expects Yaml.Sequence") {
        val codec  = Schema[List[Int]].derive(YamlBinaryCodecDeriver)
        val result = codec.decodeValue(Yaml.Scalar("not a sequence"))
        assertTrue(result.isLeft)
      },
      test("map expects Yaml.Mapping") {
        val codec  = Schema[Map[String, Int]].derive(YamlBinaryCodecDeriver)
      ...
      test("map key error propagation") {
        val codec = Schema[Map[Int, String]].derive(YamlBinaryCodecDeriver)
        import zio.blocks.chunk.Chunk
    ...
    suite("Option handling")(
      ...
      test("nullValue for Option is None") {
        val codec = Schema[Option[String]].derive(YamlBinaryCodecDeriver)
        assertTrue(codec.nullValue == None)
      }
    ),
    suite("sequence and map codecs")(
      test("encode/decode empty list") {
        val codec  = Schema[List[String]].derive(YamlBinaryCodecDeriver)
      ...
      test("nullValue for List is empty") {
        val codec = Schema[List[String]].derive(YamlBinaryCodecDeriver)
        assertTrue(codec.nullValue == List.empty)
      },
      ...
      test("nullValue for Map is empty") {
        val codec = Schema[Map[String, Int]].derive(YamlBinaryCodecDeriver)
        assertTrue(codec.nullValue == Map.empty[String, Int])
      }
    ),
    suite("YamlFormat")(
    ...
    suite("toKebabCase")(
      test("camelCase field names are kebab-cased") {
        ...
        val codec = Schema[CamelCase].derive(YamlBinaryCodecDeriver)
    ...
    suite("record field error propagation")(
      test("invalid field value returns error") {
        ...
        val codec  = Schema[TypedRecord].derive(YamlBinaryCodecDeriver)
    ...
    suite("variant mapping with multiple entries fails")(
      test("mapping with 2 entries fails variant decode") {
        ...
        val codec = Schema[V].derive(YamlBinaryCodecDeriver)
        import zio.blocks.chunk.Chunk
